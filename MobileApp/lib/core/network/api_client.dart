...
class ApiClient {
  ...
  ApiClient() {
    // The base URL goes through [EnvironmentConfig.url] so a loopback host is
    // rewritten to `10.0.2.2` on the Android emulator (see EnvironmentConfig.url).
    final baseUrl = EnvironmentConfig.url(
  ...
  Future<Response<T>> _handleRequest<T>(
    ...
    try {
      ...
      if (e.response != null) {
        ...
        final data = body is Map ? body.cast<String, dynamic>() : null;
...
class _AuthInterceptor extends Interceptor {
  ...
  /// with it any boot-time fetch — a caller then spins indefinitely
  /// instead of redirecting to login). Capping it turns a stalled
  /// refresh into a clean sign-out via [onUnauthorized].

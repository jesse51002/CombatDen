...
/// rather than on a new slot kind inside the shared `theme_flutter`
/// package. A platform adopting the engine should not have to change
/// its architecture to get layout selection, and this app is the
/// reference implementation of that constraint.
///
/// The precedence chain itself lives in [FormatResolver], shared with
/// `ThemeMotion` and the dev picker, so all three can never disagree
/// about what is live. If the engine later promotes layout to a
/// first-class `layout_set`, only [FormatResolver] changes; every call
/// site and every screen stays as it is.
class ThemeLayout {

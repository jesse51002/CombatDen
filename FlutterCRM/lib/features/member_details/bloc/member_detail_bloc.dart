...
class MemberDetailBloc
  ...
  void _onPageChanged(
    ...
    final s = state;
    if (s is! MemberDetailLoaded) return;
    emit(s.copyWith(currentMembershipIndex: event.pageIndex));
  }
  ...
  Future<void> _onLinkParent(
    ...
    await _runMutation(
      ...
      action: () => _repository.linkMemberAccount(
        event.childCrmUserId ?? s.member.crmUserId,
        event.parentCrmUserId,
      ),
    );
  }

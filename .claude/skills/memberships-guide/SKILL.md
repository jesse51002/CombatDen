...
itself is owned by `sync-guide` too. **Two task-era exceptions:** `update_price`
only validates + creates the `membership_reprice` task (no lock wrap, no sync —
the executor locks and converges itself, in the background), and every
ITEM-targeted op (cancel, mark_paid_cash, add/remove discounts, update_price)
first runs the **in-task guard** (`TasksService.assert_memberships_not_in_task`)
— a membership referenced by a pending/running task item rejects with
`MembershipInTaskError` → HTTP 409. Most ops also expose a preview (a `preview_*` method or a
...
| **update_price** (`memberships_update_price.py` + `memberships_reprice.py`) | the **opt-in price upgrade**, run as a tracked **`membership_reprice` task** (202 + task_id; the CRM polls `GET /tasks/{id}`). The endpoint half validates (membership live, plan's single `is_active` price via `member_memberships_get_active_price.sql`, **rejects a no-op** — already on the active price → 400, never a task — and the in-task guard → 409), creates the task + item (typed params: `target_price_id`, `prorate`) and fires it in the background. The operation itself is **`MemberMembershipsReprice.reprice(member_id, old_item_id, target_price_id, prorate)` — fully TASK-AGNOSTIC** (plain params in, the successor's item_id out; it knows nothing about tasks): under the family lock it re-validates → `_pre_sync_payments` → ONE txn (cancel old row **effective today** via `member_memberships_cancel_immediate.sql` + insert successor at the new price (`not_added`) + COPY live applied discounts onto it via `applied_discounts/copy_applied_discounts.sql`) → convergent sync → verify successor `applied` AND old row `deleted`. It is **idempotent on its own**: a re-run that finds the old row cancelled with a live successor at the target price (`member_memberships_find_successor.sql`) resumes straight to the converge. The task linkage lives only in the thin adapter `MemberMembershipsRepriceTaskHandler` (`memberships_reprice_task_handler.py`, the registered `membership_reprice` handler): it translates item params and stamps `task_items.new_item_id` through the reprice's generic in-transaction `record_successor` hook. **No revert** — failures retry (3 attempts, like the bulk sync), then the task is `failed` and the reconciler's push sweep converges the pending successor; staff retry = re-submit. Old applied-discount rows stay pinned to the old row as records; consumed `once` discounts are never re-granted. |
...
- **Membership service:**
  ...
  preview), `memberships_cancel`, `memberships_update_price` (reprice request +
  restaged preview), `memberships_reprice` (the task-agnostic reprice op),
  `memberships_reprice_task_handler` (the thin `membership_reprice`
  task adapter), `memberships_freeze`,

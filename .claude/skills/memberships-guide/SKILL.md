...
| **update_price** (`memberships_update_price.py` + `memberships_reprice.py`) | the **opt-in price upgrade**, run as a tracked **`membership_reprice` task** (202 + task_id; the CRM polls `GET /tasks/{id}`). The endpoint half validates (membership live, plan's single `is_active` price via `member_memberships_get_active_price.sql`, **rejects a no-op** — already on the active price → 400, never a task — and the in-task guard → 409), creates the task + item (typed params: `target_price_id`, `prorate`) and fires it in the background. The operation itself is **`MemberMembershipsReprice.reprice(member_id, old_item_id, target_price_id, prorate)` — fully TASK-AGNOSTIC and standalone** (plain params in, the successor's item_id out; it knows nothing about tasks and handles its own failure like every other DB-first op): under the family lock it validates → `_pre_sync_payments` → ONE txn (cancel old row **effective today** via `member_memberships_cancel_immediate.sql` + insert successor at the new price (`not_added`) + COPY live applied discounts onto it via `applied_discounts/copy_applied_discounts.sql`) → convergent sync → **verify successor `applied` AND old row `deleted`, else REVERT** (delete the copies via `applied_discounts/delete_copied_discounts.sql` → delete the pending successor → clear the old row's `cancel_date`; skipped if the successor's line already stamped — known-residual doctrine) and raise, leaving the membership exactly as it was. The task linkage lives only in the thin adapter `MemberMembershipsRepriceTaskHandler` (`memberships_reprice_task_handler.py`, the registered `membership_reprice` handler): it translates item params and, on success, records the returned successor onto `task_items.new_item_id`. A failed task therefore changed nothing — its item just carries the error; retry = re-submit. Old applied-discount rows stay pinned to the old row as records; consumed `once` discounts are never re-granted. |

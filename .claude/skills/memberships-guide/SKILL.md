...
| **charge_card** (`memberships_charge_card.py`) | ad-hoc, **outside any subscription**: the request carries an explicit **`paid_by_member_id`** (validated self-or-linked-parent of the beneficiary `member_id`); creates a one-off Stripe invoice for `amount_cents` + `reason` **on the payer's customer** (metadata's `member_id` stays the beneficiary); `paid_cash=true` routes it out of band instead of charging the card. The `invoice.paid` + `invoice_payment.paid` webhooks persist the CRM invoice + charge rows. A **definitive non-collection** — the shared `_require_collected` raising `PaymentsNotCollectedError` on the one-time charge path (see the start row) — is the THIRD route on the 207 contract: the router answers it with `MemberMembershipsChargeCardResponse` (`status=not_collected` + `decline_reason`, **reusing retry-card's status enum and its exact wording** — one spelling, three routes) on **207**, never the 500 it used to get by falling through to the base `PaymentsStripeError` arm (it is a subclass, so that arm must stay BELOW it). **The success contract is untouched: 204, no body** — the 207 is declared in `responses` with its `model`, and since FastAPI forbids a response model on a 204 route the handler returns an explicit `Response` / `JSONResponse`. A `stripe.CardError` on this route is still a 500 — a KNOWN GAP, unlike retry-card's `declined` arm. |
...
| `POST /charge-card` | ad-hoc card/cash charge — a collected charge is **204 with no body** (unchanged); a DEFINITIVE non-collection is a result on **207** (`MemberMembershipsChargeCardResponse`, `status=not_collected` + `decline_reason`), never a 500. A decline is still a 500 (known gap) |

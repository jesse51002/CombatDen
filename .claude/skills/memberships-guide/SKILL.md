...
| `idempotency_key` | nullable UUID; one-time/trial START rows carry a deterministic per-row key `uuid5(request.idempotency_key, "<member_id>:<price_id>")`; recurring + preview rows carry NULL. A partial unique index + `ON CONFLICT (idempotency_key) WHERE idempotency_key IS NOT NULL DO NOTHING` drops duplicate rows on a retried start; a short `RETURNING` result signals a replay and `_crm_insert` raises `MembershipStartReplayError` → **HTTP 409** (the check runs before the insert commits, so a shortfall rolls back; the original rows + their discounts + charge stand unchanged). |
| `created_at` | |

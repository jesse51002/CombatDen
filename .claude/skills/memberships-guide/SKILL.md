...
3. `invoice = await pay(sub, account, idempotency_key=str(key))` — returns the now-PAID invoice
   ...
   **"This invoice is already settled — nothing left to collect."** when there are none — so a
   ...
   WITHOUT collecting (SCA needs authentication, no decline raised) is turned into a
   `PaymentsStripeError` instead — a system-shaped failure (→ **500**), never booked as a phantom
   success and deliberately NOT dressed up as a decline.
...
207 `declined` with the reason, success → 200 `paid`, non-card Stripe failure → 500); the live DB
...
| `POST /retry-card` | retry the payer's saved default card on that same open invoice → `MemberMembershipsRetryCardResponse` (`status=paid` on 200; a DECLINE is a result: `status=declined` + `decline_reason` on **207**, never a 500) |

...
| cardinality | one per offering, shared by many members | **recurring:** one active per (member, gym, plan); **one_time/trial:** may STACK — several at once on the same plan (e.g. two class packs, or buying another before the first is used up) |
...
keyed on the PAYER — item-scoped ops (cancel / update_price / discounts /
...
| **link / unlink** (`linked.py`) | `link_account(member_id, parent_member_id)` sets **only** `members.account_linked_to_id` — a pure relationship change that no longer NULLs the child's stripe/card/freeze fields (the `linked_account_no_stripe` CHECK is **dropped**; a self-paying linked member keeps their own billing state). The link is the **authorization layer** (who may pay for whom), never the billing key. `unlink_account(member_id)` clears it; `check_link_account` is a read-only pre-flight. **Pure DB change — no Stripe sync, no preview.** Both require the member to have **zero active recurring memberships** (`_assert_no_active_recurring`), so the link change can't strand a live subscription. Owns its **own** locking: locks **two accounts** (`[member_id, parent_member_id]` / `[member_id, old_parent_id]`); since `PayingMemberLock` is non-reentrant the facade delegates these **bare** (no outer `lock(...)` wrap). Self-contained — does **not** extend `MemberMembershipsBase`. |

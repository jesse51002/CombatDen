...
| **retry_card** (`memberships_retry_card.py`) | the CARD twin of `mark_paid_cash`, line for line: same recurring-only + linked-to-Stripe + not-canceled validations, same PAYER resolution (`paid_by_member_id`), same open-invoice lookup — but it pays that invoice on the payer's **saved DEFAULT card** instead of out of band. The payment primitive omits BOTH `paid_out_of_band` and `payment_method` (`InvoicePayParams()`), which is exactly what makes Stripe charge the default card, and it stamps NO `crm_paid_with_cash` metadata. Stripe's `invoice.paid` / `invoice_payment.paid` webhooks do the CRM write — the op writes no invoice/charge rows itself. A **decline raises out of Stripe and propagates** (router → 500, never 502/503/504); the membership simply stays overdue. This is the manual re-run of a failed renewal charge, the counterpart to letting Stripe dunning retry it. |
...
`upgrade`, prorating reprice, `mark-paid-cash`, `retry-card` — `MemberMembershipsService` fires a

...
| **authorize / de-authorize payer** (`linked.py`) | **Many-to-many** (`member_authorized_payers`): a member may have MANY authorized payers AND be an authorized payer for others. `link_account(member_id, payer_member_id, *, signer_name, consent_acknowledged, …)` is **sign-gated** — in ONE transaction it records the payer's signature on the gym's **default authorized-payer waiver** (resolved via `WaiversService.get_default_waiver_for_member` → `record_signature`) and inserts the `member_authorized_payers(member_id, payer_member_id, gym_id, signature_id)` row (no orphan signatures). `unlink_account(member_id, payer_member_id)` deletes that row (the signature persists as append-only audit). `check_link_account(member_id, payer_member_id)` is a read-only pre-flight (not-self / payer-in-gym / not-already-authorized). The authorization (who may pay for whom) is `member_authorized_payers`, never the billing key. **Pure DB change — no Stripe sync, no preview, and NO no-active-recurring guard** (an authorization never contributes a membership line, so it can't strand billing). Injects `WaiversService`. Owns its **own** locking: locks the **two accounts** `[member_id, payer_member_id]`; since `PayingMemberLock` is non-reentrant the facade delegates these **bare**. Self-contained — does **not** extend `MemberMembershipsBase`. |

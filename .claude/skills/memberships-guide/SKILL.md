...
from `src/tasks/`** — the reprice that runs as a tracked task is the
standalone, task-agnostic `MemberMembershipsReprice` op; the
request-to-task orchestration and the **in-task guard**
(`TasksService.assert_memberships_not_in_task` — a membership referenced by a
pending/running task item rejects with `MembershipInTaskError` → HTTP 409,
wired into every item-targeted endpoint: cancel, mark_paid_cash, add/remove
discounts, the reprice request) live ABOVE the facade, in the router (the
composition layer). Most ops also expose a preview (a `preview_*` method or a
...
| **reprice** (`memberships_reprice.py` — `MemberMembershipsReprice`) | the **opt-in price upgrade**, run as a tracked **`membership_reprice` task** (`PUT /price` → 202 + task_id; the CRM polls `GET /tasks/{id}`). The op is **fully TASK-AGNOSTIC and standalone** — `reprice(member_id, old_item_id, target_price_id, prorate) -> successor item_id` (plain params in, successor out; imports nothing from `src/tasks`) — and handles its own failure like every other DB-first op: under the family lock it validates → reprices to the **PINNED `target_price_id` as-is** (fetched by id via `member_memberships_get_price.sql`, **NOT re-checked against the plan's *current* active price** — the user pinned what they chose at request time; a newer price created before the task runs must NOT divert or fail the upgrade, and a deactivated CRM price keeps a usable Stripe price since `plans_price.py` never archives one) → `_pre_sync_payments` → ONE txn (cancel old row **effective today** via `member_memberships_cancel_immediate.sql` + insert successor (`not_added`) + COPY live applied discounts via `applied_discounts/copy_applied_discounts.sql`) → convergent sync → **verify successor `applied` AND old row `deleted`, else REVERT** (delete the copies via `applied_discounts/delete_copied_discounts.sql` → delete the pending successor → clear the old row's `cancel_date`; skipped if the successor's line already stamped — known-residual doctrine) and raise, leaving the membership exactly as it was. **A membership already on the target is a no-op** — `reprice` returns the row's own id without touching anything (no defensive re-sync; the reconciler converges any drift), so the task completes with `new_item_id == old_item_id` and nothing bills even with `prorate=True`. Two more pure methods: `resolve_target_price` (fail-fast request validation — membership live, not cancelled/ended; pins the plan's single `is_active` price via `member_memberships_get_active_price.sql`; does NOT reject a no-op) and `preview_reprice` (the staged dry-run). **The task layer is the one bridge:** `MembershipRepriceTaskHandler` (`src/tasks/service/tasks_membership_reprice_handler.py`, the registered `membership_reprice` handler) `create`s the task (validate via `resolve_target_price` → enqueue) and `execute_item`s it (call `reprice` → record the returned successor onto `task_items.new_item_id`); the router orchestrates guard → create → fire. A failed task changed nothing — its item just carries the error; retry = re-submit. Old applied-discount rows stay pinned to the old row as records; consumed `once` discounts are never re-granted. |

...
| `end_date` | non-recurring expiry — **AUTOMATIC-only by convention**: the purchase-stamped duration expiry and the check-in depletion auto-end write it, and the check-in reversal's un-end restores it (back to the duration-derived expiry, or NULL for a pure class-count pack). A HUMAN never writes it (recurring plans cannot have one — trigger) |
| `cancel_date` | the **MANUAL** terminal date — a human terminating early: the recurring cancel flow AND `cancel_one_time` (the one-time/trial `POST /cancel-one-time` op) both write it; **locks only once the membership is removed from Stripe (`stripe_sync_status = 'deleted'`)** — while the cancel is unconfirmed it stays clearable, which is how a failed cancel reverts (trigger). Never touched by the check-in reversal, so a staff-ended pack can't be resurrected by attendance removal |
...
| `idempotency_key` | nullable UUID; **EVERY** real START row — one-time, trial **and recurring** — carries a deterministic per-row key `uuid5(request.idempotency_key, "<member_id>:<price_id>")`; only `preview_add` rows carry NULL. A partial unique index + `ON CONFLICT (idempotency_key) WHERE idempotency_key IS NOT NULL DO NOTHING` drops duplicate rows on a retried start; a short `RETURNING` result signals a replay and `_crm_insert` raises `MembershipStartReplayError` → **HTTP 409** (the check runs before the insert commits, so a shortfall rolls back; the original rows + their discounts + charge stand unchanged). **Recurring rows are keyed too, deliberately:** `trg_recurring_no_active_memberships` is a `SELECT COUNT` inside a `BEFORE INSERT` trigger — correct but **not race-safe**, so two concurrent re-fires (a kiosk double-tap) both pass it and both insert, giving two rows and two subscription line items. The partial unique index is the only guard that serializes that race. |
| `created_at` | |
...
| `trg_prevent_cancel_date_overwrite` | a written `cancel_date` locks: RECURRING once `stripe_sync_status = 'deleted'` (clearable while unconfirmed, so a failed cancel reverts); ONE_TIME / TRIAL **immediately** — a pack's manual cancel is a pure DB write with no Stripe confirmation step, so it is final the moment it lands |
...
3. `frozen` — the **subject member's** freeze window (`freeze_start_date`/`freeze_end_date`)
   covers today. Freeze lives on `members`, resolved through the membership's own
   `member_id`, so a membership is frozen when its subject member is frozen —
   regardless of who pays. This is why **freeze is per-subject-member, not account-level**.
4. `active` — otherwise.
...
mark_paid_cash / retry_card) lock the affected row's `paid_by_member_id` (read up-front; it's
...
`LockBusyError` → HTTP 409. **That 409 depends on a typed re-raise in the
router, not on the global handler alone:** the lock is taken INSIDE each
handler's `try` and `LockBusyError` subclasses `Exception` directly, so every
handler whose service locks carries `except LockBusyError: raise` ABOVE its
generic `except Exception` → 500 arm (a `raise` inside an `except` escapes the
whole `try`, so the sibling arm can't re-catch it) and documents the 409. All 14
locking handlers have it; `cancel-one-time` (bare, pure DB write),
`reprice-plan` (the lock is taken in the background executor, outside the
request) and `refund` (a FOR UPDATE row lock, not the payer lock) deliberately
don't. `tests/memberships/test_billing_endpoint_guards.py` parametrizes the
propagation over all 14 — without it the front desk gets a 500 for a retryable
conflict (clicking Retry Card while a bulk reprice holds the payer). The
sub-services don't lock; the facade is the single guard point. **Every mutating op is DB-first and pre-synced: it first converges
the payer to a clean DB↔Stripe baseline
(`_pre_sync_payments`, so it never builds on a drifted DB), then writes the desired
state to the DB, calls the param-less sync (`update_payments_recurring` for all ops), then verifies the `stripe_sync_status` writeback
...
cancel, mark_paid_cash, retry_card, add/remove discounts, the single reprice, the upgrade) live ABOVE
...
| --- | --- |
| **start** (`memberships_start.py`, `MemberMembershipsStart.start(request)`) | **ONE list-based op:** a payer + a list of N membership items (each `member_id` + `price_id` + its `discount_ids` + inline `custom_discounts`); a single membership is just a one-item list. The op **never links** — every non-payer item member must have ALREADY authorized the payer (validation rejects otherwise with an "authorize them first" error), which is what lets the whole request run under the payer's single family lock (the facade locks payer + all item members). **ONE payer per request** (`payer_member_id`) — its `paid_by_member_id` is stamped on every inserted row; the payer may be a top-level account OR a **self-paying linked member** (no longer top-level-only). **Phase A** validates everything up-front in the shared `MemberMembershipsStartValidation` (payer is in-gym → links: every non-payer item member must have authorized the payer, i.e. the payer is each item's own member or one of that member's authorized payers → plan/price rows → intra-request duplicate checks (the request validator structurally rejects two items with the same (member, `price_id`) — multiplicity is `quantity`, not repeated items; `_check_no_recurring_duplicates` additionally rejects two recurring items on the same (member, plan) at DIFFERENT prices, which the (member, price) dedup can't see; `_check_recurring_quantity` rejects `quantity > 1` on a recurring item — only one_time/trial may carry `quantity > 1`) → per-member existing-**recurring** check (`_check_no_existing` is recurring-only via `member_memberships_check_existing.sql`'s `plan_type = 'recurring'` filter — an active/frozen recurring blocks a second recurring; one_time/trial stack) → **waiver gate** (`_check_waivers` + `member_memberships_start_waivers_check.sql`: for every (member, plan) the member must have signed each of the plan's `waiver_ids` at a version ≥ the waiver's re-sign **floor** — the highest version with `requires_resign` true, so a minor edit doesn't re-block a prior signer; **ALL plan types**; raises `WaiverGateError` → **HTTP 422** with the structured unsigned `{member_id, waiver_id, name}` list so the CRM routes to signing; runs before any Stripe call and also surfaces in the start **preview**; a no-op for plans with empty `waiver_ids`; deep detail in `waivers-guide`) → discounts live & not `custom`); any failure rejects with nothing written or billed. **Phase B (pure DB):** one multi-row pending insert (NULL `stripe_item_id`, `not_added`) + per-item minted customs + applied discounts — **discounts at creation**, so the first (one-time: the only) invoice is discounted; the start is the one flow allowed to apply a custom (`allow_custom=True`, single-use DB-enforced — `discounts-guide`); any failure undoes everything (nothing billed). **At most two charges:** **Phase C** = ONE consolidated one-time invoice for the `one_time`/`trial` group via `PaymentSyncOneTime.charge_one_time` (one line per membership, item-level coupons; a **trial is a $0 line** on the same invoice; writeback stamps `stripe_item_id` = invoice LINE id + `stripe_one_time_invoice_id` + post-discount `total_price` + `applied`); an optional **card entered at checkout** (`payment` = `MemberMembershipsStartPayment{payment_method_id, set_default}` — card-only, so rejected together with `paid_with_cash`) is handled by `set_default`: when **`set_default`** the card is promoted to the payer's saved default **FIRST** — before any pre-sync, insert, or charge, via `MembersManagementService.update_card` — so it bills BOTH this one-time invoice AND the recurring converge below; when **not** `set_default` it is a **one-off** for THIS invoice only (attach → pay → detach, saved default untouched — `payments-guide`/`sync-guide`). Recurring can only bill the saved default, so a request with any recurring membership MUST set `set_default` (rejected otherwise). The default-save distinguishes a **card decline** from a **system failure**: a genuine `stripe.CardError` while promoting the card to default (a bank decline — it can decline even on attach, the kiosk's most common failure) surfaces as `failed` DATA in the breakdown (all requested items `failed`, `charge_count` 0 — nothing written or charged, since this runs BEFORE `_insert_all`, so there is nothing to clean up — the router's **207** decline contract) so the client shows "try another card"; any NON-card failure (a `stripe.InvalidRequestError`, a system/gateway error) still ABORTS (raises → **500**) with nothing written or charged — never a membership left billing the old card, and never a system failure mistaken for a decline. A *later* charge failure (after the default already changed) is the accepted case — staff re-edit, never reverted; **Phase D** = ONE recurring converge for the recurring group via `update_payments_recurring` (writeback stamps `stripe_item_id` + next_due_date + `applied`). The recurring first charge is **verified synchronously now**: the engine sends the create/add card path `error_if_incomplete` (cash → `pay_first_invoice_out_of_band`, paid out of band), so a **declining card fails the recurring group** — the create 402s leaving no subscription (or an add 402s + Stripe rolls the item change back, leaving the existing sub untouched), the group's pending rows are cleaned, and the failure surfaces as `failed` in the breakdown rather than a `created` row hiding an `incomplete` sub. Only the monthly RENEWALS after the first charge stay async (Stripe dunning → the `invoice.payment_failed` webhook). Idempotency sub-keys are `uuid5(request_key, PlanType.<group>.value)`, so a client retry dedups BOTH charges at Stripe. **Failure granularity is the charge group** (same-group items share fate). A **card decline** (a `stripe.CardError`) at ANY card-touching step — set-default, the one-time invoice, or the recurring first invoice — is **data, not an exception**: it surfaces as a `failed` item in the per-membership breakdown `MemberMembershipsStartResponse` (`results[]` with per-item `status`/`item_id`/`error`, `charge_count`, `multiple_charges`) with that group's un-billed pending rows cleaned up (the router's **207**). A **definitive NON-COLLECTION on the one-time invoice** is folded into that SAME card-failure outcome: `PaymentsStripePaymentService._require_collected` raises `PaymentsNotCollectedError` when `invoices.pay` RETURNED without collecting (the off-session PaymentIntent needs SCA / 3-D Secure — no decline, no malfunction), `_charge_one_time_group` catches it **above** its blanket arm (it subclasses `PaymentsStripeError` → `Exception`, so the generic arm would 500 it), marks the group `failed` with the **`card declined: `** prefix + `CARD_NOT_CHARGED_REASON` ("The card could not be charged.") and cleans its rows → the router's **207**, and returns `False` so a later recurring failure can still honestly claim nothing was collected. Start does NOT distinguish it (founder decision — the desk offers another card either way; retry-card is the one path that still does), and the exception's own SCA wording is deliberately dropped here because "collect another way" is the wrong advice at a kiosk. **This is what stops a kiosk booking an unpaid membership:** the raise lands BEFORE `PaymentSyncOneTime._writeback`, so no row is stamped `stripe_item_id`/`applied`, and `_verify_group(keep_unverified=True)` is never reached — the rows are provably un-billed (`not_added`, NULL `stripe_item_id`), which makes deleting them correct rather than an un-billing. Previously the charge returned `status="open"`, nobody inspected it, the writeback stamped `applied`, `_charge_one_time_group` returned `True`, and the member walked out unpaid on a **201**. A **NON-card** failure at a charge step (a `stripe.InvalidRequestError`, `APIError`, `APIConnectionError`, rate-limit, or any system/gateway error) always cleans the group's un-billed rows — **and when the ONE-TIME leg is what failed, `start`'s own arm additionally deletes the request's RECURRING rows**, which the single `_insert_all` already committed as `not_added` and whose converge is now never going to run, making them provably un-billed (the ONE-TIME group is deliberately NOT swept from outside like that: `charge_one_time` pays the invoice BEFORE its writeback, so those rows may already be billed, and a billed line is never un-billed). Leaving a recurring ghost STRANDS the member — `trg_recurring_no_active_memberships` is a `BEFORE INSERT` trigger counting an uncancelled `not_added` row as active, so EVERY retry (any idempotency key) dies on the insert with a raw DB error → **500**, over a charge that never happened; it fires before the `ON CONFLICT (idempotency_key)` resolution, so the RETURNING-shortfall / `MembershipStartReplayError` 409 path is never even reached, and `member_memberships_status` hides the blocking row from every check the CRM makes, until the reconciler's orphan sweep clears it. The failure is never dressed up as a decline (money ops never 502/503/504, and a gateway error must never masquerade as "your card was declined") — but **how it ANSWERS depends on whether money has already moved in this request**, because the response must never claim less than what actually happened: **(a) nothing collected yet** (the pure-recurring cart, or the one-time leg itself failing) → it **propagates** to a non-retryable **500**, whose "nothing created" is literally true; **(b) the one-time leg already charged** (`_charge_one_time_group` returns whether it COMMITTED, and `start` threads that into `_converge_recurring_group(..., one_time_committed=…)`) → the recurring group is marked `failed` with the distinct **`system failure: `** reason prefix → the router's **207**, because `_verify_group(keep_unverified=True)` KEEPS those billed one-time rows, so a 500 saying "nothing created" would be a flat lie about a collected charge and a live membership. The **two** reason prefixes on `results[].error` are the contract clients branch on — **`card declined: `** (the card did not collect, whether refused or returned un-collected; offer another card) and **`system failure: `** (our side broke; another card can't help, staff must finish it). Neither may be a prefix of the other and the system-failure reason may never contain "declined" (test-locked in `tests/memberships/test_start_207_contract.py`) — and the swallowed exception is logged with its stack inside the arm, since the router never sees it. A **successfully billed one-time row is NEVER un-billed** (kept + flagged for reconciliation if its writeback was unconfirmed). |
...
| **freeze / unfreeze** (`freeze.py`) | **per-subject-member.** `freeze` writes `freeze_start_date`/`freeze_end_date` on the subject member's `members` row (`member_memberships_freeze_profile.sql`), then re-converges every distinct payer that bills any of that member's recurring memberships via `update_payments_recurring` (best-effort per payer; the reconciler self-heals residuals). The facade discovers those payers via `_get_recurring_payers_for_member` (`member_memberships_recurring_payers_for_member.sql`) and locks the subject member + all affected payers before delegating (mirroring `cancel_many`). The sync bills a frozen membership at $0 via a synthetic 100%-off (see `sync-guide` §7); the membership stays on the subscription with its real `stripe_item_id` and `applied` status. `unfreeze` clears the freeze window (`member_memberships_unfreeze_profile.sql`) then re-converges the same payers — the next converge bills the full amount. No `pause_collection`. The gym is resolved **server-side** from the subject member's own row (`member_gym_id.sql`) — the freeze/unfreeze requests carry **no `gym_id`** (a member belongs to exactly one gym), and the freeze/unfreeze-profile rowcount guard stays as defense-in-depth. |
| **mark_paid_cash** (`memberships_settle.py` — `MemberMembershipsSettle`) | recurring-only, and **rejects a canceled membership** (`cancel_date <= gym-today` — staff re-enroll instead of paying a dead membership); resolves the membership row's PAYER (`paid_by_member_id`) and settles **that payer's** monthly subscription's open Stripe invoice **out of band** (no card charge). Runs the shared `settle_open_invoice(item_id, member_id, key, *, pay=pay_open_subscription_invoice_out_of_band)` (see §7.1), which pays the invoice and then **applies that exact invoice IN-REQUEST** — advancing `next_due_date` and writing the CRM invoice/charge rows **before the op returns**, NOT leaving them to the `invoice.paid` webhook/sweep (which never cover an OLD failed-renewal invoice). Cash is a backup — future cycles still auto-charge the card. |
| **retry_card** (`memberships_settle.py` — the CARD twin, same `MemberMembershipsSettle`) | line-for-line the same as `mark_paid_cash` — same recurring-only + linked-to-Stripe + not-canceled validations, same PAYER resolution (`paid_by_member_id`), same shared `settle_open_invoice` body **and same in-request apply** — differing ONLY in the injected `pay` strategy: `pay_open_subscription_invoice_on_card` charges the payer's **saved DEFAULT card**. Its `InvoicePayParams()` omits BOTH `paid_out_of_band` and `payment_method`, which is exactly what makes Stripe charge the default card, and it stamps NO `crm_paid_with_cash` metadata. A **decline raises out of Stripe and propagates through the service** (which applies nothing — the membership simply stays overdue) and **the ROUTER turns it into a RESULT, not a failure**: a `MemberMembershipsRetryCardResponse` with `status=declined` + `decline_reason` (Stripe's own `user_message`) on **207** — the same "a definitive answer about the money is an outcome" contract the START path uses for its per-item `failed` entry, and still never auto-replayed by a proxy. **THREE outcomes, not two:** a collected charge is `status=paid` on 200; a bank refusal is `status=declined` on 207; and a **definitive non-collection** — `PaymentsNotCollectedError`, raised by the shared `_require_collected` (which guards the one-time CHARGE path too — see the start row) when `invoices.pay` returned without raising but the invoice never reached `paid` because the off-session PaymentIntent needs SCA / 3-D Secure — is `status=not_collected` on **207** with its own staff-facing reason ("… needs extra authorization the member has to complete. Collect payment another way."). That third one is deliberately NOT `declined` **here and only here**: this is the staff card-repair path, where "the card needs authentication" is actionable — start and charge-card report the same error as an ordinary card failure, since a kiosk's answer is just "use another card". Its arm MUST sit ABOVE the `PaymentsStripeError` arm (it is a subclass, so the base arm would otherwise 500 it). An UNRECOGNISED failure is unchanged: the base `PaymentsStripeError` / any other exception is still a **500** (never 502/503/504) — a malfunction must never read as a definitive answer about the money. `decline_reason` is the one "why nothing was collected" slot on BOTH non-paid outcomes; `status` is the machine discriminator. This is the manual re-run of a failed renewal charge, the counterpart to letting Stripe dunning retry it. |
| **charge_card** (`memberships_charge_card.py`) | ad-hoc, **outside any subscription**: the request carries an explicit **`paid_by_member_id`** (validated self-or-linked-parent of the beneficiary `member_id`); creates a one-off Stripe invoice for `amount_cents` + `reason` **on the payer's customer** (metadata's `member_id` stays the beneficiary); `paid_cash=true` routes it out of band instead of charging the card. The `invoice.paid` + `invoice_payment.paid` webhooks persist the CRM invoice + charge rows. This is the THIRD route on the 207 contract, answering as `MemberMembershipsChargeCardResponse` — **reusing retry-card's status enum**: a **bank decline** (`stripe.CardError`) is `status=declined` + Stripe's `user_message`, and a **definitive non-collection** (the shared `_require_collected` raising `PaymentsNotCollectedError` on the one-time charge path — see the start row) is the SAME `status=declined`, carrying `CARD_NOT_CHARGED_REASON`. `not_collected` is not reachable here — retry-card is the one path that still distinguishes it. What the guard still buys this route is that a non-collection can never come back 204. **The success contract is untouched: 204, no body** — the 207 is declared in `responses` with its `model`, and since FastAPI forbids a response model on a 204 route the handler returns an explicit `Response` / `JSONResponse`. **Arm order:** `PaymentsNotCollectedError` subclasses `PaymentsStripeError`, so it must stay ABOVE that base arm; `stripe.CardError` subclasses neither it nor `ValueError`, so it only has to sit above the blanket `except Exception` — which is precisely where an expired card used to land, 500-ing an ordinary decline with the bank's reason discarded. |
...
| **authorize payer** (`linked.py`) / **remove-authorization** (cascade, `memberships_service.py`) | **Many-to-many** (`member_authorized_payers`): a member may have MANY authorized payers AND be an authorized payer for others. `link_account(member_id, payer_member_id, *, waiver_version_id, signer_name, consent_acknowledged, ip_address, user_agent, operator_employee_id)` is **sign-gated in ONE request**: it calls the **shared** `WaiversService.sign_waiver` (the one signing path) to sign the gym's **default authorized-payer waiver** as the payer — rendering the payer's name (`{{member_name}}`) and the member-being-paid-for (`{{payee_name}}`) into the text, version-locked on the echoed `waiver_version_id` (**409** on a stale echo) — then inserts the `member_authorized_payers(member_id, payer_member_id, gym_id, signature_id)` row referencing the new signature. Signing commits its **own** txn and the authorization insert is **separate (NOT atomic)** — a retry after a failed insert leaves a harmless extra append-only signature (by design; signing + authorizing are one request but not one transaction). The payee's name comes from the link-check read (`member_authorized_payers_link_check.sql` now returns `candidate_first_name/last_name`). Full waiver/signing detail: `waivers-guide`. Authorizing is otherwise a **pure DB change** — no Stripe sync, no preview, no no-active-recurring guard (it never contributes a membership line). `check_link_account(member_id, payer_member_id)` is a read-only pre-flight (not-self / payer-in-gym / not-already-authorized). **De-authorization is the cascade `remove_authorization(member_id, payer_member_id, idempotency_key)` on the facade (`POST /link/remove`) — the ONLY unlink path:** it cancels the payer's funded recurring memberships for this member FIRST (the `cancel` op, full sync+verify), THEN deletes the `member_authorized_payers` row (`member_authorized_payers_delete.sql`); the signature persists as append-only audit. **There is no bare de-authorize** — deleting the authorization without cancelling would strand billing (the membership's `paid_by_member_id` keeps pointing at the de-authorized payer, so Stripe keeps charging), which is exactly why removal cascades. The authorization (who may pay for whom) is `member_authorized_payers`, never the billing key. `link_account` injects `WaiversService` and owns its **own** locking: locks the **two accounts** `[member_id, payer_member_id]` (delegated **bare**, since `PayingMemberLock` is non-reentrant); `remove_authorization` likewise locks the two accounts. `MemberMembershipsLinked` is self-contained — does **not** extend `MemberMembershipsBase`. |
...
After any **invoice-creating** membership op whose new invoice id it does NOT know up
front — `charge_card`, `start` (charge groups), `upgrade`, prorating reprice —
`MemberMembershipsService` fires a **fire-and-forget invoice fetch** via
`MembershipsInvoiceFetchRunner.start_for_payer`.
...
request**. They share one body — **`MemberMembershipsSettle.settle_open_invoice`**
(`service/memberships_settle.py`) — so a change (a new validation, the apply seam) can never
land on one and drift on the other. The facade holds the payer lock and calls it with the
strategy:

- `mark_paid_cash` → `pay=payment_service.pay_open_subscription_invoice_out_of_band`
- `retry_card` → `pay=payment_service.pay_open_subscription_invoice_on_card`

`settle_open_invoice(item_id, member_id, idempotency_key, *, pay)` does:

1. Load the `member_memberships_get.sql` row; validate **recurring** + **linked to a Stripe
   subscription** (`stripe_item_id`) + **not canceled** (`cancel_date > gym-today`). Any
   failure raises `ValueError` **before** any Stripe call.
...
3. `invoice = await pay(sub, account, idempotency_key=str(key))` — returns the now-PAID invoice
   ...
   **"This invoice is already settled — nothing left to collect."** when there are none — so a
   ...
   decline raises out of Stripe and propagates through this layer untouched — the **router** is
   where it becomes a 207 `declined` RESULT (see `retry_card` in §6); a card retry that returns
   WITHOUT collecting (SCA needs authentication, no decline raised) is turned into the distinct
   `PaymentsNotCollectedError` — never booked as a phantom success, and deliberately NOT dressed
   up as a decline, but just as DEFINITIVE as one, so the router answers it with its own 207
   `not_collected` result rather than a 500. Only an unrecognised `PaymentsStripeError` is a 500.
...
`tests/memberships/test_memberships_settle.py` (which also pins the router contract: decline →
207 `declined` with the reason, SCA non-collection → 207 `not_collected` with its own reason,
success → 200 `paid`, unrecognised Stripe failure → 500); the live DB
assertions (next_due advanced, invoice `paid`, no webhook needed) live in
`tests/memberships/test_memberships_retry_card_live.py`.
...
- **Freeze (per-subject-member) vs. cancel (membership-level) vs. end (non-recurring).**
  Three distinct stops: **freeze** bills the frozen member's own memberships at $0
  via a synthetic 100%-off discount through the regular sync (the freeze window lives
  on `members`, keyed on the subject's own `member_id`; freezing one member does not
  affect other members even if they share a payer); **cancel** sets `cancel_date` on
  one recurring membership (stays active through the paid period); **end** is the
...
| `POST /` | start the payer's family memberships in one call — list of items in, per-membership breakdown out (`MemberMembershipsStartResponse`); ≤2 charges (201). Any `failed` item → **207**, which declares the SAME response `model` as the 201 (it is the primary kiosk decline surface, so a generated client must see the `results[]` body); the item's reason prefix says which kind of failure (`card declined: ` / `system failure: `) |
...
| `POST /cancel-one-time` | cancel a ONE-TIME / TRIAL membership early — sets `cancel_date = today` → status `cancelled`; pure DB write, no Stripe, returns the `cancel_date` (recurring rejected — use `DELETE /`) |
| `POST /mark-paid-cash` | pay the open invoice out of band (204) |
| `POST /retry-card` | retry the payer's saved default card on that same open invoice → `MemberMembershipsRetryCardResponse` (`status=paid` on 200; a DEFINITIVE non-collection is a result on **207**, never a 500 — `status=declined` + `decline_reason` when the bank refused, `status=not_collected` + its own reason when the payment needs SCA authentication) |
| `POST /charge-card` | ad-hoc card/cash charge — a collected charge is **204 with no body** (unchanged); a card that did not collect is a result on **207** (`MemberMembershipsChargeCardResponse`), never a 500 — `status=declined` either way, with Stripe's `user_message` when the bank refused and `CARD_NOT_CHARGED_REASON` when the pay returned without collecting |

Plan endpoints (`plans_router.py`) call `verify_gym_employee`. **Every
membership endpoint (`memberships_router.py`) is staff-only** — all per-member
routes gate on `verify_gym_employee_for_member` (staff-only; the member
themselves is excluded), and the one plan-wide op `batch-reprice`
(`POST /plans/{plan_id}/reprice`) gates on `verify_gym_employee` (gym-level).
No membership route uses `verify_can_view_member` (self-or-staff); that guard is
for read-only member views elsewhere, never for a billing mutation or its preview.

The full per-member staff-only set: `cancel`, `freeze`/`unfreeze`,
`start` / `start-preview` (guard runs on the payer **and** every item member),
`update-price`, `upgrade` / `upgrade-preview`, `cancel-one-time`, `cancel-preview`,
`add`/`remove` discounts, `mark-paid-cash`, `retry-card`, `charge-card`, `refund`. Membership
billing is staff-managed end to end, so a member must not self-enroll,
self-cancel, self-reprice/upgrade, self-settle, self-freeze, or self-grant a
discount. The read-only **previews are staff-only too** (no member-portal path):
they expose proration and refund math that is a staff tool, so they match their
mutation's guard rather than `verify_can_view_member`.
...
- **Membership service:**
  ...
  `memberships_mark_paid_cash`, `memberships_retry_card`, `memberships_charge_card`,
  ...
  **`memberships_invoice_fetch`** (`MemberMembershipsInvoiceFetch` — the fetch+apply
  ...
  profile pair (`member_memberships_freeze_profile.sql`, `member_memberships_unfreeze_profile.sql`),
  `member_memberships_recurring_payers_for_member.sql` (payer discovery for the freeze op),
  `..._delete_pending`). The one-time charge engine the start calls

...
| `POST /charge-card` | ad-hoc card/cash charge — a collected charge is **204 with no body** (unchanged); a DEFINITIVE answer about the money is a result on **207** (`MemberMembershipsChargeCardResponse`), never a 500 — `status=declined` + Stripe's `user_message` when the bank refused, `status=not_collected` + its own reason when the payment needs SCA authentication |

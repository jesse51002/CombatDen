...
description: >-
  ...
  applied snapshots that pin a membership to a value version), item-scoped
  (per-membership) discounts, linked/family discounts as real version-backed
  discount entries a plan references by id, and coupons computed at sync then
  written back to Stripe. Load this whenever you touch
  anything discount-shaped: gym_discounts / gym_discount_values, the
  member_membership_applied_discounts snapshots, the apply/remove path
  (MemberMembershipsUpdateDiscounts), the sync-time coupon computation
  (PaymentSyncCoupons / payment_sync_builder), the once/ongoing lifetime +
  end_date, the percent×quantity fix, or the CRM discount UI. Trigger on
  "discount", "coupon", "discount version", "apply a discount", "snapshot",
...
## 1. Three tables: identity, versioned values, applied snapshots
...
2. **`gym_discount_values`** — **versioned, truly immutable VALUE rows**, exactly
   ...
   **never updated or touched** — the table is **service_role-write-only**
   ...
   snapshots.
3. **`member_membership_applied_discounts`** — the **slim applied snapshot**:
   one row = one membership (`item_id`) frozen to one immutable value version
   (`value_id`). The Stripe-gated half (holds the written-back
   `stripe_coupon_id`).

**Apply = INSERT** a snapshot referencing the discount's **active** `value_id`.
**Remove = DELETE** that row. **Never edit.** A preset already applied is left
**frozen** (never re-resolved). To change a member's discount you remove the row
and add another.

**Why this is the predictability guarantee:** editing a discount mints a NEW
version; existing snapshots still point at their OLD `value_id`, so they are
untouched — an edit affects only *future* applications. "What discount does this
member have, and which exact version" is a local, history-accurate fact you can
prove from `value_id`. (The thing we never want: an edit fanning out to silently
re-bill every holder.)

### Snapshot columns (`member_membership_applied_discounts`)
...
The percent/dollar + mode + lifetime are **not** copied onto the snapshot — they
live on the referenced `gym_discount_values` row, reached by
...
Every applied-snapshot column is user-immutable via the column guard
(`MEMBER_MEMBERSHIP_APPLIED_DISCOUNTS` in
`Database/python_data/schema/immutable_columns.py`). The **system**, at
service-role, legitimately writes back two **outcome** fields during sync:
`stripe_coupon_id` (the current resolved coupon) and `end_date` (stamped when a
`once` discount is consumed). That is why the applied table is the **Stripe-gated
half** (unfiltered base + `security_invoker` filtered view exposing only
`stripe_coupon_id IS NOT NULL` rows + `hide_incomplete_stripe_records` + INSERT/
UPDATE revoked for `authenticated`). The value rows, by contrast, are immutable
*by version* (REVOKE the value columns; only `is_active` flips).
...
flow passes its id to `add_preset_ids`, which freezes a snapshot to the discount's
**active** value version. Editing the linked discount mints a new version, so
future family applications get it — like a regular discount. The `linked` tag
keeps it out of the regular per-membership discount picker (which lists only
`preset`); family billing via `members.account_linked_to_id` is unchanged.
...
## 4. Coupons are computed at sync, then written back

*This section is the discount-side summary. The full sync engine — the
`update_payments_recurring` orchestration sequence, the read-before-write gate,
the deterministic coupon-id scheme, line consolidation, and the snapshot
writebacks — is documented in `sync-guide`; the Stripe coupon/subscription
primitives it calls are in `payments-guide`.*

Presets and snapshots store **intent**; **no Stripe coupon is pre-baked**. The
sync (`update_payments_recurring` → `_attach_computed_coupons`, the per-line
logic in `payment_sync_builder.py` + the coupon helper `PaymentSyncCoupons`)
reads each applied snapshot **joined to its value version** for the
percent/dollar + mode (`get_applied_discounts_by_member.sql` joins
`gym_discount_values`), then for each consolidated line (price `P`, quantity
`N`):

1. **Read the subscription's current Stripe discounts first** — via
   `PaymentsStripeSubscriptionService.get_subscription` (a genuinely new read
   path; the old `execute_sync` only ever *pushed* desired state). The union of
   `sub.discounts` + every `item.discounts` is the live coupon set.
2. **Exclude any snapshot past its `end_date`** — this is how we enforce
   arbitrary end dates.
3. **Apply the `once` consumption gate** (§5).
4. **Aggregate per line, grouped by `discount_mode`** so `once` and `ongoing`
   never mix: `line_percent = (Σ per-unit percents) ÷ N`; `line_amount =
   Σ per-unit dollar_offs`.
5. **Find-or-create the coupon** on the gym's Connect account using a
   **deterministic per-account coupon ID** from the value signature
   ...
   `round(percentage_off * 100)`) or `amt_<cents>_<mode>`. Creation passes that
   id so a repeat/race collides and is treated as "already exists" — idempotent,
   one coupon per distinct value reused, **no coupon registry table.** `once` →
   a Stripe `once` coupon; `ongoing` → a Stripe `forever` coupon (the `end_date`
   cutoff is enforced by *us*, never by Stripe).
6. **Write the resolved `stripe_coupon_id` back onto each contributing
   snapshot** (`set_snapshot_coupon_id.sql`, service-role) and attach the coupon
   to the line. The writeback is **per-value** (a value's coupon is written only
   onto the same-mode snapshots that contributed it).

### The percent × quantity fix (why per-line aggregation exists)

Stripe consolidates same-price memberships onto **one line with a quantity.** A
naive 10%-off coupon on a quantity-2 line discounts **both** units. The fix: a
10% discount meant for **1 of 2** units becomes **5% on the quantity-2 line**
(`percent ÷ quantity`); fixed dollars are summed. Computed from the member's
**own** memberships only — deterministic, no cross-member reshuffle.

### Where the coupon write happens — sync only, never preview

`apply_discounts` writes snapshot rows then re-syncs, so the sync resolves the
coupon. The **preview path does NOT attach coupons** (a true dry-run cannot
safely find-or-create Stripe coupons or write back DB rows), so
`/discounts/preview` reflects the pre-discount-resolution bill. Intentional, not
a bug.
...
apply-time the snapshot's **absolute `end_date` is resolved** (`_resolve_end_date`:
`apply_date + duration` via `relativedelta`, or the explicit `end_date` copied).
`once` discounts leave `end_date` NULL until the sync stamps it on consumption.
...
A `once` discount's whole lifecycle lives in the **sync**. A just-applied `once`
snapshot has `stripe_coupon_id = NULL` and `end_date = NULL`. On each sync,
reading the subscription's **current** Stripe discounts:

- **NULL coupon, or coupon present on the sub** → still **pending** → participates
  in aggregation; the first sync find-or-creates its coupon, attaches it, and
  **writes `stripe_coupon_id` back** (the presence handle). **If the consolidated
  count changed its computed value, the sync swaps the coupon and writes back the
  new id.**
- **coupon absent from the sub** (after Stripe invoiced it) → **consumed → done**
  → the sync **stamps `end_date = today`** (`stamp_snapshot_end_date.sql`,
  idempotent) and never re-adds it; the `end_date` exclusion handles it from then
  on, so we stop querying Stripe.
...
- **Automatic propagation of a discount edit to existing holders.** Today an edit
  ...
  guarantee); existing snapshots stay pinned to their old `value_id`. The version
  linkage exists so a future "re-apply this discount's active version to its
  holders" bulk action *can* find them. Default until then: immutable after apply.
...
- **The scheduled reconciler is a functional dependency, not just a drift
  ...
  finalization on an **idle** member depend on the daily reconciler running
  `execute_sync` on its own. Building it is out of scope (see `PaymentRefactor.md`
  §4).
...
- **Schema:** `Database/supabase/schemas/gym_discounts.sql` (identity),
  ...
  (slim snapshots). Access rules in the parallel `access_rules/` files. Linked
  pricing lives on `membership_plans.sql`.
- **Models/enums:** `Database/python_data/schema/gym_discount.py`
  ...
  `member_membership_applied_discount.py`, `immutable_columns.py`
  (`GYM_DISCOUNT_VALUES`, `MEMBER_MEMBERSHIP_APPLIED_DISCOUNTS`).
...
- **Sync-time coupons:** `payment_sync/payment_sync_coupons.py`
  (`PaymentSyncCoupons`), `payment_sync_builder.py` (`plan_line_discounts`),
  `membership_payment_sync_service.py` (`_attach_computed_coupons`),
...
- **CRM member billing-detail read:**
  ...
  `applied_discounts` JSONB carrying the **full snapshot shape**
...
- **Sibling skills:** `memberships-guide` (the plans/prices + `member_memberships`
  that host these snapshots), `sync-guide` (the engine that computes the coupons),
  `payments-guide` (the Stripe coupon/subscription/webhook primitives).

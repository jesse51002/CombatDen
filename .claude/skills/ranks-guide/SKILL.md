...
`gyms.sub_rank_type` is an enum **`stripes | div`** (`NOT NULL DEFAULT 'stripes'`)
— a single choice for the whole gym, **not** per rank. Each main rank only chooses
its `sub_rank_count`. The type is **writable via the ordinary gym-update path**
(`PUT /api/v1/gyms/{id}` → `GymUpdateData.sub_rank_type`, a mutable column that
rejects an explicit null); `from_preset` also sets it from the preset's implied
type.
...
- `rank_display_name(name, sub_rank_type, sub_index) -> str` → `"Blue Belt"` at a
  leaf/base position, else `"Blue Belt · 2 Stripes"`.
...
`sub_rank_count = 5` (base + 4 stripes); a div with N divisions is `N`. This makes
`current_sub_index ∈ [0, count-1]` and `count == 0 ⇔ sub_index IS NULL` a single
uniform, type-invariant invariant — so switching `stripes ↔ div` is a pure
re-label on read, never a member rewrite.
...
ranks service on **every** assign/promote/backfill path (and mirrored in the seed):

- `sub_rank_count > 0` ⇒ `current_sub_index` is **required** and in `[0, count-1]`.
  You can never sit "in the main rank" generically when it has sub-ranks.
- `sub_rank_count == 0` ⇒ `current_sub_index` is **NULL** (the main rank is itself
  the leaf).
...
not stored: `ceil(classes_to_next_major / sub_rank_count)` when `count > 0`, else
`classes_to_next_major`. That derived step is a member's immediate progress
denominator.
...
`ON CONFLICT (gym_id, main_rank_num_order) DO NOTHING`) and, when the preset
implies a sub-rank type, sets the gym's `sub_rank_type`.

- **`bjj_belts`** — White→Black, `sub_rank_count = 0` (flat belts), no implied type.
...
- **`RanksBase`** — shared reads: `_list_ranks_in_session`, `_gym_sub_rank_type`,
  and `_next_leaf` (enumerate leaves per main; rank-less → lowest leaf; top main +
  top sub → `ValueError("highest rank")`).
- **`RanksMembers`** — the only member-writing paths: `promote_member`,
  `set_member_rank` (validates the leaf invariant), `_apply_member_rank` (writes +
  logs `rank_changed` with derived names), `backfill_lowest_for_gym`.
...
- **`RanksReads`** — the two paginated member reads (`COUNT(*) OVER()` +
  `start_index`/`count`): `list_ready_to_promote` (active, ranked, not top-of-ladder
  members sorted by `classes_since / step_denominator` DESC — the promotion board)
  and `list_members_in_rank` (the roster for one main rank).
...
- **Next sub-rank** — advance `current_sub_index` by one within the main rank.
  ...
  base leaf); the CRM only offers "next sub" while `current_sub_index < count-1`.
...
- **assign to a `count>0` rank with no sub_index** → 400 (invariant).
- **assign to a `count==0` rank with a sub_index** → sub_index forced to NULL.
- **delete a main rank** → neighbor-reassign members to the replacement's base leaf
  (lower → higher → NULL), silent (no activity), FK never dangles.
- **change `gyms.sub_rank_type`** with sub-indexed members → pure re-label on read,
  no member rewrite.

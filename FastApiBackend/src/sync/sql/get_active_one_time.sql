-- Read the family's PENDING non-recurring memberships for the one-time sync.
-- Covers BOTH 'one_time' and 'trial' plans: a trial is billed identically as
-- a $0 line on the same consolidated invoice, so it gets the same two-id
-- writeback and 'applied' confirmation as a paid one-time. Reads the
-- UNFILTERED base table (service-role) so the just-inserted PENDING rows
-- (stripe_sync_status 'not_added') are visible — the sync bills them on a
-- single consolidated invoice and stamps each.
--

...
class GymsOnboardingService:
    """Read + refresh a gym's Stripe onboarding status.

    Both entry points are keyed on ``gym_id``; the caller's ownership
    of the gym is verified at the router layer (``verify_gym_owner``)
    before these run.

        * ``refresh(gym_id)``: reads the gym's Stripe state,
        ...
        * ``new_link(gym_id)``: cheap "give me a new hosted URL"
          path for resume flow, without re-reading the Stripe account.
    ...
    async def refresh(self, gym_id: UUID) -> GymOnboardingStatusResponse:
        """Refresh the status for the given gym.

        Args:
            gym_id: The gym whose Stripe onboarding to refresh. The
                caller's ownership is verified at the router layer.
        ...
        Raises:
            ValueError: If the gym does not exist, or the row has no
                Stripe account id (not in onboarding).
        ...
        """
        gym = await self._lookup_gym_by_id(gym_id)
        if gym is None:
            raise ValueError("No gym found for this id")
    ...
    async def new_link(self, gym_id: UUID) -> GymOnboardingLinkResponse:
        """Mint a fresh AccountLink without re-reading the account.

        Only allowed while status is ``pending`` — the client is
        expected to route ``complete`` gyms elsewhere. The caller's
        ownership of ``gym_id`` is verified at the router layer.

        Raises:
            ValueError: If the gym does not exist, has no Stripe
                account, or is not in a pending state.
        """
        gym = await self._lookup_gym_by_id(gym_id)
        if gym is None:
            raise ValueError("No gym found for this id")
    ...
    async def _lookup_gym_by_id(self, gym_id: UUID) -> dict | None:
        """Read a gym's Stripe state from the ``gyms`` table by id."""
        sql = load_sql(SQL_DIR / "gyms_get_stripe_state.sql")
    ...
    async def _set_status(self, gym_id: UUID, status_value: str) -> None:
        """UPDATE gyms.stripe_onboarding_status (with retry)."""
    ...
    async def _clear_stripe_account_id(self, gym_id: UUID) -> None:
        """Wipe the Stripe account linkage on a read-side 404."""

-- Read the family's PENDING one-time memberships for the one-time sync. Reads the
-- UNFILTERED base table (service-role) so the just-inserted PENDING rows
-- (stripe_sync_status 'not_added') are visible — the sync bills them on a single
-- consolidated invoice and stamps each.
--

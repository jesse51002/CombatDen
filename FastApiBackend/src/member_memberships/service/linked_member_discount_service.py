...
class LinkedMemberDiscountService:
    ...
    async def calculate_linked_discount_ids(
        ...
        Args:
            parent_id: The paying parent's crm_user_id.
        ...
        for row in active_memberships:
            member_plans[UUID(str(row["crm_user_id"]))].add(
                UUID(str(row["plan_id"])),
            )

        if exclude_memberships:
            for m in exclude_memberships:
                member_plans[m.crm_user_id].discard(m.plan_id)

        if include_memberships:
            for m in include_memberships:
                member_plans[m.crm_user_id].add(m.plan_id)
        ...
        for row in members_with_discounts:
            child_id = UUID(str(row["crm_user_id"]))
            if child_id != parent_id:
                children_with_discount.add(child_id)

        if include_memberships:
            for m in include_memberships:
                if m.has_linked_discount and m.crm_user_id != parent_id:
                    children_with_discount.add(m.crm_user_id)
    ...
    async def persist_assignments(
        ...
        async with self._db_pool.session() as session:
            for crm_user_id, discount_id in zip(
                member_ids,
                discount_ids,
                strict=True,
            ):

...
-- same statement (:activity_type is bound to
-- RANK_CHANGED_ACTIVITY_TYPE), so each member's progress anchor
-- starts at the backfill moment rather than their join date. A gym
-- with no ranks yields an empty `lowest` CTE, making the whole
-- statement a no-op.
WITH lowest AS (
    SELECT rank_id, main_name, sub_name
    FROM gym_ranks
    WHERE gym_id = :gym_id
...
SELECT
    ...
    jsonb_build_object(
        ...
        'old_rank_name', NULL,
        'new_rank_name',
            NULLIF(TRIM(COALESCE(b.main_name, '') || ' '
                       || COALESCE(b.sub_name, '')), '')
    )
FROM backfilled b

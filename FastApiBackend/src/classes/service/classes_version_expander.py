...
1. **Ownership windowing.** A version owns the occurrences whose ORIGINAL
   instant (``original_date`` + the slot's own time in the version's frozen
...
2. **Cross-version slot dedup (no slot doubling).** When two versions both
   produce an owned candidate at the same ``(original_date, original_time)``
   slot, the EARLIER version wins — a class has at most ONE original
   occurrence per exact slot. Dedup is deliberately SLOT-level, not
   day-level: a boundary day may legitimately show occurrences from BOTH
   versions at different times (the old version's 06:00 already ran before a
   mid-day mint; the new version's 18:30 is upcoming) — with several slots
   per day legal, that is the honest rendering, not doubling.
...
class ClassesVersionExpander:
    ...
    def expand(
        ...
        Args:
            versions: ALL of the class's schedule versions (any order; sorted
                ...
                frozen timezone.
            instance_exceptions: Single-slot overrides for this class, each
                bound to one ``(original_date, original_time)``.
        ...
        claimed_slots: set[tuple[date, time]] = set()
    ...
    def original_start_at(
        ...
        """The UTC instant of the ``(original_date, slot_time)`` slot.

        The ownership instant: the original slot interpreted in the version's
        own frozen timezone. Public because the mint engine
        (``ClassesVersionsService``) computes the same instant when collecting
        future-keyed rows for the version-change wipe — it must never drift
        from the expander's own ownership arithmetic.
        """
    ...
    def _owned_occurrences(
        ...
        claimed_slots: set[tuple[date, time]],
        instance_exceptions: list[ExpanderInstanceException],
        range_exceptions: list[ExpanderRangeException],
        window_start: date,
        window_end: date,
        ...
        """One version's owned occurrences, claiming slots as it goes.
        ...
        ``claimed_slots`` is shared across versions in ``effective_from``
        order, so an earlier version's claim suppresses a later version's
        candidate at the same exact ``(original_date, original_time)`` (rule
        2 — slot-level, so a boundary day CAN carry both versions'
        different-time occurrences).
        """
        ...
        for occ in expanded:
            slot_instant = self.original_start_at(
                version, occ.original_date, occ.original_time
            )

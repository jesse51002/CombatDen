...
-- schedule version (gym_class_schedules_current), the seven per-weekday
-- instructor slots resolved to "first_name last_name" via a LEFT JOIN on
-- gym_employees per slot (NULL slot -> NULL name). The response shape is the
-- flat class-as-it-stands-now the CRM form edits; window/board reads load the

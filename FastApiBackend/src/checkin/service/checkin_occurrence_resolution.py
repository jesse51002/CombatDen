...
``resolve_original`` turns ``(class_id, occurrence_date)`` into the
``EffectiveOccurrence`` whose ORIGINAL date is ``occurrence_date`` — the
identity every checkin API call addresses — by loading the class's schedule
versions + exceptions and expanding through the pure
``ClassesVersionExpander`` (the one-way ``checkin -> classes`` dependency).

Why a naive single-day expand isn't enough: an occurrence is addressed by
its ORIGINAL date, never its effective (post-reschedule) date — see the
...
expanding once and filtering by ``original_date`` — the same problem
...
class CheckinOccurrenceResolution:
    ...
    async def resolve_original(
        ...
        when the class has never been scheduled, the date isn't a recurrence
        date, or (with the default ``include_cancelled=False``) it's
        cancelled that day. ``include_cancelled=True`` emits a cancelled
        occurrence flagged instead — the sign-up path uses it to distinguish
        "cancelled that day" from "never a recurrence date" in its errors.
        """

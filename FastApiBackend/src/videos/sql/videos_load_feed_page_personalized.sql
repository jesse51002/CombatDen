-- Paginated real-gym video feed page, PERSONALIZED to one member's video-taste
-- embedding. Identical candidate set / filters / pagination to
-- videos_load_feed_page.sql, but LEFT JOINs video_rag and orders enriched videos
-- by cosine distance to the member's profile embedding first; un-enriched videos
-- (no video_rag row → NULL distance) fall to the end by relevance. Used by the
-- feed route only when a member_id is supplied AND that member has a built
-- profile embedding; otherwise the plain videos_load_feed_page.sql runs.
--
-- Parameters:
...
--   :member_embedding   the member's profile embedding (pgvector text form)
--   :limit              page size
...
ORDER BY
    (r.embedding <=> CAST(:member_embedding AS vector)) ASC NULLS LAST,
    v.relevance_index,
    v.video_id

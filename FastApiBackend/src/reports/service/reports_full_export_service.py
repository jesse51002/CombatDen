...
(``stripe_webhook_events``, ``resource_locks``, ``tasks``/``task_items``, the
``gym_video_*`` pool, and ``gym_growth_metrics`` — the Growth page's
hourly-recomputed metric cache, fully derivable from the source tables that ARE
exported).
"""

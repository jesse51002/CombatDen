...
``gyms.stripe_onboarding_status`` value using the shared mapper
(so this path and the explicit ``GET /api/v1/gyms/me/onboarding``
refresh path never drift) and writes it to ``gyms_unfiltered``.
...
from src.gyms import SQL_DIR as GYMS_SQL_DIR
...
class AccountUpdatedHandler:
    """Sync a connected account's onboarding status into the CRM."""
    ...
    async def handle(
        ...
        sql = load_sql(GYMS_SQL_DIR / "gyms_set_onboarding_status.sql")

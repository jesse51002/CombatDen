...
| `checkin` | Class **attendance** — the consumer side of `classes`. **Gated lazy check-in** (`POST /api/v1/checkin`: plan eligibility → trial→one_time→recurring, punch-card + room capacity, auto-end depleted packs; idempotent, materializes the occurrence on first attendance) + **staff batch check-in** (`POST /api/v1/checkin/batch` — one occurrence, many members → 207; `class_id` + `occurrence_date` in the body) + attendance **streak** (`GET /api/v1/streak`) + per-cycle class usage (feeds member billing detail). `CheckinService` is a facade over `CheckinOccurrenceResolver` (the one-way `checkin → classes` dependency: reuses `ClassesExpander` + `ClassesMaterializer`) + `CheckinMemberGate` (eligibility/capacity/select/write) |

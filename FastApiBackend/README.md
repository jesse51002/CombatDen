...
flowchart TB
  ...
  FB["⚙️ FastApiBackend — CRM / billing + video API<br/>14 domains · ~98 routes<br/>members · gyms · classes · ranks · rewards · waivers<br/>discounts · memberships · plans · stripe_webhooks · tasks<br/>videos (merged VideoService API + owner feed edits w/ YouTube metadata) · presets (import a gym template)<br/>video_config (LLM-powered agent: versioned spec authoring + query gen + feed learning)<br/>+ payments · sync · reconciler (router-less; reconciler = twice-daily billing sweep)"]
  ...
  LLMProvider["🤖 Anthropic / LLM (Pydantic AI)<br/>video_config agent + query generation<br/>default: claude-sonnet-4-6 · swappable via video_agent_model"]
  ...
  FB -->|"video-config: conversational agent + query generation"| LLMProvider
...
| `videos` | The merged **VideoService API**: a **public** slug-keyed template catalog (`/api/v1/videos/templates*` — the 76 `video_gym*` demo templates: cards, detail, feed, preview) plus a real gym's authed live content keyed by UUID. `GET /api/v1/gyms/{id}/videos` supports `?owner` (bool — the owner "Your videos" section, `gym_video_feed` rows with `video_run_id` NULL, run-independent) and `?rejected` (bool — the scan's rejected list); default = the gym's latest scan run, accepted videos. `/videos/preview`, `/videos/spec`, `/showcase` (the latter assembles `gym_video_spec` + `gym_classes ⋈ gym_employees` + `gym_rewards`). The `video` pool row carries `added_via` (`web_query` \| `manual`) — marks deletability: `manual` = gym-owned custom (hard-deletable); `web_query` = shared, reject-only. `gym_video_feed` uses `scan_status` (accepted\|rejected) + `rejection_type` (automatic\|manual) + `reject_reason` + `rejected_at`; the reject audit is retained when a video is re-accepted. The owner hand-edits the feed: `POST …/videos/lookup` fetches a link's real metadata for a confirmation (no write); `POST …/videos` adds it (id extracted, **real metadata fetched from the YouTube Data API**, feed row inserted into the owner section, and the new `video` pool row is **owned by that gym** — `video.gym_id` set, `added_via='manual'` — a private custom video); `DELETE …/videos/{video_id}?owner=true` removes from the owner section (+ hard-deletes the pool row if `added_via='manual'`); `DELETE …/videos/{video_id}` rejects the latest-run row (`scan_status='rejected'`, optional reason; pool row untouched); `POST …/videos/{video_id}/keep` un-rejects (audit kept). Otherwise read-only; the shared `video` pool's bulk rows + templates are written by the separate VideoService batch job |
| `presets` | Import a gym-type template into a gym's **real** production tables in one transaction (`POST /api/v1/gyms/{id}/presets/import`): opens a new `video_run` and copies the template's good videos into it as the gym's latest served feed; seeds ~3 into the owner "Your videos" section on first import (owner-section-empty check, so re-imports don't pile up); writes a `system_update` version of `gym_video_spec` + real `gym_classes` (synthesized schedule defaults) + instructors as `gym_employees` + `gym_rewards` + `gyms.theme_design_id`. FK-safe overwrite (soft-delete classes, deactivate rewards, upsert trainers). Owner + email-allowlist gated (`preset_import_allowed_emails`, demo: owner1) |
| `video_config` | LLM-powered conversational agent for authoring and evolving a gym's video config (the `gym_video_spec` — its `videos_desc`, `avoid_desc`, and `queries`). Five gym-employee-gated routes: `GET /` (latest spec via `gym_video_spec_latest` view), `PUT /` (save confirmed draft → `admin_update` spec version), `POST /agent` (one conversational turn via Pydantic AI / Anthropic), `POST /generate-queries` (single structured LLM call), `POST /refine-from-feed` (fold owner curation signals from `gym_video_feed` → `feed_update` version). `gym_video_spec` is **append-only versioned** (three writer paths: agent, preset import, feed refiner). Prompts live in `src/video_config/prompts/*.md`. Model defaults to `anthropic:claude-sonnet-4-6`, swappable via `settings.video_agent_model` |

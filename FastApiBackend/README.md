...
flowchart TB
  ...
  FB["⚙️ FastApiBackend — CRM / billing + video API<br/>15 domains · 109 routes<br/>members · gyms · ranks · rewards · waivers · discounts<br/>memberships · plans · stripe_webhooks · tasks<br/>classes (CRUD + instance/range exceptions + schedule board + undo/reschedule + expander/materializer)<br/>checkin (is_member-gated/lazy single + batch check-in + per-occurrence attendees + attendance streak + per-cycle usage — reuses classes' expander/materializer)<br/>videos (real-gym feed + LLM spec/agent authoring:<br/>litellm for query gen / feed refiner · Pydantic AI for the conversational agent)<br/>presets (import a gym template + public template catalog) · theme (gym showcase)<br/>+ payments · sync · reconciler (router-less; reconciler = twice-daily billing sweep)"]

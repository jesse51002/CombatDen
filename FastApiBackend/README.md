...
| `checkin` | Class **attendance** — the consumer side of `classes`. **`is_member`-gated lazy check-in** (`POST /api/v1/checkin`: materializes the occurrence on first attendance, awards points, auto-ends depleted packs; idempotent). `is_member=true` (kiosk) runs the strict gate — best eligible plan with capacity (trial→one_time→recurring), else rejected with a `skip_reason`. `is_member=false` (staff, the default) ALWAYS records — attributes to the best available membership (NULL plan/item when none) and returns gate conditions as `warnings`. Plus **batch check-in** (`POST /api/v1/checkin/batch` — one occurrence, many members → 207; `class_id` + `occurrence_date` in the body) + **per-occurrence attendees** (`GET /api/v1/checkin/attendees`) + attendance **streak** (`GET /api/v1/streak`) + per-cycle class usage (feeds member billing detail). `CheckinService` is a facade over `CheckinOccurrenceResolver` (the one-way `checkin → classes` dependency: reuses `ClassesExpander` + `ClassesMaterializer`) + `CheckinMemberGate` (one gate eval → block-vs-warn); the read-only `CheckinAttendeesService` lists an occurrence's attendees |

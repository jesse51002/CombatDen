...
flowchart TB
  ...
  FB["⚙️ FastApiBackend — CRM / billing + video API<br/>15 domains · 117 routes<br/>members · gyms · ranks · rewards · waivers · discounts<br/>memberships · plans · stripe_webhooks · tasks<br/>classes (CRUD + instance/range exceptions + schedule board incl. signup_count + undo/reschedule + expander/materializer)<br/>checkin (is_member-gated/lazy single + batch check-in + sign-ups/reservations + per-occurrence combined roster + attendance streak + per-cycle usage — reuses classes' expander/materializer)<br/>videos (real-gym feed + LLM spec/agent authoring:<br/>litellm for query gen / feed refiner · Pydantic AI for the conversational agent)<br/>presets (import a gym template + public template catalog) · theme (gym showcase)<br/>+ payments · sync · reconciler (router-less; reconciler = twice-daily billing sweep)"]
...
| `classes` | Gym class scheduling (the *producer* side). **Class CRUD** (admin/owner-gated writes via `verify_gym_admin_or_owner`; trainers read-only via `verify_gym_employee`) + per-occurrence **instance / range exceptions** + the **schedule board** (`GET /instances` — classes expanded over a window with exceptions applied, each occurrence carrying `attendance_count` AND `signup_count`, future and past) + **un-occur (cancel) / reschedule** a single occurrence (its attendance teardown loops the `checkin` domain's shared `CheckinReverser` per attendee — a deliberate `classes → checkin` dependency, so the per-member reversal isn't duplicated), all built on a **pure recurrence + exception expander** (`ClassesExpander`, no I/O) and `ClassesMaterializer` — THE single range-parameterized `class_history` materialize entry point (`materialize(gym_id, start_date, end_date)` + the `materialize_current` convenience wrapper, plus the single-occurrence `find_or_create_history` primitive both are built on), owning its own load + expand and a shared forward cutoff (`materialize_future_hours`) so a not-yet-started occurrence's editable fields aren't frozen too early. The attendance *consumer* side (gated check-in, batch, sign-ups, streak, cycle usage) lives in the sibling **`checkin`** domain, which depends on this domain's expander + materializer; `signup_count` is read the other way — a plain cross-domain SQL read of `checkin`'s `class_signups` table, not a code dependency |
| `checkin` | Class **attendance + sign-ups** — the consumer side of `classes`. **`is_member`-gated lazy check-in** (`POST /api/v1/checkin`: materializes the occurrence on first attendance, awards points, auto-ends depleted packs; idempotent). `is_member=true` (kiosk) runs the strict gate — best eligible plan with capacity (trial→one_time→recurring), else rejected with a `skip_reason`. `is_member=false` (staff, the default) records a clean check-in but holds a warned one for confirmation (`requires_confirmation`, nothing written) unless `ignore_warnings` overrides — which records with best-available/NULL attribution and the `warnings` surfaced. **Sign-ups (reservations)** — `POST` / `DELETE /api/v1/signup` — reserve a member a spot on an occurrence WITHOUT recording attendance (idempotent create; `SignupService`). Capacity is reserving: both the sign-up create path and the check-in gate reject/block only when the DISTINCT count of members **signed-up OR attended** (`class_signups` ∪ `member_attendance`, one shared union query) is already at the occurrence's effective `max_capacity` AND this member isn't already in that set — so a signed-up member's later check-in is never blocked by their own reservation, only a fresh uncounted walk-in is. Plus **batch check-in** (`POST /api/v1/checkin/batch` — one occurrence, many members → 207; `class_id` + `occurrence_date` in the body) + the **combined roster** (`GET /api/v1/checkin/attendees` — everyone signed up OR attended, each flagged) + attendance **streak** (`GET /api/v1/streak`) + per-cycle class usage (feeds member billing detail). There is no facade — the single-check-in router injects `CheckinClassResolver` (the one-way `checkin → classes` dependency: reuses `ClassesExpander` + `ClassesMaterializer`) + `CheckinMemberGate` (one gate eval → block-vs-warn/confirm) directly, and `BatchCheckinService` injects the same two; the read-only `CheckinAttendeesService` builds the combined roster. Check-in **reversal** also lives here: `CheckinReverser` is the shared per-member reversal core (delete attendance + claw back points + drop the activity + reverse the pack auto-end), importing **nothing** from `classes`; `CheckinRemover` (the `DELETE /checkin` endpoint) wraps it with occurrence-finding. **Deliberate exception to the one-way seam:** `ClassesUndoService` (in `classes`) loops `CheckinReverser` over every attendee when it un-occurs / future-wipes an occurrence — a sanctioned `classes → checkin` dependency (the OPPOSITE direction) so the reversal isn't duplicated; cycle-free because the reverser has no `classes` import |

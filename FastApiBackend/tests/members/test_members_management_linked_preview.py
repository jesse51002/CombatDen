...
1. The ``user_gym_profiles_unfiltered`` row is unchanged
...
async def test_preview_link_with_paying_parent_no_mutation(
    ...
    try:
        ...
        parent_profile_before = await _fetch_profile(db_pool, parent.crm_user_id)
        ...
        child_row = await _fetch_profile(db_pool, child.crm_user_id)
        assert child_row["account_linked_to_id"] is None
        assert child_row["linked_discount_id"] is None

        parent_row = await _fetch_profile(db_pool, parent.crm_user_id)
...
async def test_preview_unlink_not_linked_raises(
    ...
    try:
        with pytest.raises(ValueError, match="not linked"):
            await management_service.preview_unlink_account(member.crm_user_id)
    finally:
        await delete_member_data(db_pool, member.crm_user_id)

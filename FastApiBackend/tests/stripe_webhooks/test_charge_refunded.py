...
Verifies that a refund:
  - finds the parent ``user_gym_charges`` row via ``stripe_charge_id``
    ...
    async with db_pool.session() as session:
        result = await session.execute(
            text(
                ...
                "stripe_refund_id, refunds_charge_id, crm_user_id "
...
async def _fetch_parent_charge_id(db_pool, stripe_charge_id: str):
    async with db_pool.session() as session:
        result = await session.execute(
            text(
                "SELECT charge_id FROM user_gym_charges "
...
async def test_charge_refunded_single_full_refund(
    ...
    assert str(r["crm_user_id"]) == str(webhook_fixture.crm_user_id)

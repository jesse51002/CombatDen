...
addressed by its ORIGINAL date, never its effective (post-reschedule) date,
so a naive ``[occurrence_date, occurrence_date]`` expand window would
silently drop a rescheduled occurrence (the inner expander filters on the
EFFECTIVE date landing in-window). ``_resolution_window`` widens the window
to also cover a reschedule's ``new_date`` before expanding once and
filtering back to ``original_date``.
...
async def test_no_versions_returns_none() -> None:
    ...
    result = await resolution.resolve_original(uuid4(), _OCCURRENCE_DATE)
...
async def test_non_recurrence_date_returns_none() -> None:
    ...
    result = await resolution.resolve_original(class_id, _OCCURRENCE_DATE)
...
async def test_cancelled_occurrence_returned_flagged_when_include_cancelled() -> (
    ...
    result = await resolution.resolve_original(
        class_id, _OCCURRENCE_DATE, include_cancelled=True
    )
...
async def test_rescheduled_occurrence_resolves_by_original_date() -> None:
    ...
    when addressed by its ORIGINAL date — the window-widening fix. A bare

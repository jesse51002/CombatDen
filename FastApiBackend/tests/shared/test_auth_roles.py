...
async def test_get_employee_id_returns_the_matched_row_id() -> None:
    ...
    assert (
        await auth.get_employee_id(uuid4(), {"email": "a@b.com"}, OWNER_ADMIN)
        == employee_id
    )
...
async def test_verified_account_returns_lowercased_email() -> None:
    ...
    assert (
        await auth.verify_verified_account({"email": "New@Owner.com"})
        == "new@owner.com"
    )
    assert _params(session)["email"] == "new@owner.com"
...
async def test_verified_account_403_when_unconfirmed() -> None:
    ...
    with pytest.raises(HTTPException) as exc:
        await auth.verify_verified_account({"email": "ghost@test.com"})
    assert exc.value.status_code == 403
...
async def test_member_self_passes_for_the_member() -> None:
    ...
    await auth.verify_member_self(
        member_id, {"email": "Member@Test.com"}, gym_id=gym_id
    )

    params = _params(session)
    assert params == {"member_id": str(member_id), "email": "member@test.com"}
...
async def test_member_self_passes_for_a_parent_on_a_child_row() -> None:
    ...
    await auth.verify_member_self(uuid4(), {"email": "parent@test.com"})
...
async def test_member_self_403_when_account_unverified() -> None:
    ...
    with pytest.raises(HTTPException) as exc:
        await auth.verify_member_self(uuid4(), {"email": "member@test.com"})
    assert exc.value.status_code == 403
...
async def test_member_self_403_when_email_does_not_match() -> None:
    ...
    with pytest.raises(HTTPException) as exc:
        await auth.verify_member_self(uuid4(), {"email": "other@test.com"})
    assert exc.value.status_code == 403
...
async def test_member_self_403_when_member_is_at_another_gym() -> None:
    ...
    with pytest.raises(HTTPException) as exc:
        await auth.verify_member_self(
            uuid4(), {"email": "member@test.com"}, gym_id=uuid4()
        )
    assert exc.value.status_code == 403
...
async def test_member_self_404_when_member_missing() -> None:
    ...
    with pytest.raises(HTTPException) as exc:
        await auth.verify_member_self(uuid4(), {"email": "x@y.com"})

...
async def _create_linked_child(
    ...
    the filtered ``member_billing_profile`` view.
    ...
    async with db_pool.session() as session:
        await session.execute(
            ...
            {
                "parent": str(parent_member_id),
                "id": str(child.member_id),
            },
        )
        await session.commit()
    return child.member_id
...
async def test_linked_discount_child_loses_qualification_tiers_shift_up(
    ...
    Exact assignment by member_id ordering is an implementation
    ...
    try:
        ...
        # path — including the ``(member_id, plan_id)`` filter

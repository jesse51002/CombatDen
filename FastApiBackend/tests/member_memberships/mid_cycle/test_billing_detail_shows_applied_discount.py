...
membership's applied discounts via the snapshot model.
...
path now aggregates each membership's currently-active applied-discount
snapshots (filtered view, joined to the pinned value version) into the
membership card.
...
async def test_billing_detail_surfaces_active_applied_discount(
    ...
    try:
        ...
        # apply below will sync and write back a coupon (making the snapshot
        # visible through the filtered applied-discounts view).
        ...
        # The rich snapshot fields the CRM needs to group + remove the
        ...
        # Ongoing forever (no duration / explicit end_date) -> snapshot
        # end_date is NULL and the currently-active filter keeps it.

...
async def _fetch_total_price(db_pool, crm_user_id: UUID, plan_id: UUID) -> int:
    ...
    async with db_pool.session() as session:
        result = await session.execute(
            ...
            {"crm_user_id": str(crm_user_id), "plan_id": str(plan_id)},
        )
        row = result.mappings().fetchone()
    if row is None:
        raise AssertionError(
            f"No active membership for crm_user_id={crm_user_id} plan_id={plan_id}"
        )
    return int(row["total_price"])


async def _fetch_profile_monthly(db_pool, crm_user_id: UUID) -> int:
    async with db_pool.session() as session:
        result = await session.execute(
            text(
        ...
        row = result.mappings().fetchone()
    if row is None:
        raise AssertionError(f"No profile row for crm_user_id={crm_user_id}")
    return int(row["total_monthly_recurring_price"] or 0)


async def _fetch_item_id(db_pool, crm_user_id: UUID, plan_id: UUID) -> UUID:
    async with db_pool.session() as session:
        result = await session.execute(
            text(
...
async def test_family_same_plan_both_rows_sum_to_plan_total(
    ...
    try:
        ...
        parent_monthly = await _fetch_profile_monthly(db_pool, parent.crm_user_id)
...
async def test_cross_family_isolation(
    ...
    try:
        await memberships_service.start(
            crm_user_id=family_a.crm_user_id,
            gym_id=gym_id,
            plan_id=plan.plan_id,
            price_id=plan.price_id,
        )
        ...
        b_monthly = await _fetch_profile_monthly(db_pool, family_b.crm_user_id)
        assert b_monthly == 5000, (
            f"Family B profile monthly should be unchanged at 5000, got {b_monthly}"
        )
    finally:
        await delete_member_data(db_pool, family_a.crm_user_id)
        await delete_member_data(db_pool, family_b.crm_user_id)

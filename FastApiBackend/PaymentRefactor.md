...
## 7. Durable writeback reconciliation — best-effort post-charge writes (not built)

> Surfaced by the billing review as finding **C-012 / C-015**. The DB writeback
> after a sync/charge is **inherently best-effort**: the DB write always happens
> *after* the Stripe mutation, so there is an unavoidable crash window where Stripe
> succeeded but the DB write hasn't landed. Today a swallowed step only self-heals
> on the next organic converge, which is not guaranteed and — for the payer
> `sub_id` write specifically — can go wrong.

### The problem
`PaymentSyncWriteback.write()` (and the one-time writeback) persist several
independent best-effort steps (row `applied` stamps, post-discount prices, coupon
links, the payer `stripe_sub_id_month`, the monthly total). The payer `sub_id`
write is the **only** step with **no caller verify-or-revert backstop**, and the
create decision in `execute_sync` keys off `bucket.existing_sub_id`. So if the
sub_id write is swallowed while the row stamps land, the next converge reads
`sub_id = NULL` → **creates a second subscription**; the immutable
`stripe_item_id` trigger then blocks re-stamping, the *tracked* sub orphans, the
orphan sweep reaps it, and a later push-sweep hits the cancelled sub →
`cancel_dead_subscription` **cancels a paying member** while the original sub keeps
billing. Transient double-bill + membership loss. The current backstops
(`PaymentPushSweep` / `SubscriptionOrphanSweep`) don't cover this path and are
themselves untested.

### The proper fix — a durable failed-writes store (transactional outbox)
A shared "failed writes" store inside the API that **records any swallowed/failed
writeback step** (which row, which write, the desired value), drained by a
reconciler on a short cadence (~15 min) that **retries the failed writes
automatically and across process crashes** — instead of relying on the next
organic converge. This makes every best-effort mirror write durably tracked and
self-healing, and is the general solution to *all* the best-effort writeback steps,
not just sub_id.

### Cheap interim mitigation (optional, also deferred)
Co-write the payer `sub_id` **and** the membership row `applied` stamps in **one
transaction** (both come from the same `execute_sync` result). That makes the
dangerous precondition — "rows stamped `applied` but `sub_id` NULL" — impossible;
a crash then degrades to the benign "neither landed → next converge makes a fresh
sub and the orphaned one is reaped with no live rows." Still best-effort, but the
*dangerous* tail (double-bill + cancel-a-paying-member) becomes the *harmless*
tail. Small, contained change in `sync_writeback`; worth doing if the full outbox
is far off.

### Open questions
- Outbox shape: a generic `failed_writes` table keyed by (entity, op, payload) vs
  per-step queues; how to make retries idempotent against an organic converge that
  already healed the same write.
- Whether the cheap co-write mitigation ships first as an MVP stopgap, or the
  outbox is built directly.

# CustomizationEngine — Coding Standards

This file guides Claude Code when working in this package.

## What this is

`customization_engine` is a **shared Flutter package** (not an app). It is the
...
**NOT to be confused with `../CustomizationService`** — that is the Python
...
- **App-agnostic by construction.** This package must NEVER import from
  ...
  app-specific inputs are injected through `CustomizationRuntime.initialize`
...
- **Package assets** live in `assets/` and are referenced by consumers as
  `packages/customization_engine/assets/...` (centralized in

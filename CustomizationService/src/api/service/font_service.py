"""Filesystem + catalog reads behind the font endpoint.

Resolves one font slot for a run into the delivery payload: family,
...
The Google Fonts catalog is process-scoped — one instance shared across
every API request, with the in-memory TTL cache. The pipeline writer
and this service hit the same Google catalog but each process holds
its own cache.
"""
...
def _catalog() -> GoogleFontsCatalog:
    """The one process-scoped catalog. Constructed lazily on first call
    so a missing env var only fails when the font endpoint is actually
    hit, not at import time."""
    global _CATALOG
    if _CATALOG is None:
        _CATALOG = HttpxGoogleFontsCatalog(
...
_CATALOG: GoogleFontsCatalog | None = None


async def resolve_font(
    ...
    * the family the pipeline picked is no longer in the live Google
      Fonts catalog (rare — Google occasionally retires families).
    ...
    if slot is None:
        raise NotFoundError(
            f"font slot {slot_id!r} is not declared in run {app_id}/{run_id}"
        )
    entry = await _catalog().lookup(slot.family)
    if entry is None:
        raise NotFoundError(
            f"font family {slot.family!r} (slot {slot_id!r}) is no "
            "longer in the Google Fonts catalog — re-run the pipeline "
            "to pick a current family"
        )

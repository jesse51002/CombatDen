...
Both the text/vision client (``LiteLLMClient``) and the image generator
(``LiteLLMImageGenerator``) route by the provider prefix on the model id
("anthropic/…", "gemini/…", "openai/…") and hand litellm that provider's
key. This is the single source of that mapping so the two never drift.
"""

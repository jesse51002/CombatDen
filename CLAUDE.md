...
- The automated per-PR review (`claude-code-review.yml`) has been **removed** — code review is done locally via `/code-review` before opening a PR (above), so the GitHub auto-review was a duplicate spend. Only `claude.yml` (the on-demand `@claude` assistant) remains.
...
- **Always run `/code-review` before creating a PR — it's a standard gate, not optional.** Review the change set, address the findings (fix the legitimate ones; for a false positive or an established convention, reply with the rationale instead of silently applying it; surface anything needing a human call), THEN open the PR.
- **Default review effort is `medium`** (`/code-review medium`). Use it for ordinary changes.
- **Billing-critical changes use `high`–`xhigh`.** When the diff touches the billing core — `FastApiBackend/src/memberships/`, `payments/`, `sync/` (the payment-sync engine), `reconciler/`, the Stripe webhooks, or anything that decides how real members are charged — raise the effort to `high` or `xhigh`. A mis-bill is expensive; the deeper review is worth the spend.
- This local review is **the** code review for the PR — there is no automated per-PR review on GitHub anymore (removed to avoid the duplicate spend).

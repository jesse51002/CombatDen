...
- **`/code-review` agents run on OPUS, never Fable** (Fable is extremely expensive; the review fan-out multiplies that cost). The built-in review workflow's `agent()` calls inherit the session model, so a Fable session silently runs Fable reviewers — passing "use opus" in the args does NOT work. Instead: launch the workflow, take the script path from the tool result, edit every `agent()` call's opts to add `model: "opus"`, and relaunch with `{scriptPath}`.
...
- **Fable — the explore-and-plan advisor, never the implementer.** Use Fable for the *initial/major explore pass* (the broad recon that must not miss anything) and for the *planning agent* whenever an implementation plan is being drawn up — its depth is what keeps explore exhaustive and plans sound. Two rules: **(1) always a fresh agent, never a fork** — spawn it new and hand it exactly the context it needs at creation, which keeps its (expensive) usage down (a fork also inherits the parent's model and ignores a model override, so from a non-Fable session it could never be Fable anyway); **(2) it stops at the plan** — implementation stays Opus and below (Opus for well-defined substantial builds and design passes, Sonnet for mechanical volume). **If a task feels like it needs Fable-level *implementation*, that's the signal the plan isn't thorough enough — send it back to Fable for a deeper plan, don't put Fable on the keyboard.** Fable advises and plans; Opus/Sonnet execute.
...
- **Right the first time.** If the plan feels thin, or you catch yourself planning to "figure it out while coding," the plan is not done — deepen it (send it back to the Fable planning agent per the model defaults) rather than starting to build. A deeper plan is always cheaper than a rebuild.

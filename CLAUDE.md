...
Pick each subagent's model by the task using the *Workflow / sub-agent model defaults* section below (Sonnet for mechanical volume, Opus for well-defined substantial builds, Fable only for genuinely many-layered uncertain work). Never run two subagents concurrently over the same files — parallelize only across disjoint systems/domains.

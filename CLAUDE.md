...
- **ALWAYS kill any previous meld before launching a new one** — a second launch hands off to the open window and exits, git deletes the staging dirs, and the new diff shows empty. Before every `make meld` / `make meld-origin`, run the kill first **as its own separate command** (`flatpak kill org.gnome.meld 2>/dev/null; pkill -x meld 2>/dev/null; true`), then launch `make meld` in the background. This is a hard rule, not a recovery step.
- **Never put the kill in the same command that launches `make meld`, and use `pkill -x meld` not `pkill -f meld`.** `pkill -f meld` matches the *full command line*, so it matches the very bash invocation that contains `make meld` and kills the launcher mid-run (it surfaces as the launch "failing" with exit 144 and no meld window). `pkill -x meld` matches the process *name* exactly (`meld`), never the `bash` shell. Kill in one tool call, launch in the next.

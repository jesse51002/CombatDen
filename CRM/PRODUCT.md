...
payroll, not handle billing, not operate check-in. CRM deliberately sits *beside*
the gym's existing CRM and never touches the operational critical path.

## Product Purpose

CRM is the **gym admin web app** for a member-retention layer that runs alongside a class-based gym's existing CRM. The member-facing mobile app delivers contextual training content, points-based rewards, and attendance-based rank progression; CRM is where the owner configures and monitors all of that.
...
CRM surfaces two things the admin needs to manage:
...
Success for this surface: an owner can answer "is my retention engine working, and what do I change?" in under a minute, and a prospect evaluating these screens believes the product is more crafted than anything else they're shopping.

**Note on scope:** this is the production gym-admin web app — real Supabase auth, real
FastApiBackend data, Bloc state management, and Stripe billing on the CRM feature stack, with
two read-only edge integrations (the VideoService feed/gym-detail and the ThemeService theme
preview). The same screens double as a live sales artifact — they get screenshotted to sell —
so design rigor is held high everywhere.

**Note on visual direction:** CRM has a fresh, standalone visual identity, forked
...
- No other formal requirements captured yet; revisit WCAG level and keyboard-operability
  targets as the admin surface matures.

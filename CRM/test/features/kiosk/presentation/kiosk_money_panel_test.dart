...
///
/// It must NEVER promise a receipt: CombatDen has no mailer, and the connected
/// account notifies a member on a FAILED payment only, so a receipt sentence
/// would be a falsehood told on the one screen where money is about to move.
/// That failure notice is also why the address is worth showing unmasked here.
///
/// An empty address reaches this panel only via a payer adopted from the gym's
/// own records; the sentence would render with a trailing blank, so the whole
/// line is dropped — the same guard the results panel carries.
void main() {
  ...
  testWidgets('a payer with NO address gets no receipt line at all',
    ...
    expect(find.textContaining('receipt'), findsNothing);
    // The guard drops one line, not the screen.
...
/// A review state with a real preview behind it, so the panel renders the
/// whole money block rather than an empty shell.

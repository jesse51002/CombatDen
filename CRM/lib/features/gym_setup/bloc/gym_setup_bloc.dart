...
/// the next `GET /{gym_id}/onboarding` after each response,
...
class GymSetupBloc
  ...
  Future<void> _onCheckRequested(
    ...
    // The auth gate only mounts this wizard when the user
    // administers no gyms (GET /api/v1/gyms/ returned []), so there
    // is nothing to resume — go straight to the welcome step. The
    // onboarding poller (gym-id-scoped) drives the rest of the flow
    // once a gym is created in this session.
  ...
  Future<void> _onResumeAccepted(
    ...
    try {
      final link = await _gymRepository.refreshOnboardingLink(
        current.gymId,
      );
      emit(GymSetupStripeOnboardingStep(
        gymId: link.gymId,
  ...
  Future<void> _onStripePollNow(
    ...
    try {
      final status = await _gymRepository.getOnboardingStatus(
        current.gymId,
      );

...
/// Slide 3 — "Watch videos": the head of THIS gym's own curated feed
/// (`GET /api/v1/gyms/{gymId}/videos`, cached at kiosk entry), drawn as the
/// member app's video card.
///
/// It must never render `selectedGym.detail`: that showcase belongs to a
/// DEFAULT content gym, so its videos here would show another gym's feed to
/// this gym's members. The card anatomy is the MEMBER APP's, not the CRM
/// `VideoTile` (which carries admin Remove/Edit): a picture of the app being
/// marketed, not an admin surface.
class KioskVideosSlide extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return KioskSlideBody(
      // Centred fixed-width cards, so one video reads as deliberate.
      content: Row(
        ...
        children: [
          ...
          if (imageUrl.isNotEmpty)
            Image.network(
              imageUrl,
              fit: BoxFit.cover,
              // A thumbnail that won't load leaves the neutral fill, never a
              // broken-image box on a member-facing screen.
...
/// The app's play affordance as art — inert (the member watches in the app).
...
/// The channel's avatar, falling back to initials when the URL is missing or
/// won't load — never a broken image.
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        // Hidden stats -> name the channel rather than show an empty line
        // (or invent a count).
        Text(

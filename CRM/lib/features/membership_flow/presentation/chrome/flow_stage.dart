...
/// The centered, max-width content stage every flow step sits in. It scrolls
/// when the content outgrows the viewport; [center] decides whether shorter
/// content sits centered (result screens) or top-aligned.
///
/// A [footer] is PINNED to the bottom of the fold while the content scrolls
/// beneath it — a way out a tall grid can push below the fold is not a way
/// out. A [header] pins to the TOP for the same reason: whose plan / waiver /
/// card this is must not scroll away mid-step.
///
/// Its content rail is capped at the SURFACE's own measure, which is what
/// separates it from the kiosk's `KioskStage`: the check-in lane
/// (home / class pick / glance / the terminal screens) is kiosk-only and never
/// renders at a second scale, so it keeps its own stage and reads
/// `DesignConstants.navMaxWidth` directly. This one is the membership flow's,
/// and the width comes from whichever scale the host mounted.
...
/// The stage's content rail: centered, capped at the SURFACE's own measure,
/// padded.
class _Railed extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Align(
      ...
      child: ConstrainedBox(
        constraints: BoxConstraints(maxWidth: scale.stageMeasure),
        child: Padding(

...
The member-detail screen provides a `ScheduleRepository` in its scope for both the pick body's reads and Reserve's mutation (cross-feature reuse of the schedule repo; no schedule bloc). The **Waivers** "Sign waiver" affordance opens `SignWaiverDialog` (`lib/shared/widgets/sign_waiver_dialog.dart`, over the shared `SignWaiverPanel`) — renders the waiver body read-only, shows the ESIGN/UETA disclosure, captures a typed name + consent, and records a signature via `POST /api/v1/waivers/{id}/signatures` (echoing the displayed `waiver_version_id`; 409 stale-version → re-open). The membership-purchase wizard has a `signWaivers` step that blocks payment until every required waiver is signed (with the backend 422 gate as the backstop).

...
- **One feature is a shared MODULE rather than a screen.** `features/membership_flow/` owns no screen and no cubit — it holds what both membership-purchase surfaces render and obey, so a rule, a price string, a plan word or a component cannot be changed on one surface only. The two surfaces are the kiosk signup (`features/kiosk/`) and the staff start-memberships wizard (`features/member_details/.../start_memberships/`); each keeps its own orchestration, because that is where they genuinely differ (the kiosk takes a card before review, the desk settles after it) and it is the layer holding the kiosk's double-charge defences.
  - **The desk's orchestration is `features/member_details/bloc/membership_wizard/`** — `MembershipWizardCubit`, assembled from concern mixins (roster / plans / waivers / money / commit) on a shared base, walking `who → plans (×N) → [waivers] → reviewCharges → payment → results`. It calls `MemberRepository` / `MembershipsRepository` DIRECTLY rather than dispatching through `MemberDetailBloc`, exactly as the kiosk does: a purchase is a self-contained transaction with its own idempotency key, its own retry scope and its own `_sentAttempts` latch, and routing it through the page's bloc puts all three somewhere a second screen can reach them. **`previewRequest` is a LIFETIME, not a record:** every cart or roster edit nulls it, a preview answer whose own request is no longer staged is DROPPED rather than rendered (the review's trash is a tap, not an awaited call, so two of them inside one round-trip leave two previews in flight and nothing makes the network answer in order — landing the superseded cart's total beside the live cart is exactly the "quoted one number, charged another" failure the step exists to prevent), and the 422 waiver gate dies with it, since a refusal about a cart that no longer exists must not keep demanding a signature for a membership nobody is buying. The HOST (`start_memberships_wizard.dart`, a ~200-line `AppDialog`) dispatches `MemberDetailRequested` on every NEW landed result so the page behind re-reads — a partial moved real money, and a retry that creates the rest is a second landing that the page would otherwise never hear about (`announcesNewLanding`, which compares by IDENTITY precisely because an attempt with no breakdown of its own — a decline, a 409 replay — restores the previous response rather than building a new one, so it costs no extra read); `MemberDetailBloc`'s own `StartMembershipsRequested` channel is still live and still cleared on open, but the run no longer rides it. Two things live in the host rather than the cubit because both need a `Navigator`: the nested staff dialogs (`wizard_dialog_routes.dart` — new member, link member, change payer, update card, one-off card) and the two navigations (close, open a member's page). The step SCREENS are `start_memberships/steps/`, one per cubit step, each returning a `WizardStepScaffold` (the desk's binding of the shared `FlowStepScaffold`: its shell is the dialog body, its rail is the desk's six named stages — `wizard_rail.dart` — while `Step N of M` in the topbar counts the REAL `5 + N` spine, which is why the rail may stay short without lying).
  - **Both `MembershipFlowCopy` implementations answer the STEP HEADS** (`rosterStepTitle` / `planStepTitle` / `waiverStepTitle` / `reviewStepTitle` / `paymentStepTitle` / `resultsStepTitle` and their subtitles), so a step's title cannot be reworded on one surface only. A step only ONE surface walks keeps its head on that surface's own copy class as a non-override member (the kiosk's eight, reached via `kiosk_step_copy.dart`) — pushing it onto the abstract class would force the other surface to answer with a lie. Everything the DESK says that the kiosk has no version of (a payer switch, a one-off card, a blocked-plan reason naming the control that clears it) lives in `start_memberships/wizard_copy.dart` as plain constants, for the same reason `FlowDiscountCopy` does: it renders once, so there is no second voice to drift from.
  ...
  - `presentation/` — the component set (`chrome/` for the step scaffold, shell contract, rail, foot and buttons; `models/` for the view models; `widgets/` for everything a step composes). `Flow*`-named, one public widget per file.
    ...
    - **And it holds no SENTENCE either.** Every user-facing word arrives through `MembershipFlowCopy` (`config/flow_copy.dart`) — an abstract class of typed methods, deliberately not a map, so a missing line fails at compile time rather than rendering blank on whichever surface reached it first. Methods take the data they interpolate (names, amounts, dates, counts), never a pre-formatted sentence, which is what lets one voice reorder or drop a clause the other keeps. `KioskFlowCopy` is the member voice, `StaffFlowCopy` the desk's. A host that needs a step-specific word still passes it (`FlowFoot(primaryLabel:)`, `FlowSignPanel(eyebrow:)`); those parameters are nullable and fall back to the copy. `test/features/membership_flow/presentation_copy_guard_test.dart` fails on any sentence-like literal left under `presentation/`, so a wording change is a change to both surfaces or to neither.
  ...
  - `config/` — everything that differs between the two surfaces, named in one place and handed down by the `MembershipFlowTheme` `InheritedWidget` (`of` → the scale, `copyOf` → the copy; both assert rather than defaulting, since a silent fallback is how a surface renders at the other one's size or in the other one's voice).
    - `MembershipFlowScale` — the surface's TYPE ramp plus its form measure, all getters so light/dark keeps resolving through `themeController`. A shared component asks for a ROLE (`scale.panelTitle`) and the surface's scale picks the `DesignConstants` token; the scale SELECTS between existing tokens and restates no value. `KioskFlowScale` maps every role onto the `kiosk*` ramp; `AdminFlowScale` maps onto the app's own admin ladder plus the `flow*` roles that ladder lacks (`flowStatement`, `flowName`, `flowButtonLabel` / `flowButtonGhostLabel` and their three paddings, `flowFormMeasure`). It is deliberately **not** a `ThemeExtension` — this app never reads `Theme.of(context)` (see *Theming System*). Non-type tokens (colours, radii, spacing, icon sizes, shadows) still come straight from `DesignConstants` at the call site.
    - `MembershipFlowConfig` — the two factories, and the ONLY place a surface's capability set is named: `scale`, `copy`, `planGates` / `planNotes` (deliberately different types, so an advisory can never quietly become a block), `CartPolicy`, `IdentityPolicy`, and a nullable `DiscountsCapability`. **Disabled means ABSENT, not `false`:** `MembershipFlowConfig.kiosk()` takes no discounts argument, so the kiosk cannot carry one. `CartPolicy` is `1`/`1` for the kiosk and unbounded for the desk (it gates the quantity stepper — absent, never disabled); `IdentityPolicy.masked()` is the lobby reading of an address and `.full()` the desk's, and the kiosk's `kioskMaskedEmail` now delegates to it so the two cannot drift.
  - `discounts/` — **staff-only**, and the only place discount UI and math live: the inline panel that unfolds inside a picked-membership card (chips, the gym's presets as a list, and the custom form whose lifetime is a duration span XOR an explicit `end_date` XOR neither), plus the labels and the live struck-through price. Every figure comes from `domain/discount_math.dart`; nothing here re-derives one. It has its own `FlowDiscountCopy` constants rather than a third `MembershipFlowCopy` — it renders on one surface, so there is no second voice to drift from.
  - The kiosk's structural no-discounts rule survives all of it: `domain/discount_math.dart` and the whole of `discounts/` are staff-only and the kiosk must never import either, and no shared component may import a staff-only module. `test/features/kiosk/kiosk_forbidden_imports_test.dart` is the enforcement, and it walks three scopes: `lib/features/kiosk/` (the saved-card / payer-picker / discount ban plus the literal word "discount"), the shared component set (no staff-only module), and the WHOLE module — `lib/features/membership_flow/` may import neither `features/kiosk/` nor `features/member_details/presentation/`, since it is what both surfaces depend on.
...
- **Auto-saving surfaces still owe a confirmation — an inline status line, not a toast.** Settings → **Gym profile** (`features/settings/presentation/sections/gym_profile_section.dart`) is the app's one auto-save editor: it has **no Save button**, the logo persists the moment it's picked, and each text field commits **on blur** (never per keystroke — `gyms.gym_name` is NOT NULL / non-empty and renders in every member's app, so a mid-word save would ship a half-typed name). A commit only fires when the value actually changed since the last dispatched save, a blank gym name is rejected on blur (the field reverts and says why under it), and a blank address is a real clear that sends `null`, never `''`. Because the button is gone, the confirmation is `GymProfileSaveStatusView` (`sections/gym_profile_save_status.dart`) — a permanently-present line that reads "Changes save automatically." at rest, so the contract is stated *before* the first blur, then goes saving → "Saved." (the app's `check_circle_sharp` + `goodGreen` success vocabulary) → back to rest. Failure escalates to the shared `ErrorMessage` banner plus an explicit **Try again** button, because "blur the field again" is undiscoverable. A SnackBar per commit is the wrong shape here — three fields would mean three toasts. If another section adopts auto-save, extract that status widget to `lib/shared/widgets/` rather than cloning it.
- **An auto-saving handler must not run on bloc's default concurrent transformer.** `SettingsBloc`'s `GymProfileSaveRequested` is registered with `transformer: (events, mapper) => events.asyncExpand(mapper)` (sequential) because blur-driven commits can land a keystroke apart; concurrent handlers would race and would each read a stale `selectedGym` in their no-op guard. `bloc_concurrency` is deliberately **not** a dependency — `asyncExpand` is the whole of `sequential()`.
- **A dialog hosting an auto-saving section must not close on the save.** `GymProfileDialog` used to pop on the section's `onSaved`; under auto-save that would slam shut on the first field commit, so it now carries an explicit **Done** action and `onSaved` is a notification hook only.

## UX Requirement: a PREVIEW must never look like a SAVE

The Theme tab previews a theme live and persists it only when **"Set as app theme"** commits. Those are two different states and the UI must never let one read as the other — a gym owner once set a theme, never pressed the button, and believed his members' app had changed because the picker's checkmark had jumped to the card he tapped. The rules that keep them apart, in `features/members/presentation/widgets/member_app/theme_tab/`:

- **`theme_preview_state.dart` is the single source of truth** for "what is previewed vs. what is saved" (`previewedDesignId` / `hasUnsavedThemePreview` / `savedThemeLabel` / `confirmLeaveThemePreview`). The grid, the pane's status line, and the leave confirm all read it, so they can never disagree. `selectedGym.savedThemeDesignId` is the gym's real theme (null = never set); `selectedGym.designId` (with `ThemeRuntime.activeDesignId` as the boot/deep-link fallback) is merely what the phone is showing.
- **The "this is live" vocabulary belongs to the SAVED design only.** `ThemeCard` takes `isSaved` + `isPreviewing` separately: the filled `check_circle_sharp` + heavy sapphire border + "Live for members" are `isSaved`'s alone; a previewed-but-unsaved row gets the quieter outline eye, `primaryColor10` tint, hairline border, and the words "Previewing only". Meaning never rests on colour alone — glyph shape, weight, and a literal sub-label all carry it.
- **State that only members can see must be stated in words, permanently.** The pane's always-on `InfoRow('Members see', …)` — plus the grid's zero-state line when nothing is saved — is the real guarantee, because the nav rail (`pushReplacementNamed` inside shared nav chrome) and browser back/refresh are **not** interceptable. Do not weaken it into a warning that only appears when something looks wrong: the previewing sub-line is plain `text2nd` prose, deliberately **not** a `WarningMessage` (`okYellow` would imply a problem, and there isn't one).
- **The one interceptable exit asks first.** `MemberAppScreen._onTabSelected` and the pane's "Leave without setting" both call `confirmLeaveThemePreview`, which no-ops (returns true) when there's nothing unsaved or in the public browser (`gymId == null`), prompts otherwise, and on confirm reverts the preview to the saved design. Esc maps to cancel. **Every one of these affordances is inside the `gymId != null` guard** — the public theme browser can't save, so it must never be told something is unsaved.

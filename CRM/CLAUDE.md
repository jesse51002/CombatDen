...
The member-detail screen provides a `ScheduleRepository` in its scope for both the pick body's reads and Reserve's mutation (cross-feature reuse of the schedule repo; no schedule bloc). And (6) the **Rank section** (`features/member_details/.../sections/rank_section.dart`) over `RanksRepository.listRanks` + `getRankEnabled` — it loads the gym's rank ladder + enabled flag (re-fetched whenever `refreshToken` changes, like the other side-reads) to drive the belt, the **real attendance progress** (`classes_since_rank / classes_till_rankup`, both from the `/billing` payload), and the Promote / Change / Assign actions, which dispatch the **`MemberRankChangeRequested`** mutation to `MemberDetailBloc` (it holds a `RanksRepository`, calls the ranks `promote-member` / `set-member-rank` endpoints, then reloads member detail in place + bumps `refreshToken`, like a billing mutation). Only the **Waivers** "Sign waiver" affordance still opens the `ComingSoonDialog` placeholder.

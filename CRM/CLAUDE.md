...
- **CRM feature screens use Bloc.** Anything backed by the FastApiBackend — the **login / auth gate** (`features/login/`), **gym setup wizard** (`features/gym_setup/`), **members list** (`features/members_list/` → rendered by `MembersListBody` inside the **People** screen, `features/people/.../people_screen.dart` — the Members tab of the People section), **member detail + its billing dialogs** (`features/member_details/`), the **Memberships catalog** (`features/memberships/` — the Memberships / Discounts / Waivers screen; one `MembershipsRepository` behind a `PlansBloc`, `DiscountsBloc`, and `WaiversBloc`; the membership plan form and the waiver editor are full-screen **repository-direct** edit screens, not bloc-backed), the dashboard's **Overdue Payments section** (`features/home/bloc/overdue_payments_*` → `features/home/.../overdue_payments/`, which reuses `MembersListRepository`), the **Settings** screen's appearance + gym-timezone saves (`features/settings/` — a small `SettingsBloc` that persists the theme via `PUT .../employees/me/theme` (optimistic) and the gym's IANA timezone via `PUT /api/v1/gyms/{gymId}` (**not** optimistic: the **Gym timezone** section — `sections/gym_timezone_section.dart`, shown to every employee — shows the current zone with its live UTC offset and opens `dialogs/timezone_picker_dialog.dart`, a searchable picker over the FULL IANA database (the `timezone` package, lazily initialized on first use — never at app startup; every entry shows its current UTC offset, sorted by offset then name, filter matches name or offset), then confirms the pick via `ConfirmationModal`, saves, and only on success calls `selectedGym.updateTimezone` and SnackBars "Gym timezone updated" off the state's monotonic `timezoneSavedCount`; the backend re-mints every class's schedule version in the new zone — nothing is wiped, upcoming classes keep their local times)), and the **Schedule** screen's class **week board** + its Add/Edit Class form (`features/schedule/` — a `ScheduleBloc` over `ScheduleRepository` against the FastApiBackend `classes` domain: it **reads** `GET /api/v1/classes/instances` for the effective dated occurrences (each board card shows the occurrence's headcount as a `Symbols.group_sharp` chip reading **"N reserved"** on an upcoming occurrence and **"N reserved · M attended"** on a past one — the attended count only joins in once the card's `occurrenceInPast` flag is set — from `EffectiveClassInstance.signupCount` / `.attendanceCount`, carried onto `ScheduleClassEntry.signupCount` / `.attendeeCount` and rendered by the same two-count logic in both `ClassCard` (the board) and `ClassRow` (the dashboard's Upcoming Classes card, always the "N reserved" half since that list is never past)) + `GET /api/v1/classes` for the catalog, and **writes** via `POST /api/v1/classes` / `PUT /api/v1/classes/{class_id}` / `DELETE /api/v1/classes/{class_id}` — create / edit / soft-delete dispatched from `class_form_screen.dart` as `ScheduleClassCreated` / `ScheduleClassUpdated` / `ScheduleClassDeleted` (an EDIT whose schedule **shape** changed vs the loaded class — duration, recurrence unit/interval, the `weekday_slots` map (which days/slots are active, each slot's time + instructor — several times per day are legal), start/end date — first confirms via `ConfirmationModal` that the change applies going forward: upcoming dates that no longer match the new time lose their reservations and early check-ins (points reversed), past classes unaffected; identity-only edits (name, description, capacity, points, image, plans) submit without the warning. The delete confirm's copy states that upcoming reservations and early check-ins are cleared (points reversed) while past attendance history is kept) — plus **cancel exceptions** + a **two-view occurrence flow**: tapping a board card opens a small **chooser dialog** (`presentation/dialogs/class_occurrence_chooser_dialog.dart`, naming the tapped occurrence's date) with two choices, each pushed with `BlocProvider.value` so it shares the board's bloc — **"This occurrence"** opens `presentation/screens/class_occurrence_screen.dart`, a dedicated occurrence-focused screen (header = the class image banner at a **fixed 16:9 aspect ratio** — `widgets/occurrence/class_occurrence_header.dart`'s `_kBannerAspect`, matching the board card's own `ClassCard._kCardImageAspect` so a class's image reads the same on the board and here — when the class has one, then the class name + the occurrence's date), and **"All future occurrences"** opens `presentation/screens/class_form_screen.dart`, now purely the class-**definition** editor (name, description, recurrence, `ClassDaysSection`'s per-day/per-"all" columns of `ClassSlotRow`s — several times per day are legal, each with its own optional instructor, added/removed via "Add time" / a row's remove (×) — capacity, points, image, start/end date, the "Cancel a date range" secondary action, delete) with no per-occurrence actions. The occurrence screen's **"This day's details"** block defaults to a **read-only view** (`widgets/occurrence/class_occurrence_read_only_details.dart`'s `ClassOccurrenceReadOnlyDetails` — date / time (the start–end **range** via `classTimeRangeLabel`, with a small duration caption) / instructor / max capacity as a readable detail panel: `DetailField` tiles (icon + dim label above a larger, prominent value, plus an optional small `caption` line; `lib/shared/widgets/detail_field.dart`, distinct from the dense inline `InfoRow`) laid out in a responsive `FillGrid`, sourced straight from the tapped entry) — a tapped occurrence is opened to be **seen** first, not dropped straight into an editable form — with an **Edit** button that switches to the editable **override-edit section** (`widgets/occurrence/class_occurrence_override_section.dart`) and, beside Edit, a destructive-styled **"Cancel this class"** button — shown only while the occurrence is still cancellable (`ClassOccurrenceScreen._cancellable`: not already cancelled, today or later) — which confirms via `ConfirmationModal` (the copy adds the occurrence's real stakes when its counts are > 0: cancelling removes its N reservations and reverses its M check-ins and their points) then dispatches `ScheduleInstanceCancelled` (`POST /api/v1/classes/{class_id}/exceptions/instance` with `is_cancelled:true`); the override-edit section itself covers instructor, start time, max capacity, and a **date** field, pre-filled from the tapped occurrence's *effective* values (`ScheduleClassEntry.resolvedInstructorId` / `resolvedClassTime` / `maxCapacity` / `classDate`, enriched onto the tap payload in `schedule_screen.dart`'s `_entryFromInstance`), plus a **Cancel** button that discards in-progress edits (resets the fields to the entry's values) and returns to the read-only view without saving. The date field **reschedules** the occurrence to ANY other day (past, today, or future — the backend rejects only an exact-instant collision with another occurrence, surfaced as an inline 409): it defaults to the occurrence's current *effective* date, and `_save` judges `newDate` against the occurrence's **`originalDate`** (its identity) — an untouched picker on an already-rescheduled occurrence therefore re-sends the current effective date, which **preserves the move** (the upsert full-replaces the exception row, and the backend treats a re-sent unchanged landing as a no-op — no attendance wipe); picking the original date back is the explicit "move it home" action and omits `new_date`. A plain retime/instructor/capacity save on a never-moved occurrence still omits `new_date` entirely. A save the backend treats as an attendance-wiping move first warns via `ConfirmationModal` ("Move this class?" — moving to a time that hasn't happened yet clears its N check-ins and reverses their points; reservations move with the class): the save actually sends `new_date` (picked ≠ `originalDate` — a move-home omits it and never wipes), the landing instant changed (picked date ≠ `classDate` OR the edited time ≠ `resolvedClassTime` — the preserve-the-move re-save never warns), the occurrence has recorded check-ins, and the target **instant** (picked date + edited start time) is still ahead of now — the backend decides wipe-vs-keep by the instant, never the day, so moving an already-moved class to later today warns too, while a target instant already past keeps check-ins (re-dated) and stays silent. Save dispatches `ScheduleInstanceOverridden` (`POST /api/v1/classes/{class_id}/exceptions/instance` with `is_cancelled:false` and the override fields set, carrying the unedited `resolvedDurationMinutes` through so the upsert never blanks a duration override the field doesn't expose, plus the optional `new_date`; `EffectiveClassInstance.maxCapacity` reflects the per-day `new_max_capacity` instance override, resolved in `classes_schedule_reader_service.py::_build_row` (falling back to the class default) to match what the check-in capacity gate enforces, so the prefill stays correct and a re-save won't revert a saved override); a successful save pops the whole screen (no need to return to the read-only view). And the relocated `ClassOccurrenceActions` block (`widgets/form/class_occurrence_actions.dart`), which stays visible in both view and edit mode: **"Reserve members"** — the FUTURE-side counterpart of "Update attendees" below, shown while the occurrence hasn't already passed (`ClassOccurrenceScreen._canSignUp`: today or later, mirroring `_cancellable`'s window — a reservation for an already-passed session wouldn't make sense, though the backend's `/signup` endpoint itself imposes no time gate), opening `presentation/dialogs/signup/class_signup_dialog.dart`; "Update attendees" (the batch staff check-in) — shown only when **check-in is open** (`ClassOccurrenceScreen._checkInOpen`: the occurrence has started / passed, or starts within `_kCheckInOpensHours` = **2h**, mirroring the backend `checkin_opens_hours_before_start`; a class further out hides the action and the backend rejects it anyway), paired with a small caption under the button row ("Adding an attendee checks them in and awards their class points.") explaining what the action does. "Cancel this class" no longer lives in this block — it moved beside **Edit** in `ClassOccurrenceReadOnlyDetails` (see above). When the viewed occurrence IS cancelled and `EffectiveClassInstance.cancellingRangeId` is non-null (a RANGE exception, not an instance one, is what cancelled it — the backend threads this onto the board row from the expander's own precedence resolution), `widgets/occurrence/class_occurrence_cancelling_range_section.dart`'s `ClassOccurrenceCancellingRangeSection` renders in the details block's place instead: a self-contained side read (`ScheduleRepository.listRangeExceptions`, finds the row by id) labelled "Cancelled by a range: start – end" with **Edit range** (opens `dialogs/class_range_dates_dialog.dart`'s plain date picker, warns via `ConfirmationModal` — copy + the widen check shared from `data/range_exception_helpers.dart` — only when the move WIDENS coverage, then dispatches `ScheduleRangeExceptionUpdated`) and **Remove range cancellation** (`ConfirmationModal` → `ScheduleRangeExceptionDeleted`) — both run through the occurrence screen's OWN existing mutation lifecycle (a new `_Action.editRange` / `.removeRangeCancellation`) since either can flip whether this occurrence is still cancelled at all, so success ends the same way as "Cancel this class": a dialog, then pop back to the reloaded board. And — for any non-cancelled occurrence (a FUTURE one has no attendance yet, so it shows the Reserved list only) — the searchable, scrollable, **two-tab** roster (`widgets/form/class_attendee_roster.dart`, a `FutureBuilder` over its own `ScheduleRepository.listAttendees` → `GET /api/v1/checkin/attendees`, now returning everyone **signed up OR attended**, each flagged): a `ViewSwitcher` (`lib/shared/widgets/view_switcher.dart`) toggles between an **Attended** tab (`Attendee.attended`) and a **Reserved** tab (`Attendee.signedUp`) — **Attended first** — but the Attended tab (and the switcher itself) only exists once the occurrence has ≥1 attended entry; with none yet, the roster skips straight to the Reserved list (its own count standing in for the switcher). Once shown, each label carries its live count — "Attended (M)" / "Reserved (N)" — and the view defaults to Attended, then stays on whatever tab the staff member picked across a refetch (an action on one row never snaps the view back to the default). A Reserved row whose member has also attended gets a green check + "attended" caption (via `MemberRowTile.subtitle`) so a no-show reads differently from someone who showed — the Attended tab needs no such mark since every row there is attended by definition. A name search (`AppSearchBox`), scoped to the active tab, filters a height-bounded scrolling list, "No attendees yet" when the roster itself is empty. A member can appear on both tabs — which tab a row is showing on, not the member's own `Attendee.attended` flag, decides what its remove (×) action does: a **Reserved** row's removal cancels the reservation — confirm via `ConfirmationModal` ("Cancel reservation?") → `ScheduleRepository.cancelSignup` (`DELETE /api/v1/signup?member_id=&gym_id=&class_id=&occurrence_date=&occurrence_time=`) — an **Attended** row's removal reverses the check-in instead — confirm ("Remove attendee?", whose copy states the check-in is reversed and the awarded points removed) → `ScheduleRepository.removeAttendee` (`DELETE /api/v1/checkin?member_id=&gym_id=&class_id=&occurrence_date=&occurrence_time=`, mirroring `listAttendees`' date/time formatting); either path, on success the roster's own `_future` is reassigned via `setState` (re-fetching `listAttendees` so both tabs update) and a "Reservation cancelled" / "Removed from class" `SnackBar` confirms; on failure an error `SnackBar` — never a silent dismiss. Both the roster row and the batch picker's row (below) render through the shared `lib/shared/widgets/member_row_tile.dart` (`MemberRowTile`: an `InstructorAvatar` photo-or-initials avatar + name + an optional `trailing` slot, pure presentational — its optional `subtitle` slot renders the roster's green-check "attended" caption on a Reserved row whose member has also attended) so a member looks identical whether you're viewing the roster or picking from a batch picker — the roster's trailing is the remove button, `BatchCheckInMemberTile` (used inside `BatchCheckInPicker` via the shared `lib/shared/widgets/paginated_member_picker.dart`) is a thin wrapper whose trailing is the selection `Checkbox` with the whole row tappable to toggle; `BatchCheckInPicker` itself is shared by BOTH the "Update attendees" and "Reserve members" dialogs (a `description` param carries each flow's own copy — reused rather than duplicated, since the paginated member-list plumbing is identical). A cancelled occurrence collapses to just `ClassOccurrenceActions`' note (the details block — view or edit — and roster don't render); the **series editor's** "Cancel a date range" secondary action opens `class_range_cancel_dialog.dart` which `POST`s `/api/v1/classes/{class_id}/exceptions/range` with `is_cancelled:true` as `ScheduleRangeCancelled` (request models `class_instance_exception_request.dart` / `class_range_exception_request.dart`, responses unparsed; the range cancel dialog shares the processing/success terminal views in `dialogs/schedule_cancel_views.dart`, whose `ScheduleCancelSuccess` body both the series editor and the occurrence screen also reuse inside their own save-success dialogs). Below that action, edit mode also shows `widgets/form/class_cancelled_ranges_section.dart`'s **"Cancelled ranges"** section (hidden while empty) — every CANCEL range exception on the class (`GET /api/v1/classes/{class_id}/exceptions/range`, ALL of them, newest-created first, no window filter — filtered client-side here to `end_date >= today`) as a `start – end` row with Edit / Remove (`widgets/form/class_cancelled_range_row.dart`); a self-contained side read like the roster, but its mutations DO dispatch through the shared `ScheduleBloc` (`ScheduleRangeExceptionUpdated` → `PUT .../exceptions/range/{exception_id}` moves the dates, atomically re-running the create path's teardown when the range is a cancel; `ScheduleRangeExceptionDeleted` → `DELETE .../exceptions/range/{exception_id}`) so the board reloads consistently with every other schedule mutation, while the section itself refetches its own list on ANY bloc mutation success and confirms via a per-row spinner + SnackBar (never a full success dialog — editing one range doesn't invalidate the rest of the form the way it does the occurrence screen). The **"Update attendees"** action (opened from the occurrence screen) opens `presentation/dialogs/check_in/class_batch_check_in_dialog.dart` — a paginated member multi-select that `POST`s the staff batch check-in `/api/v1/checkin/batch` (the occurrence is addressed by `class_id` + `occurrence_date` + `occurrence_time` + `is_member:false` in the BODY now — the path no longer carries them; returns **207 Multi-Status**; Dio does NOT throw on a 207, so the per-member breakdown is read off the success path) as `ScheduleBatchCheckInRequested` on a DEDICATED `isCheckingIn` / `batchCheckInResult` / `checkInError` channel on `ScheduleLoaded` (kept off `isMutating` so the dialog owns its own processing → results step), reloading the board on success so attendance counts update. Check-in is **warn-first with an override**: a clean member records, but one the gate warns on (no membership / out of classes / ineligible plan / over capacity) comes back as the batch item status `needs_confirmation` — **nothing written** for that member, its `warnings` say why — the results step shows ✓ checked-in / already-in / needs-confirmation (⚠, `okYellow`) / ✗ failed per member, and, when any member needs confirmation, an **"Check in the remaining N anyway"** `AppOutlineButton` that resubmits just that subset as a fresh `ScheduleBatchCheckInRequested` with `ignoreWarnings: true` — the bloc's `_onBatchCheckIn` keeps the channel's prior `batchCheckInResult` alive across the retry (rather than clearing it) and merges the retry's (partial) response back into the full breakdown via `BatchCheckInResponse.mergeConfirmed` (replacing only the resubmitted members' rows), so already-resolved rows never disappear; a retry's transport failure surfaces inline on the results step (not a reversion to the picker) so the rendered breakdown is never lost. The shared check-in models + processing/actions views live under `lib/features/check_in/`. The **"Reserve members"** action (opened from the occurrence screen) opens `presentation/dialogs/signup/class_signup_dialog.dart` — the same `BatchCheckInPicker` reused with its own copy, then submitted via `ScheduleSignUpRequested` on its own DEDICATED `isSigningUp` / `signupResult` channel on `ScheduleLoaded`. **There is no batch sign-up endpoint** — `ScheduleBloc._onSignUp` LOOPS `POST /api/v1/signup` once per picked member, isolating each in its own try/catch (a CRM-side aggregate, `SignupBatchResponse` / `SignupBatchResultItem` in `features/schedule/data/models/signup_batch_result.dart` — not a backend-mirrored model) so one member's failure (typically `"Class is full"` once capacity is hit) never stops the rest from being attempted; the results step (`signup/signup_results_view.dart` + `signup_result_row.dart`) shows ✓ reserved / ✓ already reserved (no change) / ✗ failed (the reason) per member — no "confirm anyway" retry here, unlike check-in's warnings, since a reservation either succeeds or the room is full. A successful run reloads the board on the same best-effort-after-commit pattern as batch check-in, so the board's "N reserved" chip updates behind the still-open results step. Every write — series edit, occurrence override, instance/range cancel, batch check-in, sign-up — runs through a `_mutateAndReload` (or the dedicated check-in / sign-up channels) that toggles `isMutating` / `actionError` / `actionSuccessCount` on `ScheduleLoaded` and reloads the board on success — so a cancelled day shows its "Cancelled" badge — mirroring `PlansBloc`. **The chooser dialog, both screens, the batch check-in dialog, the sign-up dialog, and the range-cancel dialog are pushed/shown with `BlocProvider.value` so they share the board's bloc** — a save/cancel/override/check-in/sign-up reloads the board the user returns to (no caller-await refresh needed); each rides the shared bloc's `actionSuccessCount` (or, for check-in / sign-up, its own dedicated channel) for its terminal state. **Both the series editor and the occurrence screen surface a committed mutation as a success DIALOG** (`AppDialog` reusing the `ScheduleCancelSuccess` body), then pop back to the board — not a full-screen success step. The series editor still blanks to the shared `CenteredProcessingView` while processing; the **occurrence screen instead keeps its content on screen** and overlays a dimmed scrim + spinner (`widgets/occurrence/occurrence_mutation_overlay.dart`'s `OccurrenceMutationOverlay` — a `Positioned.fill` sibling of the content in a `Stack`, mirroring `member_detail_screen.dart`'s `_MutationOverlay`) so the success dialog appears **over the real "This day's details" / actions / roster**, not a blank spinner page; failure (inline error + retry) still renders in-screen on both screens. There is no GET-employees endpoint, so the instructor pickers (per-day on the series editor, single-day on the occurrence screen) are sourced from the real (id, name) pairs already resolved on the gym's classes (`InstructorOption.fromClasses`), not a mock roster; gym-id-scoped via `selectedGym.gymId`) — is built on `flutter_bloc` + `equatable`. Events describe user/system actions; states represent UI state; repositories sit behind the bloc. This is the standard for all new CRM features.
  - **Five documented exceptions** on the member-detail page use a small **read-only side read** against a repository — the fetch lives in the widget, not as a new event/state on `MemberDetailBloc` — kept isolated so they don't bloat the bloc (a section may still *react* to the bloc's shared `refreshToken` to know when to re-fetch; its data never becomes bloc state) — the same shape the edge reads use: (1) the **Waivers section** (`features/member_details/.../sections/member_waivers_section.dart`) over `MembershipsRepository.listMemberWaiverStatus`, which lists only the waivers the member must sign for the memberships they currently hold (the backend `signatures/by-member` query is scoped to the member's non-terminal memberships' plan `waiver_ids`); (2) the **Invoices section** (`features/member_details/.../sections/invoices_section.dart`) over `MemberRepository.listMemberInvoices` + `getUpcomingInvoice`, showing the one overdue-or-upcoming invoice; and (3) the **Payment History section** (`features/member_details/.../sections/payment_history_section.dart`) over `MemberRepository.getPayments` — a **paginated** read (`?limit&offset`, "Show more"), deliberately a separate request from `/billing` so the potentially long charge history isn't bundled into the member-detail payload. It is **grouped one row per invoice** (every charge against an invoice — each retry, the success, and any refunds — folds into that single row, which carries the invoice total as `amount` and the summed `refunded_amount`, with all charges under `attempts`; the query picks a representative succeeded charge for the row's status/payer/charge_id) and surfaces an invoice by **three** paths: the member **paid** it (`member_invoices.paid_by_member_id`), a **membership** they have ever held was on it (matched by `member_memberships.item_id` on the invoice line items, so pre-link invoices stay findable), or it was **for** them (their id is in the invoice's `paid_for` beneficiary list). So a parent paying for this member's class pack shows on **both** pages, a paying parent's UNRELATED invoices never leak here, and each row is labelled with the **payer** (the "Paid by" column = `paid_by_member_id`) while the invoice popup adds who it was **"For"** (`paid_for`). (Billing rows store payer + beneficiaries explicitly — `BillingPaymentRecord.paid_by_*` + `paid_for` — there is no single conflated `member_id`.) And (4) the **one-time Refunded row** (`features/member_details/.../sections/membership_details_loader.dart`, `MembershipDetailsLoader`), which sums `MemberRepository.getPayments`' `refundedAmount` across the charges whose line items carry a one-time / trial membership's `item_id` and shows a "Refunded $X" row on that membership card only once it is partially/fully refunded. And (5) the **Class history section** (`features/member_details/.../sections/class_history_section.dart`) over the checkin domain's `ScheduleRepository.getMemberClassHistory` (`GET /api/v1/checkin/history?member_id=&gym_id=&limit=&offset=`) — sits side by side with Payment History (equal-width row on desktop, stacked below the tablet breakpoint, its own `LayoutBuilder` in `member_detail_grid.dart`'s `_HistoryRow`, mirroring `_Grid`'s branch). Two labelled blocks in one card: **Upcoming** (open reservations, soonest first, always complete — unpaginated) and **History** (attended + no-show rows, newest first, **paginated** — `?limit&offset`, "Show more"). Each row shows the class name, a `classDateTimeRangeLabel` start–end + date "when" caption (the row carries a `durationMinutes` from the backend for the range), and a status pill (`InvoiceChip`, reused from the Waivers/Discounts sections — `attended` → `good`/green, `no_show` → `bad`/red, `reserved` → `brand`/the primary blue, `unknown` → `neutral`). Both this card and the Payment History card are a **fixed `DesignConstants.historyCardHeight`** with their title pinned and their rows scrolling internally (a `Scrollbar` over a `SingleChildScrollView`, "Show more" reachable at the bottom of the scroll), so the side-by-side row stays equal-height and compact instead of each card growing unbounded with its list. The **Invoices**, **Payment History**, **Refunded-row**, and **Class History** side reads all re-fetch when the bloc's `refreshToken` bumps — on every billing change and on each tick of the **post-charge invoice poll** (`InvoicePoller` → 5/10/15/30/60s after a charge / start / refund / mark-paid-cash, restarted on a new charge so only one sequence runs), so a webhook-delivered invoice appears without a manual reload (Payment History and Class History both reset to page 1, discarding any superseded in-flight page fetch). **The Invoices section also dispatches a real `MarkPaidCashRequested` mutation to `MemberDetailBloc`** for OPEN/overdue invoices (via `MarkPaidCashDialog`) — upcoming invoices show a "cash available once it opens on \<date\>" note and no action. The profile header's **Check In / Reserve** action opens `presentation/dialogs/check_in/member_class_check_in_dialog.dart` — one dialog covering BOTH a real (possibly retroactive) attendance record and a reservation, for the single viewed member, as a **two-page, class-identity-first flow**: a top-level `ViewSwitcher` (**Check in** | **Reserve**), each view owning its own in-dialog step via a plain state enum on the dialog's `State` (`_CheckInStep` / `_ReserveStep`) — the same house pattern as `charge_card_dialog`'s `_ChargeStep`, Back vs Cancel on the footer's secondary button depending on step depth. The data load is ONE fetch shared by both views: `member_check_in_pick_body.dart` is now a pure loader (no navigation of its own) that loads every effective occurrence in one `[today − 30d, today + 14d]` window via `ScheduleRepository.listEffectiveInstances`, drops cancelled days, splits CLIENT-SIDE by each occurrence's backend-computed UTC start instant (`occurredAt` — never a browser-local rebuild of the gym-local `classDate` + `resolvedClassTime`) / end (`+ resolvedDurationMinutes`) against `DateTime.now()` into **checkIn** (in session or starting within the next 2h — `_kCheckInOpensHours = 2`, mirroring the backend `checkin_opens_hours_before_start` and `ClassOccurrenceScreen`'s own constant), **past** (already ended, most-recent-first), and **reserve** (not yet started, soonest-first), then hands all three lists to a `builder` callback. Reserve **intentionally overlaps** Check-in for an occurrence starting within the next 2h (pick Check-in to record attendance now, or Reserve to hold a spot ahead of time) — only a class already in session is Check-in-only.

**Check in view**: step 1 is an occurrence-level **card grid** (`check_in_occurrence_card_grid.dart`'s `CheckInOccurrenceCardGrid`) over the **checkIn** list — the same image `ClassCard`s (`large`, 16:9, graceful no-image fallback) and `FillGrid` layout as the Reserve picker so the two views feel identical, but one card **per occurrence**, NOT per class identity (a class with two in-window slots shows two cards), each captioned with the occurrence's **effective** date + start–end time range (`classDateTimeRangeLabel`); tapping a card **directly selects** that occurrence for check-in (no drill-in — the front-desk one-tap flow), highlighted via `ClassCard.selected` (a tokened primary border + card-backed check badge overlay + `primaryColor10` wash, mirroring `CheckInInstanceTile`'s selected look; overlay-drawn so selection never shifts layout) keyed by the same `CheckInSection.keyFor` composite — plus, only once there's ≥1 past occurrence, an outline **"Check into a past class"** button. That opens step 2, an identity-level picker (`CheckInClassPicker`): one image `ClassCard` per distinct class — `check_in_class_group.dart`'s `groupInstancesByClass` groups the already-most-recent-first **past** list, so each card's "Last: …" caption is that class's most recent occurrence — reusing the schedule board's own `ClassCard` so a class reads the same here as on the board. Tapping a card opens step 3: that class's past occurrences (`CheckInSection`, title = the class name, most-recent-first) for the retroactive pick.

**Reserve view**: step 1 is the same `CheckInClassPicker`, grouping the soonest-first **reserve** list ("Next: …" captions); step 2 is the picked class's upcoming occurrences (`CheckInSection`, title = the class name, soonest-first).

Each view tracks its OWN picked occurrence (`_checkInSelected` / `_reserveSelected` on the dialog state, independent of which class-identity card was drilled into) so switching the top-level view never carries a stale pick into the other view's mutation — the `CheckInReserveSelection` (instance + `CheckInReserveAction`) submitted is derived from whichever view is active. `CheckInSection.keyFor(action, instance)` still composite-keys the tile highlight. This was a PICK-FLOW redesign only — the terminal mutation/result wiring below is untouched.

**Check in** `POST`s `/api/v1/checkin` (`is_member:false`) via `MemberCheckInRequested` on a dedicated `isCheckingIn` / `checkInResult` / `checkInError` channel on `MemberDetailLoaded` (mirrors the charge-card channel). Check-in is **warn-first with an override**: a clean check-in records — bumping `refreshToken`, the success step names the class + "+N points" and surfaces any gate `warnings` as a non-blocking note — but one that hits a gate warning (no membership / out of classes / ineligible plan / over capacity) comes back with `CheckInResponse.requiresConfirmation` true and **nothing written** (`logId` null, no `refreshToken` bump); the result step then shows the warnings (⚠, `okYellow`) and a **"Check in anyway"** action that re-dispatches the identical `MemberCheckInRequested` with `ignoreWarnings: true` to record it through the gate.

**Reserve** reuses the schedule feature's sign-up wiring for this one member/occurrence — `MemberReserveRequested` calls `ScheduleRepository.signUp` directly (`MemberDetailBloc` now takes a `ScheduleRepository` dependency alongside `MemberRepository` for this) on its own dedicated `isReserving` / `reserveResult` / `reserveError` channel. A reservation doesn't touch points/attendance/billing, so unlike check-in it does **not** refresh member detail or bump `refreshToken`. A clean reserve shows "Reserved for …"; a repeat is idempotent (`SignupResponse.alreadySignedUp`, "Already reserved"); a full occurrence surfaces the backend's `"Class is full"` error. There is **no** "confirm anyway" override for a reserve failure — mirrors the schedule feature's own "Reserve members" dialog (a reservation either succeeds or the room is full). `member_check_in_result_view.dart` renders both outcomes, branched on the terminal `CheckInReserveAction`.

The member-detail screen provides a `ScheduleRepository` in its scope for both the pick body's reads and Reserve's mutation (cross-feature reuse of the schedule repo; no schedule bloc). Only the **Waivers** "Sign waiver" affordance still opens the `ComingSoonDialog` placeholder.
...
- **Public browser (`gymId` null):** unauthenticated, slug-keyed template **feed** path via `package:http` (`VideoApiClient`). There is no per-template **detail** fetch anymore (see the "Gym detail" bullet below) — `selectedGym.detail` stays null here. Supabase is not initialized in this target — never call `ApiClient` on this branch.
...
- **Feed (template/public path):** `GET /api/v1/presets/templates/{videoGymId}/videos` (video-gym-id-keyed, paginated). The one query knob beyond paging/genre is `?rejected=true`, which serves the scan's **rejected** list (backing the videos tab's rejected-videos section). The "Show rejected" toggle is visible in both admin and public contexts.
- **Gym detail is admin-only.** `selectedGym.detail` (classes / rewards / spec) populates only in the admin context, via the `GET /api/v1/gyms/{gymId}/showcase` fetch above. There is no template/public-path detail fetch — the standalone theme-browser build target only ever mounts the Theme tab (see *Standalone theme browser*), and its phone preview draws demo content from the category-keyed showcase-defaults instead (see *ThemeService integration*). `GymDetail.fromJson` still tolerates both `video_gym_id`/`gym_id` and `specification`/`spec` keys for the admin showcase response.
- **`video_gym` id ≠ regular gym id — and there is no mapping. `selectedGym` carries BOTH, named separately.** The template catalog keys on a string `video_gym` id (`boxing`, `acro_yoga`, …); the FastApiBackend gym (`GET /api/v1/gyms/`, `GymWithRole`) keys on a **UUID**. They are **separate id spaces**, so `selectedGym` exposes two getters: **`selectedGym.gymId`** = the real gym UUID (set by `setActiveGym` at login/pick; scopes every CRM member query — members list, member-detail roster, plans, discounts) and **`selectedGym.videoGymId`** = the template content key (drives the read-only preview surfaces: feed, loyalty store, content focus, and — only as a fallback category key before any theme is picked — the phone preview; see *ThemeService integration*). (The dashboard Upcoming Classes card no longer reads this — it now reads the real `/classes/instances` schedule feed, scoped by the real `gymId`.) Never pass the real `gymId` to a video template endpoint — it 404s; never scope a member query by `videoGymId`. After login the auth gate calls `setActiveGym` (real gym) **and** seeds `videoGymId` with a **default** `video_gym` (`AppConstants.defaultVideoGymId`, `--dart-define=DEFAULT_VIDEO_GYM`, default `boxing`) for content; the **Gym presets** Settings section updates `videoGymId` after a successful import via `selectedGym.setVideoGymId(videoGymId: ...)`. **The Theme tab no longer touches `videoGymId` at all** — theme selection is fully decoupled from the content gym (see *ThemeService integration*). If a real gym→video_gym link is ever needed, it's a new schema field + endpoint + setup UI, not a reuse of the UUID.
- **Where it lives:** `lib/features/members/data/` — `video_api_client.dart` (template feed + preview), `gym_content_repository.dart` (authed real-gym showcase + videos; methods: `fetchVideos(owner, rejected)`, `fetchVideoPreview(rejected)`, `addVideo`, `lookupVideo`, `removeVideo(owner, reason)`, `keepVideo`), `youtube_url.dart` (`extractYoutubeId`, mirrors the backend extractor, used to validate the add input + derive `Video.videoId`); models in `video_feed.dart` / `gym_detail.dart`. (The Theme tab's own data files — `gym_theme_repository.dart`, `showcase_defaults.dart` — live in this same directory; see *ThemeService integration*.) The editable "Your videos" UI is `videos_tab/your_videos_feed.dart` (the stateful preview/full feed with add/remove), `your_video_tile.dart` (`feedVideoTile` + `openVideoInNewTab`), and `add_video_dialog.dart` (paste-a-link). The `StatefulWidget` + `FutureBuilder`/`setState` pattern is the right shape for the widgets that consume these reads/edits (`member_feed_section.dart`, `your_videos_feed.dart`).
...
- **Failure behavior:** the feed calls time out after **30s**; all degrade quietly (empty feed / error state) so the rest of the app stays up if the service is down.
- **`package:http` is whitelisted for public, unauthenticated endpoint reads only** — the video-template feed (`VideoApiClient`) here, plus the public showcase-defaults fetch (`ShowcaseDefaultsClient`, see *ThemeService integration*). Do not reuse `package:http` to wire other (authed) screens to the backend — CRM data goes through `ApiClient`. The admin video/showcase reads use `GymContentRepository` (ApiClient-backed). The ThemeService styles **catalog** itself does not use `http`; it rides `theme_flutter`'s own `dio` client (`StylesPager`).

## Video-agent screen

A conversational screen where the gym owner chats with an LLM agent to author the
gym's video-feed spec (keep/avoid criteria).

- **Entry:** the member-app preview's **Videos tab** — the Edit & Focus card
  (`features/members/.../videos_tab/edit_and_focus_card.dart`) pushes
  `AppRoutes.videoAgent` (`/members/app-preview/video-agent`, a sub-route of the
  preview section — not in `kAddressableRoutes`, so it keeps its parent's URL).
...
- **Screen:** `features/video_agent/presentation/screens/video_agent_screen.dart`.
  Uses `AppShell(activeRoute: AppRoutes.memberAppPreview)`. On open: calls `refineFromFeed`
...
The **Settings screen** includes a **Gym presets** section that lets an authorized admin apply a template (videos + classes + rewards + theme) to the current real gym in one step.
...
- **Architecture:** a full Bloc feature (`features/presets/`) — `PresetsBloc` + `PresetsRepository`. The repository wraps `ApiClient` for both the catalog browse (`GET /api/v1/presets/templates`) and the import (`POST /api/v1/gyms/{gym_id}/presets/import`). The section lives in `features/settings/presentation/sections/gym_presets_section.dart` and is composed of three sub-widgets (`gym_presets_section.dart` → `gym_presets_content.dart` → `gym_presets_template_card.dart`), each ≤150 lines.
- **Import flow:** the admin picks a template card → taps "Apply preset" → `PresetsBloc` POSTs the import → on success calls `selectedGym.setVideoGymId(videoGymId: videoGymId)` so the member-app preview immediately reflects the imported gym's content. Theme is intentionally **not** touched by this call (see *ThemeService integration*) — the import still writes `gyms.theme_design_id` server-side, but applying it live is the Theme tab's job. The `PresetsImportResponse` carries `videos_imported`, `classes_imported`, `rewards_imported`, and `theme_design_id`.
...
## ThemeService integration (live theme preview + FastApiBackend persistence)

The member-app **Theme tab** (`member_app_screen.dart` → `LiveThemePreviewTab`) is a **live theme preview**: a phone frame renders branded member-app showcase screens that re-theme the instant the admin picks a theme. Second deliberate edge integration — the picker + branding are still read-only against ThemeService; persisting the *choice* is an ordinary FastApiBackend call, not part of the ThemeService edge.

- **How it works:** depends on the shared **`theme_flutter`** package (path dep, `../ThemeService/ThemeFlutter`) for the customization **runtime + resolvers**, and for the picker itself. Both `ThemeGrid` (the side grid) and `LibraryView` (the full-screen library) page `theme_flutter`'s own **`StylesPager`** directly against `GET /apps/{appId}/styles` — there is no CRM-local pager and no presets/template catalog involved (the Settings → Gym Presets template catalog is a separate, unrelated surface — see *Gym presets Settings section*). The **showcase widgets live locally** in `lib/showcase/` (this app is their only consumer); they resolve branding through `theme_flutter`'s `ThemeColor` / `ThemeImage` / `ThemeFont` / `ThemeText` / `ThemeIcon`. The tab calls `ThemeRuntime.initialize(...)` once (lazily, idempotent) and `selectDesign(id)` on tap.
- **Category filter pills read `ThemeStyle.category`** — a wire field ThemeService stamps on every styles card from its classification vocabulary (see `../ThemeService/README.md`). `LibraryView._chipsFor` builds its filter chips from the distinct `category` values in the loaded page; `LibraryCard` displays it uppercased.
- **Picking a theme is theme-only, decoupled from the content gym.** `selectedGym.selectStyle(style)` records only the previewed design id (`selectedGym.designId`) and its `themeCategory`, and re-brands the live preview (`ThemeRuntime.selectDesign`) — it never touches `videoGymId`, `detail`, or `displayName`. The gym's real content (classes/rewards/spec) is fetched independently, scoped by the real `gymId`.
- **Preview demo content:** the phone preview's demo class/reward cards come from the **public** `GET /api/v1/theme/showcase-defaults` (category-keyed; `showcase_defaults.dart`'s `ShowcaseDefaultsClient` + `loadShowcaseDefaults()` fetch it once and cache app-side), looked up by the picked style's `themeCategory` (falling back to the legacy `videoGymId`-based group when no category is known yet, e.g. before any theme has been picked). Priority order in `theme_preview_pane.dart`: the real gym's own `selectedGym.detail` (admin context, when present) → the fetched showcase-defaults → the bundled `kShowcase*ByGroup` constants (`lib/showcase/showcase_group_defaults.dart`) as the offline last resort if the fetch fails.
- **Persistence:** the Theme tab boots from the gym's saved theme — `GymWithRole.themeDesignId` is passed into `setActiveGym` at login/gym-pick and exposed as `selectedGym.savedThemeDesignId`. An admin-only **"Set as app theme"** button (`SetAppThemeButton`, beneath the phone preview, self-hidden in the public standalone browser) commits the currently-previewed design via `PUT /api/v1/gyms/{gymId}/theme` (`SetAppThemeBloc` + `GymThemeRepository`; **not optimistic** — mirrors the Settings gym-timezone save) and updates `selectedGym.savedThemeDesignId` only on success. The button renders a disabled "Current app theme" checkmark once the previewed design already matches the saved one.
- **Where it lives:** tab chrome (including `set_app_theme_button.dart`) in `lib/features/members/presentation/widgets/member_app/theme_tab/` + the shared `lib/shared/widgets/phone_frame.dart`; the data layer is `lib/features/members/data/gym_theme_repository.dart` (the `PUT`) and `lib/features/members/data/showcase_defaults.dart` (the public showcase-defaults client + cache); the bloc is `lib/features/members/bloc/set_app_theme_*.dart`. The showcase screens it renders are the self-contained `lib/showcase/` module (screens + router, slots, content, tokens, assets, and `celebrations/` / `home/` / `rewards/` / `support/` subfolders; bundled fallbacks in `assets/showcase/`). The catalog model is the engine's `ThemeStyle` (no CRM-side duplicate — consume the engine's, DRY).
- **Dependency:** the ThemeService read-only API must be running for the styles catalog + branding. Base URL defaults to `http://localhost:8001`; override with `--dart-define=CUST_BASE_URL=http://<host>:<port>`. The showcase-defaults fetch and the "Set as app theme" save instead talk to the FastApiBackend (`BACKEND_BASE_URL`, default `http://localhost:8000`), like every other backend call.
- **Failure behavior:** the engine's resolvers never throw — they fall back to bundled defaults. The catalog grid degrades quietly (error message, no crash) if the service is down; the phone still renders the fallback look. A showcase-defaults fetch failure degrades to the bundled constants (see above); a save failure surfaces an inline error via `SetAppThemeBloc` with no state lost.
...
- **The module is `LiveThemePreviewTab`** (`features/members/.../theme_tab/live_theme_preview_tab.dart`). The admin preview embeds it; the standalone target mounts it full-screen. It is self-contained (bootstraps `ThemeRuntime`, owns its selection state via the `selectedGym` global + `theme_flutter`'s `StylesPager`). Its one host-specific knob is `routePath` (the URL path the previewed theme mirrors onto as `?theme=…`): default `AppRoutes.memberAppPreview` (embedded), `AppRoutes.home` for the standalone.
...
- **Same edge integrations as the embedded tab — no VideoService dependency.** The standalone hits the ThemeService styles catalog (`:8001`, for the picker + branding) and the FastApiBackend's public `GET /api/v1/theme/showcase-defaults` (`:8000`, for the phone preview's demo content); both must be running locally. It never talks to VideoService — the old template-catalog gym-detail fetch this build target used to rely on is gone (see *ThemeService integration*).
...
- `../VideoService/` — the video data pipeline (scrape / scan). The CRM's video template endpoints are now served by the FastApiBackend (`/api/v1/presets/templates*`), not directly by VideoService. VideoService is still the source of video data but is no longer a runtime dependency of the CRM.
...
- `http` — **for public, unauthenticated endpoint reads only** (no Supabase session): the video-template feed (`VideoApiClient`, see *Video template integration*) and the public showcase-defaults endpoint (`ShowcaseDefaultsClient`, see *ThemeService integration*). Nothing else; CRM data goes through `ApiClient`.

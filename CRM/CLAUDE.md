...
- **CRM feature screens use Bloc.** Anything backed by the FastApiBackend — the **login / auth gate** (`features/login/`), **gym setup wizard** (`features/gym_setup/`), **members list** (`features/members_list/` → rendered by `features/members/.../members_screen.dart`), **member detail + its billing dialogs** (`features/member_details/`), and the dashboard's **Overdue Payments section** (`features/home/bloc/overdue_payments_*` → `features/home/.../overdue_payments/`, which reuses `MembersListRepository`) — is built on `flutter_bloc` + `equatable`. Events describe user/system actions; states represent UI state; repositories sit behind the bloc. This is the standard for all new CRM features.
- **Edge / non-CRM screens stay stateless.** The screens that only render content or drive the read-only edge integrations — the **member-app preview** (`features/members/.../member_app_screen.dart` + its tabs), the **theme browser**, and the kept showcase/demo surfaces (schedule, growth, employees, QR) — use `StatelessWidget` for pure display and `StatefulWidget` only for local UI state (tab index, scroll controller) or to host a `FutureBuilder` over an edge read. They do not need a bloc. The **dashboard (`features/home/`) is mixed**: its hero + Live Attendance cards are still mock and stateless, but the **Overdue Payments section — stacked in the left column under Live Attendance** — is the one live, bloc-backed CRM surface there.
...
- **All FastApiBackend calls go through `ApiClient`** (`lib/core/network/api_client.dart`). It wraps a single configured `dio` instance, attaches the **Supabase JWT** as a Bearer token on every request, applies a **30s timeout**, and on a **401 refreshes the Supabase session and retries once**; if the refresh fails it calls `ApiClient.onUnauthorized` (wired in `main.dart`) to sign the session out, dropping the auth gate back to the login screen. Never create a raw `Dio` or make bare HTTP calls for CRM data — use `ApiClient`.
...
## VideoService integration (read-only edge feed + gym detail)

The member-app **videos tab** pulls its feed live so the admin previews real thumbnails and titles instead of placeholder art; a second read fetches the selected gym's **detail** (classes / rewards / spec) once into memory. Both are **read-only** and scoped to the VideoService — they do not go through `ApiClient` and are not a license to wire other screens to a backend over `http`.

- **Feed:** `GET /gyms/{gymId}/videos` (gym-id-keyed, paginated; follows the `SelectedGym` global). The one query knob beyond paging/genre is `?rejected=true`, which serves the scan's **rejected** list (backing the videos tab's rejected-videos section). Read-only, no auth, no other feed endpoints. Widening this is a user decision, not a default.
- **Gym detail:** `GET /gyms/{gymId}` via `lib/features/members/data/gym_api_client.dart` (`GymApiClient`, `package:http`) fetches the gym's whole content detail (spec + classes + rewards) **once** into the `SelectedGym` global. It backs the loyalty rewards store, the videos content focus, the videos tab's "Your videos" row/grid (`videos_tab/your_video_tile.dart`, with bundled fallback), the phone preview, and the kept **Schedule** screen + dashboard **Upcoming Classes** card (read from memory — gym-driven, not placeholder, by design).
- **`video_gym` id ≠ regular gym id — and there is no mapping. `selectedGym` carries BOTH, named separately.** The VideoService keys on a string `video_gym` id (`boxing`, `acro_yoga`, …); the FastApiBackend gym (`GET /api/v1/gyms/`, `GymWithRole`) keys on a **UUID**. They are **separate id spaces**, so `selectedGym` exposes two getters: **`selectedGym.gymId`** = the real gym UUID (set by `setActiveGym` at login/pick; scopes every CRM member query — members list, member-detail roster, plans, discounts) and **`selectedGym.videoGymId`** = the VideoService content key (drives the read-only preview surfaces: feed, loyalty store, content focus, phone preview, Schedule screen, dashboard Upcoming Classes). Never pass the real `gymId` to a VideoService endpoint — it 404s and breaks every content surface; never scope a member query by `videoGymId`. After login the auth gate calls `setActiveGym` (real gym) **and** seeds `videoGymId` with a **default** `video_gym` (`AppConstants.defaultVideoGymId`, `--dart-define=DEFAULT_VIDEO_GYM`, default `boxing`) for content; the **Theme-tab gym picker** (real VideoService ids) overrides `videoGymId` only. If a real gym→video_gym link is ever needed, it's a new schema field + endpoint + setup UI, not a reuse of the UUID.
- **Where it lives:** `lib/features/members/data/` — `video_api_client.dart` (feed + `GET /gyms/{gymId}/videos/preview`), `gym_api_client.dart`, `gyms_pager.dart` (pages the gyms list for the theme/gym picker), all wrapping `package:http`; models in `video_feed.dart` / `gym_detail.dart`. The `StatefulWidget` + `FutureBuilder` pattern is the right shape for the widgets that consume these reads (`member_feed_section.dart`, `library_view.dart`).
- **Dependency:** the VideoService (sibling, see below) must be running. Base URL defaults to `http://localhost:8002`; override with `--dart-define=VIDEO_BASE_URL=http://<host>:<port>`.
- **Failure behavior:** the gyms list / gym-detail calls time out after **15s** (the `/gyms` list can take ~9s on the small App Runner instance), the feed after **30s**; all degrade quietly (empty feed / error state) so the rest of the app stays up if the service is down.
- **`package:http` is whitelisted for this integration only.** Do not reuse `VideoApiClient` or `package:http` to wire other screens to a backend — CRM data goes through `ApiClient`. (The ThemeService integration below does not use `http`; it rides `theme_flutter`'s own `dio` client.)
...
Skills for this app live in `.claude/skills/<name>/SKILL.md`.

- **`qa-crm`** — drive the live admin app in a headless browser and prove every page renders (debug build + dev auto-login + deep-link each page + screenshot). Use it whenever you QA / smoke-test the pages or touch routing, auth, the members list, or member detail.

**Always look for a skill before starting work.** Before any task, check `.claude/skills/` (and the sibling systems' skills when the work crosses into them) and **load the most relevant one(s)** — don't reinvent a workflow a skill already captures. When the user's request matches a skill, invoke it.

**Skills are living documents.** When working through a skill (or a reference doc / `SKILL.md` it loads) you realize its guidance is wrong, outdated, or holding the work back — a recommended source that returns bad results, a step that no longer fits, a better tool you've found — do not silently work around it. Use the better approach for the task, then **recommend the specific skill fix to the user and wait for approval** (per *No assumptions*); on approval, **update the skill file** so the lesson sticks. Likewise, when you build a new repeatable workflow worth keeping, add a skill for it. Skills are ever-evolving — every real-world correction should feed back into them.
...
- Global `ThemeData` lives in `lib/shared/themes/app_theme.dart` and is wired in `main.dart`. It maps DesignConstants into Material 3's `ColorScheme` and `TextTheme` so stock Material widgets auto-theme.
...
Every authenticated screen must offer a way to sign out. Authenticated screens wrap themselves in `AppShell`, whose left nav rail carries the sign-out path. Never ship an authenticated screen with no way out.
...
- `http` — **for the VideoService integration only** (see above). Backs `VideoApiClient` / `GymApiClient` / `GymsPager` and nothing else; CRM data goes through `ApiClient`.
...
- **dart-defines** carry the **edge integration** base URLs (compile-time `String.fromEnvironment`): `VIDEO_BASE_URL`, `CUST_BASE_URL`, plus the theme-browser link targets `LANDING_URL` / `PRICING_URL` / `BOOK_URL`. They default to `localhost` (dev) and are overridden at build time for prod (see *Production deployment*). `DEFAULT_VIDEO_GYM` (default `boxing`, in `AppConstants.defaultVideoGymId`) picks the `video_gym` seeded after login — see *VideoService integration*.
...
- **Build for prod with the edge base URLs** (they override the `localhost` defaults): `make build-web` → `flutter build web --release --base-href=/ --no-tree-shake-icons --dart-define=CUST_BASE_URL=https://theme.combatden.net --dart-define=VIDEO_BASE_URL=https://video.combatden.net`, followed by `python3 deploy/prune_web_fonts.py build/web`.

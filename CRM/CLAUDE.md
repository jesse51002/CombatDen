...
- `theme_flutter` (path dep, `../ThemeService/ThemeFlutter`) — **for the live theme preview only**. The shared white-label runtime + resolvers (showcase screens live locally in `lib/showcase/`). Transitively pulls `dio`, `flutter_svg`, `get_it` — **not** a license to wire `dio` into screens outside the theme preview.

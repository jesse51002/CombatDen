...
- **For multipart image uploads, `ApiClient` exposes `postMultipart`.** `ImageUploadRepository` (`lib/core/uploads/`) wraps it and returns a CDN URL. The **single** upload UI is the shared `ImageUploadPickerField` widget (`lib/shared/widgets/form/image_upload_picker_field.dart`) — opens the system file picker via `image_picker`, uploads through `ImageUploadRepository`, and calls back (`onUploaded`) with the CDN URL. It carries a `category` (the backend upload category) and an optional `defaultImageUrl` to preview a platform default (with a "choose your own" caption) before the user has uploaded one. There is **no** URL-paste variant — every image surface is a real CDN upload through this one widget; never build a parallel upload flow.

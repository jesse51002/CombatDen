...
**Check in** `POST`s `/api/v1/checkin` (`is_member:false`) via `MemberCheckInRequested` on a dedicated `isCheckingIn` / `checkInResult` / `checkInError` channel on `MemberDetailLoaded` (mirrors the charge-card channel). Check-in is **warn-first with an override**: a clean check-in records — bumping `refreshToken`, the success step names the class + "+N points" and surfaces any gate `warnings` as a non-blocking note — but one that hits a gate warning (no membership / out of classes / ineligible plan / over capacity / required waiver not signed) comes back with `CheckInResponse.requiresConfirmation` true and **nothing written** (`logId` null, no `refreshToken` bump); the result step then shows the warnings (⚠, `okYellow`) and a **"Check in anyway"** action that re-dispatches the identical `MemberCheckInRequested` with `ignoreWarnings: true` to record it through the gate.
...
The member-detail screen provides a `ScheduleRepository` in its scope for both the pick body's reads and Reserve's mutation (cross-feature reuse of the schedule repo; no schedule bloc). The **Waivers** "Sign waiver" affordance opens `SignWaiverDialog` (`lib/shared/widgets/sign_waiver_dialog.dart`, over the shared `SignWaiverPanel`) — opens with a prominent "Signing for <member>" banner (avatar + name + a 'signed by the member or their parent / legal guardian' note) and renders the waiver body read-only with its `{{placeholders}}` pre-filled client-side for display (`lib/core/utils/waiver_render.dart`, a display-only mirror of the backend render: member/gym/date values plus the signer name filled LIVE as it is typed (a `___` blank while empty); the backend's stored `rendered_body` stays authoritative), shows the ESIGN/UETA disclosure, captures a typed name + consent, and records a signature via `POST /api/v1/waivers/{id}/signatures` (echoing the displayed `waiver_version_id`; 409 stale-version → re-open). The two authorize-payer link dialogs pre-render the same way (payer = `member_name`, payee = `payee_name`). `WaiverResponse.waiverType` (`payer_auth` | `custom`, resilient-parsed with an `unknown` fallback) drives the special-casing: the plan form's waiver selector offers ONLY `custom` waivers (the backend also rejects non-custom `waiver_ids` at plan write), and the payer-auth agreement is badged "Payer agreement" in the Waivers tab + editor with its Delete and Member-Sign actions hidden (body editing stays). The membership-purchase wizard has a `signWaivers` step that blocks payment until every required waiver is signed (with the backend 422 gate as the backstop).
- **Edge / non-CRM screens stay stateless.** The screens that only render content or drive the read-only edge integrations — the **member-app preview** (`features/members/.../member_app_screen.dart` + its tabs), the **theme browser**, and the kept showcase/demo surfaces (growth, the **employees** surface — now the Employees tab of the People screen, `features/employees/.../employees_list_body.dart`, still stateless/mock — and the Settings screen's **QR codes** section) — use `StatelessWidget` for pure display and `StatefulWidget` only for local UI state (tab index, scroll controller) or to host a `FutureBuilder` over an edge read. They do not need a bloc. The **dashboard (`features/home/`) is mixed**: its hero card is still mock and stateless, but the **Live Attendance card** (left column, top), the **Overdue Payments section** (left column, under it), **and the Upcoming Classes card** (right column) are the live, bloc-backed CRM surfaces there. Upcoming Classes reads the **real** schedule feed — `features/home/bloc/upcoming_classes_*` over `ScheduleRepository.listEffectiveInstances` (the same `GET /api/v1/classes/instances` the board uses), self-contained `RepositoryProvider` + `BlocProvider` like Overdue; its request window spans the **recent past (~7 days) through the next ~14 days** — the past part isn't displayed, it's there only so the backend `/classes/instances` read **materializes** any already-ended-but-unrecorded occurrence on dashboard open (the same materialize-on-read the schedule board triggers, since the dashboard is the landing page). The `upcoming_classes_generator.dart` (`upcomingClassesFromInstances`) then drops cancelled occurrences, keeps those **still in session or upcoming** (END time = `occurred_at` + `resolvedDurationMinutes` is after now, so a class currently running is included — not just strictly-future ones), sorts by `occurred_at`, caps the teaser, and day-groups them. **Live Attendance** (`features/home/bloc/live_attendance_*` → `features/home/.../live_attendance_card/`) shows the IN-SESSION occurrence(s)' combined signed-up ∪ attended roster: two reads per load over the same `ScheduleRepository` — `listEffectiveInstances` over `[today − 1d, today + 14d]`, split by the backend-computed UTC `occurredAt` against `DateTime.now()` (in session = started and not yet ended via `resolvedDurationMinutes`), then `GET /api/v1/checkin/attendees` per shown occurrence (parallel, each isolated — one failed roster read flags just its section with a couldn't-load note instead of failing the card) — rows flagged green **Checked In** (attended) / red **Not Here** (reserved only), each deep-linking to `/members/detail/<id>`. When nothing is live the card **falls forward to the soonest upcoming occurrence's reservations** (blue "Reserved" pills instead of "Not Here"; every occurrence sharing that exact start instant shows, and overlapping live classes each get their own labelled section). A **60s poll** dispatches a silent refresh (a failed tick keeps the last data and later self-heals an error state; overlapping fetches are dropped/sequence-guarded so a slow stale snapshot never lands after fresher data). The card also hosts its own **loaded `ScheduleBloc`** (initial week from the shared `currentWeekStart()` in `features/schedule/data/schedule_week.dart`, init'd with `ScheduleInitRequested.loadBoard: false` so only the class catalog is fetched — the week board is never rendered here; the 60s tick also re-inits it out of a stuck `ScheduleError` so the footer self-heals instead of staying silently disabled) so its pinned footer runs REAL actions on the first shown occurrence, sharing that bloc via `BlocProvider.value` exactly as the board does: **"Check In Member"** opens the batch check-in dialog (enabled only inside the 2h check-in window), **"View all"** pushes the occurrence screen; both refresh the card on return.

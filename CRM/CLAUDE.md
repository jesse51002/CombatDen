...
- **For multipart image uploads, `ApiClient` exposes `postMultipart`.** `ImageUploadRepository` (`lib/core/uploads/`) wraps it and returns a CDN URL (`cdn.combatden.net/...?v=<hash>`). The upload UI is the shared `ImageUploadPickerField` widget (`lib/shared/widgets/form/image_upload_picker_field.dart`) — opens the system file picker via `image_picker`, uploads through `ImageUploadRepository`, and calls back with the CDN URL. Used for reward catalog images (`reward_form_dialog.dart` in `features/rewards/`) and member photos (`edit_member_dialog.dart` in `features/member_details/`). This is a **separate widget** from `ImageUploadField` (`lib/shared/widgets/form/image_upload_field.dart`) — the class form's paste-a-URL field (opens `ImageUrlDialog`, bubbles the URL up via `onChanged`; no upload, no `ApiClient` call). Don't conflate the two: `ImageUploadField` = URL paste, `ImageUploadPickerField` = real CDN upload.
...
- `image_picker` — **image upload flows only** (reward catalog images in `features/rewards/` and member photos in the edit-member dialog in `features/member_details/`). Used via `ImageUploadPickerField` — do not call `ImagePicker` directly at a call site.

...
- `qr_flutter` — **the kiosk "Get the CombatDen App" modal only** (`features/kiosk/presentation/widgets/kiosk_get_app_modal.dart`): renders a real, scannable QR encoding the per-gym app-download page URL (`kKioskAppDownloadBaseUrl` + the gym id). The kiosk home QR *placeholder* is still a static glyph (the live per-scan check-in code is Phase G) — this is the only real QR in the app. Scoped to the get-app modal; don't reuse it elsewhere.

...
- **For multipart image uploads, `ApiClient` exposes `postMultipart`.** `ImageUploadRepository` (`lib/core/uploads/`) wraps it and returns a CDN URL (`cdn.combatden.net/...?v=<hash>`). The **single** upload UI is the shared `ImageUploadPickerField` widget (`lib/shared/widgets/form/image_upload_picker_field.dart`) — opens the system file picker via `image_picker`, uploads through `ImageUploadRepository`, and calls back (`onUploaded`) with the CDN URL. It carries a `category` (the backend upload category) with three values: `reward` (reward catalog images, `reward_form_dialog.dart` in `features/rewards/`), `member` (member photos, `edit_member_dialog.dart` in `features/member_details/`), and `class` (class images, `class_details_section.dart` in `features/schedule/`). An optional `defaultImageUrl` previews a platform default (with a "choose your own" caption) when the user hasn't uploaded one yet — the class form uses this to show `AppConstants.defaultClassImageUrl`; the default is preview-only and never bubbled up. There is **no** URL-paste variant — every image surface is a real CDN upload through this one widget.
...
- `image_picker` — **image upload flows only** (reward catalog images in `features/rewards/`, member photos in the edit-member dialog in `features/member_details/`, and class images in `features/schedule/`). Used via `ImageUploadPickerField` — do not call `ImagePicker` directly at a call site.

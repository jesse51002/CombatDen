...
- **One feature is a shared RULEBOOK rather than a screen.** `features/membership_flow/` owns no UI: its `domain/` holds the pure functions and plain data both membership-purchase surfaces — the kiosk signup (`features/kiosk/`) and the staff start-memberships wizard (`features/member_details/.../start_memberships/`) — import so a rule, a price string or a plan word cannot be changed on one surface only. Nothing in `domain/` may hold a widget, a cubit, async I/O or a repository call; state and I/O stay in the surface that owns them. The kiosk's structural no-discounts rule survives this: `domain/discount_math.dart` is staff-only and the kiosk must never import it (`test/features/kiosk/kiosk_forbidden_imports_test.dart` is the enforcement).

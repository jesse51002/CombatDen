...
- **CRM feature screens use Bloc.** Anything backed by the FastApiBackend — the **login / auth gate** (`features/login/`), **gym setup wizard** (`features/gym_setup/`), **members list** (`features/members_list/` → rendered by `features/members/.../members_screen.dart`), **member detail + its billing dialogs** (`features/member_details/`), the **Memberships catalog** (`features/memberships/` — the Memberships / Discounts / Waivers screen; one `MembershipsRepository` behind a `PlansBloc`, `DiscountsBloc`, `WaiversBloc`, and the per-waiver `WaiverRosterBloc`), and the dashboard's **Overdue Payments section** (`features/home/bloc/overdue_payments_*` → `features/home/.../overdue_payments/`, which reuses `MembersListRepository`) — is built on `flutter_bloc` + `equatable`. Events describe user/system actions; states represent UI state; repositories sit behind the bloc. This is the standard for all new CRM features.
  - **One documented exception:** the member-detail **Waivers section** (`features/member_details/.../sections/member_waivers_section.dart`) is a small **read-only `FutureBuilder`** over `MembershipsRepository.listMemberWaiverStatus` rather than threading a new event/state through `MemberDetailBloc`. It is a pure display of sign status (no mutation), kept isolated so it doesn't bloat the member-detail bloc — the same shape the edge reads use.

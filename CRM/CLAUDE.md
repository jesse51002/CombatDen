...
- **Supabase auth gates the admin app, and only in `main.dart`.** `main.dart` boots Supabase (`SupabaseConfig.initialize`), provides the `LoginBloc`, and mounts the `AuthGate`, which routes: initial/loading → spinner, unauthenticated/error → `LoginScreen`, authenticated → **list the gyms the caller may administer (`GET /api/v1/gyms/`, owner + admin, role-annotated)** and route on the count: **0** → `GymSetupScreen`; **1** → the members workspace scoped to that gym (in a nested `Navigator` sharing the admin route table); **2+** → `GymPickerScreen`, then the workspace once a gym is chosen. Picking activates the gym via `selectedGym.setActiveGym(...)`, whose real UUID then scopes every member view. The whole authenticated subtree tears down on sign-out.
  - **Kiosk Mode intercepts the workspace.** While the app-root `KioskSessionCubit` (provided above the gate in `main.dart`) is active, the authenticated branch mounts the member self-serve `KioskScreen` — or the fail-closed `KioskLockedScreen` once the session has ended — **instead of** the workspace, so the admin nested navigator + route table + `AppShell` never mount. The admin session + `selectedGym` stay live underneath; *leaving* kiosk is what signs out (that is the core security property — a supervised iPad must never expose the admin workspace). The kiosk branch sits **inside** the authenticated branch on purpose: the persisted kiosk flag (`shared_preferences`, fate-shared with the Supabase session) is only honored while a session exists, so a flag without a session is inert and fails closed to the login screen. An absolute 12-hour runway from entry bounds every kiosk session (lockout of new flows at T+11h45, hard revoke at T+12h); it is never extended by member interaction. See `features/kiosk/` — Phase B is the security shell (`KioskSessionCubit`); Phase C1 is the member **check-in lane** the `KioskScreen` now hosts: a feature-local `KioskFlowCubit` drives the internal navigation (home → name-search / QR placeholder → class pick → `is_member: true` check-in → glance / blocked) plus a 5-minute flow-idle guard that abandons a stale draft, gating every new flow on the session's `canStartFlow` and bracketing it with `beginFlow`/`endFlow`. Phase C2 is the live retention **glance** (`KioskGlanceScreen`): a recorded check-in shows the member's streak (from the check-in response) beside their points balance + reward tiles (`KioskFlowCubit` fetches the balance via `GET /members/{id}/billing` and caches the gym-wide reward catalog once), with its OWN 8-second auto-return to home (a tap anywhere or Done also returns) — governed by that timer, never the 5-minute idle guard, since the flow has already ended. Reward tiles show a ready disc when affordable / a progress ring otherwise; both the balance fetch (→ cost-only tiles) and the catalog (→ points-only panel) degrade gracefully on failure. The week-day strip marks only today (no per-day backend source exists). The **signup** flow is still a Phase D stub, and the live per-scan **QR** code Phase G.

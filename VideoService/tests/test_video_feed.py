    ...
    return VideosOutput(
        ...
        videos=[
            VideoOutput(
                ...
                view_count=1000,
                like_count=50,
                tags=["tutorial", "clips"],  # spans both big groups
...
def test_feed_projects_from_output_and_drops_validation_fields() -> None:
    ...
    # big_groups is the coarse sort, derived from tags; this video spans both.
    assert [g.value for g in card.big_groups] == ["educational", "entertainment"]
    ...
    assert "big_groups" in dumped_card  # the new field IS served

"""Pure transforms for the Apify search path: streamers/youtube-scraper dataset
items -> VideoOutput, incl. the inline transcript, duration parsing, and dedup
across queries. No network."""

from __future__ import annotations

from scripts.scraper.transform import (
    build_outputs,
    parse_search_items,
)
...
def test_parse_and_build_basic() -> None:
    ...
    assert v.tag is None and v.gym_type == []  # untagged until the tagging pass
...
def test_hidden_counts_become_none() -> None:
    v = build_outputs(parse_search_items([_item("a", viewCount=None, likes=None)], "q"))[0]
...
def test_dedup_merges_queries_keeps_best_rank() -> None:
    # Same video from two queries; second appears at a worse rank.
    hits = parse_search_items([_item("dup"), _item("x")], "q one")
    hits += parse_search_items([_item("y"), _item("dup")], "q two")  # dup at rank 1
    out = build_outputs(hits)
...
def test_non_video_items_skipped() -> None:
    out = build_outputs(parse_search_items([{"title": "no id or url"}, _item("ok")], "q"))

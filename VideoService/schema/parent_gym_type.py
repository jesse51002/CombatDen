...
The 8-bucket vocabulary the gym browser filters by (Fighting / Yoga / … ). Every
fine discipline rolls up to exactly one parent here (`vinyasa` -> Yoga, `mma` ->
Fighting). This roll-up is **gym business logic and lives in VideoService** — not
in ThemeService, which only produces themes. Serializes as its capitalized value.
"""
...
class ParentGymType(str, enum.Enum):

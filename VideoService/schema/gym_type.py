...
The fixed, shared vocabulary of fitness *disciplines*. Like `VideoType`, this is
a deliberate exception to the company-agnostic rule: it is a fixed, shared
vocabulary every gym draws from, not a per-company value.

It is *finer* than the 8-bucket `gym_type` a theme's `customization.yaml`
carries (Fighting / Yoga / Pilates / …) — here a discipline is the specific
practice (`vinyasa`, `bjj_gi`, `rowing`). The classification pass assigns each
pooled video a *list* of these from the video's actual content (a clip can be
relevant to several disciplines, e.g. a kettlebell-rowing finisher →
`[kettlebell, rowing]`). A gym then uses its single discipline to pick which
slice of the pool it scans. Serializes as its lowercase value in YAML.
"""

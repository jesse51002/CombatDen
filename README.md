...
flowchart TB
  ...
  Resend["✉️ Resend<br/>CombatDen's own outbound mail"]
  ...
  FastApiBackend -->|"emails: staff onboarding + member app invite"| Resend
  ...
  class Database data;
  class Deploy tool;
  class Stripe,LiteLLM,Gemini,Apify,YouTube,GoogleFonts,Recraft,CDN,LLMProvider,Resend ext;
...
- **Outbound email splits into three channels that never touch each other.** **Supabase Auth (GoTrue)** sends login confirmation and password-reset mail — Dashboard config, no code. **Stripe** sends card-declined / dunning mail from each gym's own connected account, in that gym's branding — config, no code. **FastApiBackend's `emails` domain** sends only the mail CombatDen itself writes (staff onboarding, member app invite) through **Resend**, logged one row per message in `email_log`. Keeping them distinct is deliberate: CombatDen sends no payment receipts and no overdue notices, because a monthly receipt is a monthly prompt to cancel and Stripe's Smart Retries already own the dunning conversation.

...
flowchart TB
  ...
  VideoService["🎬 VideoService<br/>video pipeline + read API"]
  FastApiBackend["⚙️ FastApiBackend<br/>CRM / billing API (not deployed)"]
  ...
  CRM -->|"member feed (read-only)"| VideoService
  CRM -->|"authd API"| FastApiBackend
  ...
  VideoService -->|"video_* tables"| Database
...
- **Dashed box border = built but not yet deployed.** The CRM has been **restored to a full gym-admin CRM** — Supabase-auth login, members list + member detail, gym setup, and the Stripe billing surface are built natively and wired to the FastApiBackend over an authenticated `dio` client; its VideoService/ThemeService reads stay live. The **FastApiBackend** that backs it is built (members/gyms/classes/ranks/rewards + the restored memberships/plans/discounts/payments/webhooks) but has **no prod host yet** (`api.combatden.net` pending). Billing's last ~25% (failed-payment / proration / refund edge cases) is intentionally unfinished.
...
**📱 MobileApp** — Flutter member-facing app (the product members would use). Live features: a paginated video feed and gym detail (classes + rewards) from VideoService; the rest (home, booking, profile) is a high-fidelity prototype. Branding is resolved at runtime through the shared `theme_flutter` package. *Talks to:* VideoService (feed/gym), ThemeService (branding).

**🖥️ CRM** — Flutter **web** gym-owner admin app (formerly AppManagement). The **full CRM**, restored from the old FlutterCRM and rebuilt natively in AppManagement's design: a **Supabase-auth** login gate, **members list + member detail** (memberships, invoices, charges, discounts, linked accounts + their action dialogs), **gym setup** (Stripe Connect onboarding), and the **Stripe billing** surface — all built on BLoC + repositories calling **FastApiBackend** through an authenticated `dio` client (`lib/core/network/api_client.dart`, Supabase-JWT interceptor; e.g. `POST /api/v1/gyms/`). The two read-only edge integrations stay live: the **VideoService** feed (`/gyms/{id}/videos`, gym detail) and the **ThemeService** live theme preview (via `theme_flutter`), the latter also shipping as the standalone public **theme browser** at `themes.combatden.net`. Billing's last ~25% (Stripe edge cases) is intentionally unfinished and the backend isn't deployed yet, so end-to-end runs against a local FastApiBackend. *Talks to:* FastApiBackend (auth'd), Supabase (auth), VideoService + ThemeService (read-only). Deploys to `app.combatden.net` + `themes.combatden.net`.
...
**⚙️ FastApiBackend** — Python/FastAPI membership + billing backend (the CRM's API). Routers for members, gyms, classes, ranks, rewards, plus the restored billing subsystem — membership plans, discounts, member memberships, payments, and Stripe webhooks; a service/SQL layer; Supabase-JWT auth. The request/response contract is published as `Database/openapi.json` (48 paths). *Talks to:* Database (read/write + Supabase Auth), Stripe (payments + webhooks). **Built and wired to the CRM, not yet deployed** (prod target `api.combatden.net`); billing's last ~25% (failed-payment / proration / refund edge cases) is intentionally unfinished.

**🎬 VideoService** — owns the gym-video lifecycle, and it is **two separate halves**. The **CREATION pipeline** (driven by the `videoservice` skill — make-gym / scrape / scan — plus the `gym_maker`, `scraper`, `scan`, `sync_gyms`, `import_yaml`, `sql` scripts and the `VideoDbWriter`) scrapes YouTube via Apify, classifies with Gemini/LiteLLM, and **writes** the shared Postgres `video_*` tables. The **READ API** at `video.combatden.net` (`/gyms`, `/gyms/{id}`, `/videos` + a service layer with avatar fallback) only **reads** those tables. The two never call each other — the database is the handoff.
...
**🗄️ Database** — The shared **Supabase Postgres** that everything converges on. Holds `supabase/schemas/*.sql` (tables), `access_rules/*.sql` (RLS), Python enum mirrors in `python_data/schema/`, and the authoritative `openapi.json`. Table domains: member, gym, rank/reward, membership + billing (Stripe-gated), class scheduling, and the `video_*` tables. *Consumed by:* FastApiBackend and VideoService directly; Supabase Auth also backs CRM login.

---

## How they connect (the load-bearing wiring)

- **CRM is wired to FastApiBackend.** CRM authenticates with Supabase, then calls FastApiBackend over an authenticated `dio` client (`api_client.dart` + JWT interceptor). The members list/detail, gym-setup, and billing screens are built natively and dispatch through BLoC → repositories → the API; the VideoService/ThemeService reads remain the two read-only edge integrations. The backend isn't deployed yet, so end-to-end runs against a local FastApiBackend on `:8000`.
- **One shared database is the hub.** FastApiBackend and VideoService both read/write the same Supabase Postgres. They stay consistent by importing enum mirrors from `Database/python_data/schema/` and honoring the `Database/openapi.json` request/response contract.
- **Each engine's creation and API sides are decoupled by an artifact.** ThemeService-CREATION writes `output.yaml` (+ CDN assets) that ThemeService-API serves; VideoService-CREATION writes the `video_*` tables that VideoService-API reads. No direct call crosses the two halves.
...
- **VideoService points at ThemeService for branding.** Each gym's YAML stores a ThemeService `design_id` (and reads the theme's celebration image from the CDN), so the curated feed and the visual theme line up per gym.
...
| `Database/` | Supabase schema, RLS, enums, `openapi.json` | `Database/CLAUDE.md` |

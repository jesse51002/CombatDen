...
def all_slot_ids(app: AppFormat) -> set[str]:
    ...
    images) plus the classification pseudo-slot — the universe a regen can
    target and the seed can fill."""
    ids: set[str] = set()
    for slots in (
        app.colors,
        app.fonts,
...
def node_slots(app: AppFormat) -> dict[str, set[str]]:
    ...
    no slots); text/icon only when declared, and classification only when the
    app declares a ``categories`` vocabulary. The registry uses this to give

...
Six fields. Never invent a seventh.
...
- **Gym name** *(open text → `name`)*. The very first question, and the **only**
  ...
  names.
- **Gym type** *(single-select, uncounted — GROUNDING ONLY, not written)*.
  Right after the name, ask what kind of gym it is (`Fighting`, `Yoga`,
  `Pilates`, `Barre`, `HIIT`, `Cardio`, `Dance`, `Wellness`, or **Other**). This
  is **not stored in the brief** — gym categorization is VideoService business
  logic, not part of the theme pipeline. It is asked purely to pre-tune every
  later option bank and the prose. Uncounted (a deterministic single-select with
  no composed prose).
...
7. **Surface the run command** (a suggestion only — do NOT run it yourself).
   ...
   **If you ever do run generations** (here or via a follow-on like
   `edit_theme`), and especially when authoring/generating for **more than one
   app**, run them **strictly sequentially — one at a time, never in parallel**.
   The pipeline hits rate-limited LLM / image providers; concurrent runs, `&`,
   parallel background jobs, or batched fan-out get throttled. Surface one
   command per app and let each finish before the next; never offer a "generate
   them all at once" shortcut.
...
- Never run generations in parallel — if generation happens at all, it is
  sequential, one run in flight at a time (rate-limited providers); never `&`,
  never parallel background jobs, never a "generate all apps at once" shortcut.

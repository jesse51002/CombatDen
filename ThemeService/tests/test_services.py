...
class _FakeHTTPResponse:
    """Mimics an httpx response for the Recraft removeBackground call.

    ``content`` is the raw cutout bytes; ``json()`` wraps them as Recraft's
    ``response_format=b64_json`` shape: ``{"image": {"b64_json": ...}}``.
    """
    ...
    def json(self) -> dict:
        return {
            "image": {
                "b64_json": base64.b64encode(self.content).decode("ascii")
            }
        }
...
def test_recraft_bg_remove_writes_dst(monkeypatch, tmp_path):
    src = tmp_path / "in.png"
    ...
    asyncio.run(RecraftBackgroundRemover().remove(src, dst))
    assert dst.read_bytes() == b"transparent-png"


def test_recraft_bg_remove_error_maps_to_provider_error(
    monkeypatch, tmp_path
):
    src = tmp_path / "in.png"
    src.write_bytes(b"solid-bg-image")

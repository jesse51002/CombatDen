...
class ThemeService extends ChangeNotifier {
  ThemeService(
    ...
    ThemeAssetWarmer? warmer,
  }) : _warmer = warmer ?? ThemeAssetWarmer();

  final ThemeApiClient _apiClient;

  /// Primes the image/icon caches for the active theme and evicts the
  /// previous theme's assets on each switch. Best-effort, never throws.
  final ThemeAssetWarmer _warmer;
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      debugPrint('ThemeConfig refresh failed: $e');
      // Keep disk last-good / defaults.
    }

    _warnMissingSlots();
    notifyListeners();
    // Prime the caches for whatever loaded (network, disk last-good, or
    // nothing → a no-op). Nothing to evict on first load.
    _warmer.onConfigChanged(this);
  }
  ...
  Future<bool> selectDesign(String designId) async {
    try {
      ...
      _warnMissingSlots();
      notifyListeners();
      // Evict the outgoing theme's warmed assets, prime the incoming one's.
      _warmer.onConfigChanged(this);
      return true;
    } catch (e) {
      debugPrint('ThemeConfig selectDesign failed: $e');
      return false;
    }
  }

  /// One page of the app's selectable styles. Pass [query] to
  /// substring-filter on id or display name.
  Future<ThemeStylesPage> fetchStylesPage({
    int offset = 0,
    int limit = 20,
    String? query,
  }) => _apiClient.fetchStylesPage(
    offset: offset,
    limit: limit,
    query: query,
  );
  ...
  bool _tryAdopt(String rawJson) {
    try {
      ...
      _current = ThemeConfig.fromJson(decoded);
      return true;
    } catch (e) {
      debugPrint('ThemeConfig parse failed: $e');
      return false;
    }
  }

...
class RunCost(BaseModel):
    ...
    is a flat per-call rate (Recraft returns no usage). ``llm`` covers
    ...
    ``"recraft_remove_bg"`` key; the Recraft icon generator buckets under its
    own model id (e.g. ``"recraftv4_1_utility_vector"``). The buckets are

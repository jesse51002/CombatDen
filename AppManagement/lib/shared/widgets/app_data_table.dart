...
class _AppDataTableState extends State<AppDataTable> {
  ...
  Widget build(BuildContext context) {
    ...
    // shrink-wrap mode skips LayoutBuilder entirely so the table can be
    // wrapped in IntrinsicHeight (LayoutBuilder doesn't support intrinsic
    // dimensions). Columns size themselves via Flutter's flex system:
    // non-fill columns get `SizedBox(width: minWidth)`, fill columns get
    // `Expanded`. No horizontal scroll path in shrink-wrap mode.
    if (widget.shrinkWrap) {
      Widget table = _buildFlexTable(
  ...
  /// Shrink-wrap path: lays out columns via Flutter's flex system instead
  /// of measuring viewport width through `LayoutBuilder`, so the table
  /// supports intrinsic dimensions and can sit inside `IntrinsicHeight`.
  Widget _buildFlexTable({
    ...
    for (int i = 0; i < widget.rows.length; i++) {
      ...
      Widget rowWidget = _buildFlexRowChildren(
        cells: row.cells,
        height: DesignConstants.tableRowHeight,
      );
      if (row.onTap != null) {
        rowWidget = InkWell(onTap: row.onTap, child: rowWidget);
      }
  ...
  /// Wraps each cell in either a `SizedBox(width: minWidth)` (for fixed
  /// columns) or `Expanded` (for fill columns), separated by
  /// `spacingSmall`. Same width contract as the LayoutBuilder path,
  /// expressed through flex so it composes inside IntrinsicHeight.
  Widget _buildFlexRowChildren({
    ...
    for (int i = 0; i < widget.columns.length; i++) {
      ...
      if (col.fill) {
        ...
        children.add(
          SizedBox(width: col.minWidth ?? 80, child: cell),
        );
      }
    }
    return SizedBox(

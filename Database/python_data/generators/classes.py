...
def generate_class_history_and_attendance(
    ...
    occurrences, then assign each member a random subset of those rows
    ...
    # Build attendance: each member attends 0-15 random class_history rows
    if not history:
        return history, attendance

    for member in members:
        if random.random() < 0.05:
            continue  # ~5% of members never attended
        n_attended = random.randint(1, min(15, len(history)))
        sampled = random.sample(history, n_attended)
        for h in sampled:
            attendance.append(
                MemberAttendanceCreate(
                    log_id=uuid.uuid4(),

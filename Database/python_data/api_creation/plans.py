...
Stripe price in one call, and the response body includes `active_price` —
so we only make one API call per plan. Returns a lookup keyed by a stable
local handle ("plan0", "plan1", ...) so downstream generators that need the
real plan_id/price_id/base_cost can find them.
"""
...
# Mirrors generators/plans.PLAN_TEMPLATES but keyed for API payload.
PLAN_TEMPLATES = [
    {

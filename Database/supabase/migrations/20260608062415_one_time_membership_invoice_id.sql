...
alter table "public"."member_memberships_unfiltered" add column "stripe_one_time_invoice_id" character varying;
...
CREATE OR REPLACE FUNCTION public.prevent_stripe_one_time_invoice_id_overwrite()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE TRIGGER trg_prevent_stripe_one_time_invoice_id_overwrite BEFORE UPDATE OF stripe_one_time_invoice_id ON public.member_memberships_unfiltered FOR EACH ROW EXECUTE FUNCTION public.prevent_stripe_one_time_invoice_id_overwrite();

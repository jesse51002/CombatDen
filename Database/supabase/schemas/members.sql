...
CREATE TABLE members (
    ...
    -- email = this member's self-access to their row (matched via
    -- auth.jwt() ->> 'email', not a user_id FK). Stays NULLABLE:
    -- engagement-only members legitimately have no email and thus no
    -- self-login. NOT unique — families share an email, and one account may
    -- legitimately match several member rows at a gym.
    email VARCHAR,

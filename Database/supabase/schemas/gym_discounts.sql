CREATE TABLE gym_discounts (
    ...
    discount_type VARCHAR NOT NULL CHECK (discount_type IN ('preset', 'custom', 'family')),
    ...
    family_discount_num INTEGER CHECK (family_discount_num > 0),
    ...
    CONSTRAINT chk_family_discount_fields CHECK (
        (discount_type = 'family' AND membership_plan_id IS NOT NULL AND family_discount_num IS NOT NULL)
        OR (discount_type <> 'family' AND membership_plan_id IS NULL AND family_discount_num IS NULL)
    ),
    ...
    UNIQUE (gym_id, membership_plan_id, family_discount_num),
...
-- Trigger: enforce sequential family_discount_num per (gym_id, membership_plan_id)
-- INSERT: must be max + 1
-- UPDATE: must stay within [1..count]
-- DELETE: only the highest num can be deleted (reject gaps)
CREATE OR REPLACE FUNCTION enforce_family_discount_sequence()
RETURNS TRIGGER AS $$
DECLARE
    max_num INTEGER;
    total_count INTEGER;
...
CREATE TRIGGER trg_enforce_family_discount_sequence_insert_update
    BEFORE INSERT OR UPDATE OF family_discount_num ON gym_discounts
    FOR EACH ROW
    WHEN (NEW.discount_type = 'family')
    EXECUTE FUNCTION enforce_family_discount_sequence();

CREATE TRIGGER trg_enforce_family_discount_sequence_delete
    BEFORE DELETE ON gym_discounts
    FOR EACH ROW
    WHEN (OLD.discount_type = 'family')
    EXECUTE FUNCTION enforce_family_discount_sequence();

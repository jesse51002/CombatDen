...
CREATE TABLE template_gym (
    gym_id TEXT NOT NULL
        CONSTRAINT template_gym_id_format CHECK (gym_id ~ '^[a-z0-9][a-z0-9_]*$'),
    ...
    gym_type JSONB NOT NULL
        CONSTRAINT template_gym_type_nonempty
        CHECK (jsonb_typeof(gym_type) = 'array' AND jsonb_array_length(gym_type) >= 1),
    theme TEXT NOT NULL CONSTRAINT template_gym_theme_nonempty CHECK (theme <> ''),
    ...
    videos_desc TEXT NOT NULL
        CONSTRAINT template_gym_videos_desc_len CHECK (char_length(videos_desc) >= 2),
    avoid_desc TEXT NOT NULL
        CONSTRAINT template_gym_avoid_desc_len CHECK (char_length(avoid_desc) >= 2),
    ...
    CONSTRAINT pk_template_gym PRIMARY KEY (gym_id)
);

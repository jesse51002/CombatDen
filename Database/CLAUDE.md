...
- Append-only tables (logs / history / metrics — `gym_history`, `member_activities`, `member_reward_redemptions`) `REVOKE UPDATE` for `authenticated` (they keep a gym-scoped authenticated INSERT policy). **`class_history` and `member_attendance` go further — they are `service_role`-WRITE-ONLY** (`REVOKE INSERT, UPDATE, DELETE … FROM authenticated`, no authenticated write policy at all, exactly like `class_signups`/`tasks`): every write flows through the gated backend (the materialize / check-in / reconciler paths), so a raw client INSERT can't bypass the capacity / eligibility / points / billing-attribution logic. On `member_attendance`, `plan_id`/`item_id` are **nullable and NULL together** (enforced by `chk_attendance_membership_pair`): an admin (non-kiosk) check-in with no covering membership records both NULL (no pack drawn), while a covered check-in sets both. Cycle-count / streak reads ignore the NULL-membership rows. The composite FKs use MATCH SIMPLE, so a NULL-membership row simply skips them.
...
- **`class_signups`** — a member's reservation for a class occurrence (NOT attendance; `member_attendance` is still the only attendance record). SELECT mirrors `member_attendance`/`class_history` — a member sees their own sign-ups, gym staff see everything at their gym — but writes are a **stricter hybrid** than either sibling: like `tasks`, there is NO authenticated write policy at all (`REVOKE INSERT, UPDATE, DELETE … FROM authenticated`), because both staff AND the member themselves may create/cancel a sign-up, so the API (`verify_can_view_member`) does the authorization instead of a role-scoped RLS policy. `UNIQUE (class_id, member_id, occurrence_date)` backs the idempotent `ON CONFLICT DO NOTHING` create. See `FastApiBackend/src/checkin/service/signup_service.py`.

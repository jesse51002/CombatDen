...
## Security
- **The client roles hold NO privileges. All access goes through the backend.** `anon` and `authenticated` have every privilege revoked on every table, view and sequence in `public` (`access_rules/zz_client_privileges.sql`, applied by `migrations/20260721151119_revoke_client_data_surface.sql`), including the default privileges, so a newly created table can't silently regain them. There is no client PostgREST data surface at all: the CRM uses Supabase only for GoTrue auth (`signUp` / `signInWithPassword` / `signOut` / `resend` / `onAuthStateChange`), MobileApp has no supabase dependency, VideoService connects as `postgres`, and the FastAPI backend + the seed use `service_role`. **`service_role` and the `auth` schema are untouched** — never revoke from either (the backend, the seed, and login all break). **Never write a `GRANT ... TO anon` or `GRANT ... TO authenticated` anywhere in `access_rules/`** — it re-opens exactly this hole.
- **A column-list `REVOKE` cannot subtract from a table-level `GRANT`.** This is the gotcha that made the old rule dangerous: Postgres has no "table privilege minus one column" state, so `REVOKE UPDATE (col, …) ON TABLE x FROM authenticated` on a role that holds table-level UPDATE is a **silent no-op** — verified live (`has_column_privilege('authenticated','members','points_balance','UPDATE')` returned `true` with the column REVOKE in place). Only the whole-table form (no column list) actually removes anything. So if a column ever genuinely needs client-side write gating, the pattern is **revoke the table, then grant the columns back**:
  ```sql
  REVOKE UPDATE ON TABLE x FROM authenticated;
  GRANT UPDATE (allowed_col, other_col) ON TABLE x TO authenticated;
  ```
  The column-list REVOKEs still present in `access_rules/` are harmless leftovers (inert once the table-level grant is gone) — do not copy the pattern into new files.
- **RLS is defense-in-depth, not the lock.** Always enable Row Level Security (RLS) on every table, and keep every existing policy: with the grants gone they are a second, independent barrier, deliberately retained. But read every "`authenticated` can …" policy claim below as *RLS would permit this*, never as *a client can do this* — no client can reach any table.
- **Email confirmations must stay ON.** `config.toml` `[auth.email] enable_confirmations = true`. Identity is the **verified** email claim (the JWT `email` joined live against `gym_employees.email` / `members.email`), so an auto-confirming stack would let anyone sign up as an existing owner's address and take over that gym. The hosted project is configured separately in the Supabase Dashboard and needs a working SMTP sender; locally, confirmation mail lands in Inbucket and the seed is unaffected (it creates users through the admin API with `email_confirm=True`). **This is GoTrue's own channel, entirely separate from the backend's `emails` domain** — it never writes an `email_log` row, and configuring one does nothing for the other. (Inbucket also catches only GoTrue's mail; the backend's Resend client talks HTTPS regardless of the local stack.)
...
- **Exception — `members` (per-column gating).** The unified `members` table mixes client-writable identity (name, email, points, rank — created/edited by gym staff with no Stripe) with `service_role`-only columns: the billing columns (`stripe_*`, `card_*`, `freeze_*`, `total_monthly_recurring_price`) **and** the four RAG video-taste profile columns (`video_profile_summary`, `video_profile_embedding vector(3072)`, `video_profile_embedding_model`, `video_profile_built_at` — the backend writes them on the click/booking refresh trigger). Its access rules therefore *express* intent per-column rather than per-table: `access_rules/members.sql` revokes INSERT plus a long UPDATE column list (billing + `video_profile_*`) from `authenticated`. **Those column-list REVOKEs never did anything** (see the column-list gotcha above) and are moot now that `authenticated` holds no privileges at all — the real enforcement of "clients never write billing or taste-profile columns" is the backend's own `immutable_columns.py` / `validate_mutable_columns()` guard on user-facing update requests. (`members.account_linked_to_id` is gone — the single-parent family link was replaced by the `member_authorized_payers` junction, the **authorization** layer: who is *allowed* to pay for whom, not the billing key. Billing is per-payer via `member_memberships.paid_by_member_id` (the resolved payer, or a self-paying member). The old `linked_account_no_stripe` CHECK that forced a linked member to hold no billing state is **dropped** — a member paid for by an authorized payer legitimately holds their own `stripe_sub_id_month` / card / freeze window.) `members` is deliberately NOT filtered by a `hide_incomplete_stripe_records` policy — engagement-only members have NULL billing columns and must stay visible. Stripe-complete billing reads go through the `member_billing_profile` VIEW (`SELECT * FROM members WHERE stripe_customer_id IS NOT NULL`), which replaced the former separate `member_billing_profile` table when identity + billing were unified onto `members`.
- **`gym_employees` — identity is verified email, no `user_id` FK.** Neither `gym_employees` nor `members` carries an `auth.users` FK column; a person's access is a live join between their (lowercased) Supabase JWT `email` claim and a stored `email` column, re-evaluated on every request — both in RLS (the `is_gym_employee` / `is_gym_admin_or_owner` security-definer helpers in `schemas/gyms.sql`, matched via `auth.jwt() ->> 'email'`) and in the FastAPI layer (`Auth._resolve_employee`). `gym_employees.email` is lowercase-normalized and backed by a **partial unique index** `unique_employee_email_gym` on `(gym_id, lower(email)) WHERE email IS NOT NULL` — one row per email per gym; `members.email` carries **no** uniqueness (families legitimately share an email). `employee_type` gained a fourth value, **`front_desk`** (`owner | admin | front_desk | trainer` — that is the privilege reading order, **not** the enum's ordinal order: the value was appended last, so the type is declared `('owner', 'admin', 'trainer', 'front_desk')` and `ORDER BY employee_type` sorts owner → admin → trainer → front_desk; sort explicitly rather than relying on it), and trainers can now hold real logins — the old `chk_trainer_has_no_account` rule is **replaced** by `chk_principal_has_email` (`employee_type = 'trainer' OR email IS NOT NULL`): every login-carrying role must have an email, and only a trainer row may stay email-less (pure instructor data). `archived_at` (nullable timestamptz) is the soft-archive flag — every auth check filters `archived_at IS NULL`, so an archived row loses access instantly with **no** touch to the underlying Supabase auth account (hard delete is impossible: instructor + `member_waiver_signatures.operator_employee_id` FKs reference the row). Full model, the 4-role capability matrix, and the backend/CRM enforcement chain: **`employees-guide`** skill.
- Append-only tables (logs / metrics — `member_activities`, `member_reward_redemptions`) `REVOKE UPDATE` for `authenticated` (they keep a gym-scoped authenticated INSERT policy). **`member_attendance` and `gym_class_schedules` go further — they are `service_role`-WRITE-ONLY** (`REVOKE INSERT, UPDATE, DELETE … FROM authenticated`, no authenticated write policy at all, exactly like `class_signups`/`tasks`): every attendance write flows through the gated check-in path (capacity / eligibility / points / billing-attribution), and every schedule-version mint runs inside the backend transaction that also executes the version-change WIPE (sign-up deletion + check-in reversal + points clawback) — a raw client INSERT would bypass exactly that. `gym_class_schedules` rows are additionally write-ONCE (append-only versions; nobody UPDATEs them — the immutable schedule past). Attendance and sign-ups key an occurrence by its ORIGINAL slot (`class_id`, `original_date`, `original_time` — the owning schedule version's pre-exception slot); `member_attendance.occurred_at` is a denormalized effective-start instant for window SQL only. On `member_attendance`, `plan_id`/`item_id` are **nullable and NULL together** (enforced by `chk_attendance_membership_pair`): an admin (non-kiosk) check-in with no covering membership records both NULL (no pack drawn), while a covered check-in sets both. Cycle-count / streak reads ignore the NULL-membership rows. The composite FKs use MATCH SIMPLE, so a NULL-membership row simply skips them.
...
- **`gym_growth_metrics` is a backend-written CACHE with staff READ access** — one row per `(gym_id, key)` holding a metric's jsonb payload, recomputed wholesale by the backend's growth sweep at app launch and hourly (`FastApiBackend/src/growth/`). Service-role-write-only (`REVOKE INSERT, UPDATE, DELETE … FROM authenticated`) with a gym-staff SELECT policy via `is_gym_admin_or_owner`. No `stripe_*` column, no `_unfiltered`/view split, **not seeded** (the sweep fills it). Deliberately generic: only `key`, `type` and `data jsonb` are stored — a metric's display metadata (name, categories, sort order) lives in the backend registry and is attached at serve time, so renaming or reordering a metric needs neither a recompute nor a migration. `type` is a write-time debugging copy; the registry always wins on read. Because payloads are validated against the registry's model on READ, a row whose shape no longer matches is skipped and self-heals on the next sweep — which is what makes rolling deploys and multiple backend containers safe. It **replaced `gym_history`** (a daily active/inactive snapshot rollup, dropped in migration `20260722000000`): all history is now derived fresh from the source tables, so there is no second rollup to drift.
...
- **`email_log` + `email_suppressions` cover CombatDen's OWN outbound mail, and nothing else.** GoTrue's confirmation / password-reset mail and Stripe's card-declined mail (sent from each gym's own connected account) are separate channels that never write a row here — so an empty `email_log` says nothing about whether login mail is working. Both tables are **service-role-write-only** (`REVOKE INSERT, UPDATE, DELETE … FROM authenticated`, no authenticated write policy), no `stripe_*` column, no `_unfiltered`/view split, **not seeded**.
  - **`email_log`** — one row per intended message, and simultaneously the audit trail and the retry queue; rows are never deleted. The backend claims a row *inside the transaction of whatever triggered it* (an employee insert, a member insert), so a rolled-back create un-sends its email and a crash mid-send leaves a row the reconciler's retry sweep recovers. **`UNIQUE (idempotency_key)` is the entire no-duplicates mechanism** — the backend builds the key from the payload (`<kind>:<subject_id>:<resend_seq>`), so a double-clicked button, a retried request, and a re-run sweep all collide on one row; a *deliberate* resend bumps its own counter inside the key, which is what makes it a genuinely new send. `subject_id` is deliberately **NOT** a foreign key (polymorphic across `gym_employees` / `members`, and the log must outlive its subject being archived); it exists so the per-person resend cap is an indexed count rather than a JSONB dig. `recipient` is the address resolved at send time and then frozen — the audit answer to "where did it really go"; never read the payload for that. `CONSTRAINT sent_matches_status CHECK ((status = 'sent') = (sent_at IS NOT NULL))` mirrors `resolved_matches_status` on `member_reward_redemptions`. The `email_status` enum's **`'held'` is terminal by policy, not a transient state**: the kind was absent from the backend's `EMAIL_ENABLED_KINDS` at claim time, and the retry sweep skips held rows forever — otherwise enabling a kind months later would drain the whole backlog at once and mail everyone who ever joined. It carries a gym-scoped staff SELECT policy (`is_gym_admin_or_owner`).
  - **`email_suppressions`** — what an address has opted out OF. The `email_suppression_scope` enum is the load-bearing part: `'marketing'` (an unsubscribe) blocks marketing kinds only, so opting out of a pitch never costs someone the transactional mail that carries their access; `'all'` (a hard bounce) blocks everything, because retrying a dead address is what damages the sending domain's reputation. Whether a KIND is marketing or transactional is a property of the kind, so it lives in the backend registry (`FastApiBackend/src/emails/emails_registry.py`), never here. `gym_id IS NULL` means **global** — a dead mailbox is dead for every gym — which is why uniqueness is **two partial unique indexes** (`unique_email_suppression_gym` / `unique_email_suppression_global`) rather than one: Postgres treats NULLs as distinct, so a plain UNIQUE would let the same global bounce be recorded unboundedly many times (same reasoning as `unique_employee_email_gym`). It has **no SELECT policy at all, deliberately** — a global suppression belongs to no gym, and any policy permissive enough to expose it would leak one tenant's bounced addresses to another.
  - `email_kind` grows only by `ALTER TYPE email_kind ADD VALUE`, and **that migration must be applied before any code referencing the new value ships** — a value added by `ALTER TYPE … ADD VALUE` cannot be referenced by the transaction that added it. Full model: `FastApiBackend/CLAUDE.md` under *Emails domain*.
- **Waiver tables.** `gym_waivers` is plain gym config (no Stripe) — gated like `gym_classes`/`gym_rewards` (gym-staff SELECT plus gym-scoped INSERT/UPDATE/DELETE, identity columns immutable, no `_unfiltered` view), soft-deleted via `is_deleted`. Its `current_version_id` is a forward FK to `gym_waiver_versions` declared via `ALTER TABLE` at the bottom of the versions schema file (circular ref; the catalog row + first version + current-version pointer are written in one backend transaction). `gym_waiver_versions` is **conditionally immutable** + `REVOKE UPDATE, DELETE` for `authenticated` (clients never write version rows). The backend (service_role) edits the current version **in place while it has 0 signatures**; once a member has signed it, that version is frozen and a body edit **publishes a NEW version row** + re-points `current_version_id`, so a signature bound to a version always reproduces the exact wording (`src/waivers/service/waivers_update.py::_maybe_publish_version`). Each version carries **`requires_resign`** (BOOLEAN default true): a forked version flagged true invalidates prior signers for the membership-purchase gate (they must re-sign), while a minor edit published with `requires_resign = false` keeps prior signatures valid — the gate's compliance floor is the highest `version_number` with `requires_resign = true`. A version `body` is a **template** that may hold `{{placeholders}}` (catalog `python_data/schema/waiver_parameters.py`); at sign time `WaiversSignatures.sign_waiver` renders them (auto member/signer/gym/date + caller `waiver_args` like `payee_name`) and freezes the full **`rendered_body`** + its `content_hash` (sha256 of the rendered text) on the signature. `member_waiver_signatures` is an append-only e-sign audit record + **generic signature log** (`REVOKE UPDATE, DELETE`); members see their own, staff see their gym's, and it keeps a **gym-staff INSERT policy** (the front-desk clickwrap's RLS shape) — though in practice every write, clickwrap included, lands through the backend at `service_role` via the standalone signing endpoint + the authorize-payer link flow, since clients hold no table privileges. Its legal-evidence columns are NOT-NULL: typed `signer_name`, forced-true `consent_acknowledged`, version-pinned `waiver_version_id`, `rendered_body`, `content_hash`, `ip_address` + `user_agent`, **`esign_disclosure_version`** (the ESIGN/UETA disclosure shown), plus the NULLABLE **`operator_employee_id`** composite FK → `gym_employees` (the staff witness). Every `gym_waivers` row carries a **`waiver_type` enum** (`payer_auth` | `custom`, expandable): `custom` = a gym-authored document attachable to membership plans (`membership_plans.waiver_ids` — validated at plan write time to be existing, non-archived, `custom` waivers only); `payer_auth` = the gym's ONE **authorized-payer agreement** (signed only in the authorize-payer link flow, never plan-attachable) — protected from client tampering (`trg_protect_payer_auth_waiver` blocks archiving/deleting it for `authenticated`/`anon` roles, `idx_gym_waivers_one_payer_auth` enforces ≤1 per gym, and `waiver_type` is service_role-set at seed/create + REVOKE'd from clients + immutable for ALL roles) yet editable like any waiver, seeded as a copy of the platform default. Because the backend itself runs at service role, the trigger does NOT protect the API path — `WaiversDelete.delete_waiver` carries its own payer-auth guard, and archiving a `custom` waiver also strips its id from every plan's `waiver_ids` in the same transaction. The backend (service_role) **may hard-delete** the payer-auth waiver during gym-create teardown (`GymsCreateService._cleanup_pending`): the waiver is seeded before the Stripe account is created, so if the Stripe create fails the cleanup must remove the waiver row to leave no dangling rows.
...
- **All access rules go in `access_rules/`, not in `schemas/`.** This includes `ALTER TABLE ... ENABLE ROW LEVEL SECURITY`, `CREATE POLICY`, `REVOKE`, and `GRANT` statements. Each schema file in `schemas/` has a corresponding file in `access_rules/` with the same name. This separation exists to avoid circular dependencies (e.g. RLS policies that reference tables loaded later). The one exception is **`access_rules/zz_client_privileges.sql`** — a schema-wide rule with no owning table, named `zz_` so it sorts last inside the `./access_rules/*.sql` glob in `config.toml` (the CLI expands a glob in sorted order and de-dupes against paths already listed, so alphabetical position is what orders it).
...
only: RLS enabled, with a public SELECT policy (`anon`, `authenticated`) and INSERT/UPDATE/DELETE
revoked from `authenticated` — both defense-in-depth now that the client roles hold no privileges on
any table.

...
### Out-of-order migrations after a merge (distinct from a collision)
A *unique* version prefix can still be **older than the last version already applied** to a database. That happens whenever a branch authors a migration, then merges `main` (or another worktree runs migrations first) and picks up migrations stamped later than its own. Nothing is duplicated, so the `uniq -d` check above passes — but `supabase migration up` refuses to insert behind the high-water mark:

```
Found local migration files to be inserted before the last migration on remote database.
Rerun the command with --include-all flag to apply these migrations:
  …/20260724000000_add_gyms_address.sql
```

- **Fix it by renaming the branch's own migration to a prefix later than every applied one** (`git mv`), then run `supabase migration up` normally. This is the default. It makes filename order match apply order in *every* environment, so no teammate — and no future production deploy — hits the same prompt. It is safe precisely because the migration is not yet applied anywhere; that is also what makes renaming legal here, in contrast to the never-rename-an-applied-migration rule above.
- **`--include-all` is the escape hatch, not the fix.** It applies the straggler in place and leaves the filename permanently out of order, so every environment that already has the newer migrations will need the same flag forever. Reach for it only when the file genuinely cannot be renamed (already applied somewhere you don't control).
- **Before either, confirm the stragglers are order-independent:** read the out-of-order migration and the already-applied newer ones and check they touch unrelated objects. If the late one alters a table or view a newer applied migration also touched, apply order *does* matter and the migration may need rewriting rather than renaming.
- Renaming an unapplied migration changes no DDL, so nothing needs re-verifying beyond re-running the ascending/duplicate checks.
